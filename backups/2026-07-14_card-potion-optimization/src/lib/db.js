...
export async function addRoundArrows(memberId, count) {
  ...
  // 累加今日箭數到 localStorage（取代 Firestore 即時監聽，0 次 Firestore 讀寫）
  const todayKey = `catarrow.today-arrows.${new Date().toISOString().slice(0, 10)}`;
  const prevTotal = Number(localStorage.getItem(todayKey)) || 0;
  localStorage.setItem(todayKey, prevTotal + count);
