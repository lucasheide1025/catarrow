...
import { subscribeResults, subscribeNotifications, subscribeAppVersion, isMemberRegistered,
  ...
  subscribeMyCheckin, submitCheckin, flushPendingShootingSessions,
...
export default function MemberApp() {
  ...
  // 今日箭數從 localStorage 讀取（由 addRoundArrows 在每回合自動累加，0 次 Firestore 讀寫）
  useEffect(() => {
    const todayKey = `catarrow.today-arrows.${new Date().toISOString().slice(0, 10)}`;
    const stored = Number(localStorage.getItem(todayKey)) || 0;
    setTodayArrowsGlobal(stored);
    // 跨分頁監聽：同一瀏覽器開多個分頁時保持同步
    const handler = (e) => {
      if (e.key === todayKey) setTodayArrowsGlobal(Number(e.newValue) || 0);
    };
