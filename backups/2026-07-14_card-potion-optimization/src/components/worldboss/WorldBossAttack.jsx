...
import { addPracticeLog, getCertRecords, subscribeCertification, subscribeCardCollection, addArcherXP, addAdventurerXP, addArrowdew, addGachaCoins, addRoundArrows, addCoins, recordGuestBattleStats, subscribePotions, usePotions, recordPotionUsed, finalizeGameShootingSession } from "../../lib/db";
...
export default function WorldBossAttack({ event, onBack, guestOverride, onComplete }) {
  ...
  useEffect(() => {
    const readFromStorage = () => {
      const key = `catarrow.today-arrows.${new Date().toISOString().slice(0, 10)}`;
      setTodayArrows(Number(localStorage.getItem(key)) || 0);
    };
    readFromStorage();
