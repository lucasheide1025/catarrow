...
function SessionHistoryBrowser({ sessions }) {
  ...
  return <section><ST>完整射擊歷史</ST><Card className="p-3"><p className="text-[11px]" style={{ color:"var(--text-muted)" }}>依目前篩選條件瀏覽本機歷史摘要；每頁 20 場，不會重新下載逐箭資料。</p><div className="mt-3 flex flex-col gap-3">{[...new Map(items.map(item => [month(item), items.filter(other => month(other) === month(item))])).entries()].map(([label, group]) => <div key={label}><div className="mb-1 text-xs font-bold" style={{ color:"var(--text-secondary)" }}>{label}</div>{group.map(item => <div key={item.id} className="flex items-center justify-between border-t py-1.5 text-xs" style={{ borderColor:"var(--border-subtle)" }}><span>{SOURCE_LABELS[item.source?.mode] || item.source?.mode || "射擊紀錄"} ・ {item.shootingConfig?.distanceM ?? "—"}m</span><span className="font-bold text-blue-300">{Number(sessionMetrics(item).averageArrow || 0).toFixed(2)} ／ {sessionMetrics(item).arrowCount || item.arrowCount} 箭</span></div>)}</div>)}</div>{sessions.length > pageSize && <div className="mt-3 flex items-center justify-between"><button type="button" disabled={page === 0} onClick={() => setPage(value => Math.max(0, value - 1))} className="rounded bg-white/10 px-3 py-1 text-xs disabled:opacity-40">上一頁</button><span className="text-xs" style={{ color:"var(--text-muted)" }}>{page + 1}／{Math.ceil(sessions.length / pageSize)} 頁</span><button type="button" disabled={(page + 1) * pageSize >= sessions.length} onClick={() => setPage(value => value + 1)} className="rounded bg-white/10 px-3 py-1 text-xs disabled:opacity-40">下一頁</button></div>}</Card></section>;
}
...
export default function MemberPerformance({ profileOverride = null, coachView = false }) {
  ...
  return <div className="p-4 flex flex-col gap-4" style={{ minHeight:"100dvh", backgroundImage:"url(/ui/page-bg.webp)", backgroundSize:"cover", backgroundPosition:"top center", backgroundAttachment:"local" }}>
    ...
    {tab === "overview" && <>
      ...
      {coachView && <Card className="p-3"><div className="text-sm font-bold" style={{ color:"var(--text-primary)" }}>教練分析摘要</div><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><div className="rounded bg-white/5 p-2"><span style={{ color:"var(--text-muted)" }}>真實射擊</span><div className="mt-1 font-black text-emerald-300">{coachSummary.arrowCount} 箭／{coachSummary.sessionCount} 場</div></div><div className="rounded bg-white/5 p-2"><span style={{ color:"var(--text-muted)" }}>遊戲戰績</span><div className="mt-1 font-black text-indigo-300">{game.count} 場／{game.wins} 勝</div></div></div><div className="mt-3 text-xs" style={{ color:"var(--text-secondary)" }}>模式箭數：{coachSummary.byMode.map(([mode, arrows]) => `${SOURCE_LABELS[mode] || mode} ${arrows}`).join(" ・ ") || "尚無資料"}</div><div className="mt-1 text-xs" style={{ color:"var(--text-secondary)" }}>距離箭數：{coachSummary.byDistance.map(([distance, arrows]) => `${distance}m ${arrows}`).join(" ・ ") || "尚未記錄距離"}</div><p className="mt-2 text-[10px]" style={{ color:"var(--text-muted)" }}>已有距離設定 {coachSummary.configuredSessions}／{coachSummary.sessionCount} 場；未設定距離的舊紀錄不會混入同條件平均。</p></Card>}
      <section><ST>近期趨勢</ST><div className="mb-2 flex gap-1 overflow-x-auto pb-1">{PERIOD_OPTIONS.map(([value, label]) => <button key={value} type="button" onClick={() => setFilters(current => ({ ...current, period:value }))} className={`shrink-0 rounded px-3 py-1.5 text-xs font-bold ${filters.period === value ? "bg-emerald-500 text-white" : "bg-white/10"}`} style={filters.period === value ? undefined : { color:"var(--text-secondary)" }}>{label}</button>)}</div><p className="mt-1 mb-3 text-[10px]" style={{ color:"var(--text-muted)" }}>期間以場次完成時間分類；後續數據皆以此期間為範圍。</p>
