...
exports.UNLOCK_REQS = exports.TIERED_RESOURCES = exports.RESOURCE_NAMES = exports.MAX_COLLECT_HOURS = exports.DEFAULT_VILLAGE = exports.CATDEX_PRODUCTION_MULT = exports.BUILDING_LIST = exports.BUILDINGS = exports.ARROWDEW_COSTS = void 0;
exports.calcPendingResources = calcPendingResources;
exports.canUpgrade = canUpgrade;
exports.getBuildingStage = getBuildingStage;
exports.getDefaultAllocation = getDefaultAllocation;
exports.getMaxSlots = getMaxSlots;
exports.getProductionRate = getProductionRate;
exports.getResourceKey = getResourceKey;
exports.getStageMultiplier = getStageMultiplier;
exports.getUpgradeRequirements = getUpgradeRequirements;
exports.getVillageLastCollectedMs = getVillageLastCollectedMs;
exports.getVillageLevel = getVillageLevel;
exports.getWorkerCatMultiplier = getWorkerCatMultiplier;
exports.isBuildingUnlocked = isBuildingUnlocked;
exports.normalizeBuildingAllocation = normalizeBuildingAllocation;
var _catLevel = require("./catLevel");
var _catVillageAlbums = require("./catVillageAlbums");
// src/lib/villageData.js

const BUILDING_LIST = exports.BUILDING_LIST = ['mine', 'farm', 'harbor', 'hunting', 'market', 'warehouse', 'alchemy', 'gacha', 'archery'];
...
const RESOURCE_NAMES = exports.RESOURCE_NAMES = {
  arrowdew: '箭露',
  ore: '礦物',
  melon: '瓜瓜',
  fish: '鮮魚',
...
// 升等箭露費用（index = 目標等級）
const ARROWDEW_COSTS = exports.ARROWDEW_COSTS = [0, 0, 50, 100, 180, 300, 480, 750, 1150, 1800, 2800, 4200, 6200, 9000, 13000, 19000, 27000, 38000, 54000, 75000, 105000];
...
const MAT = {
  mine: {
    '5-8': ['melon', 1, null, 0],
    '9-12': ['melon', 2, 'meat', 1],
    '13-16': ['melon', 3, 'meat', 2],
    '17-20': ['melon', 4, null, 0, true]
  },
  farm: {
    '5-8': ['ore', 1, null, 0],
    '9-12': ['ore', 2, 'fish', 1],
    '13-16': ['ore', 3, 'potion', 1],
    '17-20': ['ore', 4, null, 0, true]
  },
  harbor: {
    '5-8': ['ore', 1, 'melon', 1],
    '9-12': ['ore', 2, 'melon', 2],
  ...
  market: {
    '5-8': ['ore', 1, 'melon', 1],
    '9-12': ['fish', 1, 'meat', 1],
    '13-16': ['fish', 2, 'potion', 1],
    '17-20': ['fish', 3, null, 0, true]
  },
  warehouse: {
    '5-8': ['meat', 1, 'ore', 1],
    '9-12': ['meat', 2, 'ore', 2],
    '13-16': ['meat', 3, 'driedfish', 1],
    '17-20': ['meat', 4, null, 0, true]
  },
  alchemy: {
    '5-8': ['melon', 1, 'fish', 1],
    '9-12': ['melon', 2, 'fish', 2],
    '13-16': ['melon', 3, 'driedfish', 1],
    '17-20': ['melon', 4, null, 0, true]
  },
  gacha: {
    '5-8': ['driedfish', 1, 'can', 1],
    '9-12': ['driedfish', 2, 'can', 2],
    '13-16': ['driedfish', 3, 'potion', 2],
    '17-20': ['driedfish', 4, null, 0, true]
  },
  archery: {
    '5-8': ['ore', 1, 'can', 1],
    '9-12': ['ore', 2, 'potion', 1],
    '13-16': ['ore', 3, 'fur', 1],
    '17-20': ['ore', 4, 'fur', 3, true]
  }
};
...
function getUpgradeRequirements(buildingId, targetLevel) {
  ...
  const materials = [];
  if (m[0]) materials.push({
    resource: m[0],
    tier: m[1],
    count: mainCount
  });
  if (m[2] && subCount > 0) materials.push({
    resource: m[2],
    tier: m[3],
    count: subCount
  });
...
function getBuildingStage(level) {
  if (level <= 4) return 1;
  if (level <= 8) return 2;
...
const UNLOCK_REQS = exports.UNLOCK_REQS = {
  ...
  // harbor OR hunting >= 2（market 特殊邏輯在 isBuildingUnlocked）
  warehouse: {
    market: 3
  },
  alchemy: {
...
function isBuildingUnlocked(buildingId, buildings) {
...
function getVillageLevel(buildings) {
...
// ── 生產曲線：1.18^(level-1) ──────────────────────────────
// 讓後期成長更有感，Lv.20 ≈ 21×
function getProductionRate(buildingId, level) {
...
const MAX_COLLECT_HOURS = exports.MAX_COLLECT_HOURS = 24;

// ── 貓貓圖鑑生產加乘（預留，尚未實裝）──────────────────────
// 未來「貓貓圖鑑」實裝後，依收集進度回傳 >1 的倍率，透過
// calcPendingResources(village, { catDexMult }) 傳入放大全村產能。
// 現在恆為 1.0 → 不影響任何現有平衡，只是先把接線留好。
const CATDEX_PRODUCTION_MULT = exports.CATDEX_PRODUCTION_MULT = 1.0;

// 需要 tier 後綴的資源（_t1~_t5）
const TIERED_RESOURCES = exports.TIERED_RESOURCES = new Set(['ore', 'melon', 'fish', 'meat', 'driedfish', 'can', 'potion', 'fur', 'archer']);

// 取得正確的 Firestore 資源 key
function getResourceKey(resource, tier) {
...
function getDefaultAllocation(lv) {
...
function normalizeBuildingAllocation(level, rawAllocation) {
...
function getVillageLastCollectedMs(value, now = Date.now()) {
  const timestampMs = value?.toMillis?.();
  const parsed = Number.isFinite(timestampMs) ? timestampMs : value instanceof Date ? value.getTime() : typeof value === "number" ? value : typeof value === "string" ? Date.parse(value) : NaN;
  return Number.isFinite(parsed) && parsed > 0 ? Math.min(parsed, now) : now - 3600000;
}

// ── 計算建築駐紮貓咪的生產加乘倍率 ────────────────────────
// 基礎產能加乘 = 1 + (貓咪等級 × 2%) + (羈絆等級 × 0.5%)
function getWorkerCatMultiplier(catData) {
...
// ── 計算待採集資源（前端預覽用）───────────────────────────
function calcPendingResources(village, opts = {}) {
  const now = Date.now();
  ...
  for (const id of BUILDING_LIST) {
    ...
    const albumMult = (0, _catVillageAlbums.villageAlbumMultiplier)(albumXp[id]);
...
function canUpgrade(buildingId, buildings, resources) {
  ...
  if ((resources?.arrowdew || 0) < req.arrowdew) {
    return {
      ok: false,
      reason: `箭露不足（需 ${req.arrowdew.toLocaleString()}）`
    };
  }
  for (const mat of req.materials) {
    ...
    if ((resources?.[resKey] || 0) < mat.count) {
      return {
        ok: false,
        reason: `${RESOURCE_NAMES[mat.resource] || mat.resource} T${mat.tier} 不足`
      };
    }
  }
  return {
...
const DEFAULT_VILLAGE = exports.DEFAULT_VILLAGE = {
  buildings: {
    mine: 1,
    farm: 1,
    harbor: 1,
