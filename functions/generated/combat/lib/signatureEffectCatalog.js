...
exports.TIER_SKILL_ATK_MULT = exports.SIGNATURE_EFFECTS = void 0;
exports.getSignatureEffect = getSignatureEffect;
exports.parsePhasePassives = parsePhasePassives;
exports.parseSignatureSegment = parseSignatureSegment;
exports.parseSignatureSummary = parseSignatureSummary;
exports.validateSignatureEffects = validateSignatureEffects;
...
const TIER_SKILL_ATK_MULT = exports.TIER_SKILL_ATK_MULT = Object.freeze({
  1: {
    normal: 1.05,
    miniBoss: 1.10,
    boss: 1.15
...
function parseSignatureSegment(segment) {
  const s = segment.trim();
  if (!s || s === "無傷害" || s === "不附加狀態") return {
    type: "none"
  };
  let m;
  // N段攻擊，總倍率=基準(×K)(，無視護盾N%)
  if (m = s.match(/^(\d)段(?:光翼)?攻擊，總倍率=(基準|小王基準|大王基準)(?:×([\d.]+))?(?:，無視(防禦|護盾)([\d.]+)%)?$/)) {
    return {
      type: "damage",
      hits: num(m[1]),
      baseKind: BASE_KIND[m[2]],
      mult: m[3] ? num(m[3]) : 1,
      pierceDefPct: m[4] === "防禦" ? num(m[5]) : 0,
      pierceShieldPct: m[4] === "護盾" ? num(m[5]) : 0
    };
  }
  // 延遲攻擊基準×K(於下回合)
  ...
  if (m = s.match(/^(基準傷害|基準|小王基準|大王基準)(?:×([\d.]+))?(?:，無視(防禦|護盾)([\d.]+)%)?$/)) {
    return {
      ...
      baseKind: BASE_KIND[m[1]],
      mult: m[2] ? num(m[2]) : 1,
      pierceDefPct: m[3] === "防禦" ? num(m[4]) : 0,
      pierceShieldPct: m[3] === "護盾" ? num(m[4]) : 0
    };
  }
...
function parsePhaseClause(clause) {
  ...
  let m;
  if (m = s.match(/^(?:護盾量?\+?|獲一次)([\d.]+)%?護?盾?$/)) return {
    shieldPct: Number(m[1])
  };
  if (m = s.match(/^(?:技能傷害|基準傷害|多段總傷|總傷|傷害)\+([\d.]+)%$/)) return {
    damagePct: Number(m[1])
  };
  if (m = s.match(/^減傷\+?([\d.]+)%$/)) return {
    reductionPct: Number(m[1])
  };
  if (m = s.match(/^反射\+([\d.]+)%$/)) return {
    reflectPct: Number(m[1])
  };
  if (m = s.match(/^狀態幅度\+([\d.]+)%$/)) return {
    statusPct: Number(m[1])
  };
  if (m = s.match(/^延遲段\+([\d.]+)%$/)) return {
    delayedPct: Number(m[1])
  };
  if (m = s.match(/^(?:穿盾|無視護盾再)\+([\d.]+)%$/)) return {
    shieldPiercePct: Number(m[1])
  };
  return null;
}
function parsePhasePassives(counterSummary = "") {
  const match = counterSummary.match(/70%\s*HP(.+?)，40%\s*HP(.+)$/);
  if (!match) return null;
  const p70 = parsePhaseClause(match[1]);
  const p40 = parsePhaseClause(match[2]);
  if (!p70 || !p40) return {
    error: counterSummary
  };
  return {
    p70,
    p40
...
function parseSignatureSummary(monster) {
...
const SIGNATURE_EFFECTS = exports.SIGNATURE_EFFECTS = Object.freeze(Object.fromEntries(_monsterExpansionCatalog.EXPANSION_MONSTERS.filter(monster => monster.signatureSkillId).map(monster => [monster.signatureSkillId, parseSignatureSummary(monster)])));
function getSignatureEffect(skillId) {
  return SIGNATURE_EFFECTS[skillId] || null;
}

// 完整性驗證（PRD 72 前置檢查）：252 全解析、1~3 積木、傷害基準與遭遇類型一致
function validateSignatureEffects() {
  const problems = [];
  const monstersById = Object.fromEntries(_monsterExpansionCatalog.EXPANSION_MONSTERS.map(monster => [monster.id, monster]));
  ...
  for (const entry of entries) {
    ...
    for (const block of entry.blocks) {
      if ((block.type === "damage" || block.type === "delayedBurst") && block.baseKind && monster && block.baseKind !== monster.encounter) {
