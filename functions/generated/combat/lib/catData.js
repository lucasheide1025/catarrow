...
exports.CHAPTER_BOND_LV = exports.CAT_TYPE_MAP = exports.CAT_TYPES = exports.CAT_SKILL_GROUPS = exports.CAT_IDS = exports.CAT_FORGE_COSTS = exports.CAT_EQUIP_SLOTS = exports.CAT_EQUIP_MAX_PLUS = exports.CAT_EQUIP_MAX_ENHANCEMENT = exports.CAT_EQUIP_GRADE_NAMES = exports.CAT_EQUIP_GRADE_COLORS = exports.CAT_EQUIP_GRADE_BG = exports.CAT_COMBAT_BASE = exports.CAT_BUILD_PROFILES = exports.CATS = exports.BOND_THRESHOLDS = void 0;
exports.calcCatEquipBonus = calcCatEquipBonus;
exports.calcCatSkillChance = calcCatSkillChance;
exports.calcCatSkillEffect = calcCatSkillEffect;
exports.calcForgeCost = calcForgeCost;
exports.catEquipEnhancement = catEquipEnhancement;
exports.catEquipLevel = catEquipLevel;
exports.drawRandomCat = drawRandomCat;
exports.getBondLevel = getBondLevel;
exports.getBondProgress = getBondProgress;
exports.getCatBattleBonus = getCatBattleBonus;
exports.getCatChapters = getCatChapters;
exports.getCatStatMult = getCatStatMult;
// src/lib/catData.js — 貓貓陪練系統資料定義

// ── 九隻貓咪 ─────────────────────────────────────────────────
const CATS = exports.CATS = {
  daming: {
    id: "daming",
    name: "大娘",
    color: "玳瑁貓",
...
const CAT_IDS = exports.CAT_IDS = Object.keys(CATS);
const CAT_COMBAT_BASE = exports.CAT_COMBAT_BASE = {
  attack: {
    hp: 150,
    atk: 18,
    def: 7
...
const CAT_BUILD_PROFILES = exports.CAT_BUILD_PROFILES = {
  daming: {
    title: "守護治療",
    allocation: {
      hp: 1.18,
...
// desc = 純故事敘述，無任何機制加成
const CAT_TYPES = exports.CAT_TYPES = {
  attack: {
    id: "attack",
    label: "攻擊型",
    icon: "⚔️",
...
// ── 羈絆等級閾值（1 ~ 50 級）─────────────────────────────────
const BOND_THRESHOLDS = exports.BOND_THRESHOLDS = Array.from({
  length: 51
}, (_, i) => {
  if (i === 0) return 0;
...
function getBondLevel(bond) {
  ...
  for (let i = 0; i < BOND_THRESHOLDS.length; i++) {
    if (bond >= BOND_THRESHOLDS[i]) lv = i;else break;
  }
  return Math.min(50, lv); // 0–50
}
function getBondProgress(bond) {
  ...
  return {
    lv,
    pct: Math.round((bond - cur) / (next - cur) * 100),
    toNext: next - bond
  };
}
...
const CHAPTER_BOND_LV = exports.CHAPTER_BOND_LV = [null, 0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50];
//                                    ch1 ch2 ch3 ch4 ch5 ch6 ch7 ch8 ch9 ch10 ch11

// ── 章節定義（故事文字留白，等現實故事提供後填入）────────────
function getCatChapters(catId, isDeceased = false) {
  const chapters = [{
    ch: 1,
    title: "第一次相遇",
    bondLv: 0,
...
// ── 羈絆加成（取決於類型和羈絆等級）────────────────────────
function getCatBattleBonus(catType, bondLevel) {
  const lv = bondLevel || 0;
  if (catType === "attack") return {
    atkMult: 1 + lv * 0.02,
    critsBonus: lv * 0.01
  };
  if (catType === "defense") return {
    defBonus: lv * 3,
    hpBonus: lv * 20
  };
  // allround
...
function getCatStatMult(catType, bondLevel) {
  const lv = bondLevel || 0;
  if (!lv) return 1.0;
...
// ── 抽貓機率（開貓貓箱）────────────────────────────────────
function drawRandomCat(ownedCatIds = []) {
...
// ── 貓貓類型固定對應（位置決定，不可自選）────────────────
const CAT_TYPE_MAP = exports.CAT_TYPE_MAP = {
  daming: "allround",
  gege: "allround",
  meimei: "allround",
  niuniu: "attack",
...
const CAT_SKILL_GROUPS = exports.CAT_SKILL_GROUPS = {
  daming: "heal",
  gege: "heal",
  meimei: "heal",
  niuniu: "atk",
...
const CAT_EQUIP_SLOTS = exports.CAT_EQUIP_SLOTS = [{
  id: "bow",
  label: "弓",
  icon: "🏹",
  image: "/assets/cat_equip/bow.jpg",
  stat: "atk",
  matKey: "ore"
}, {
  id: "arrow",
  label: "箭",
  icon: "🪃",
  image: "/assets/cat_equip/arrow.jpg",
  stat: "atk",
  matKey: "melon"
}, {
  id: "arrowhead",
  label: "箭頭",
  icon: "🗡️",
  image: "/assets/cat_equip/arrowhead.jpg",
  stat: "atk",
  matKey: "fish"
}, {
  id: "armor",
  label: "護胸",
  icon: "🛡️",
  image: "/assets/cat_equip/armor.jpg",
  stat: "def",
  matKey: "meat"
}, {
  id: "shoulder",
  label: "肩甲",
  icon: "🦺",
  image: "/assets/cat_equip/shoulder.jpg",
  stat: "def",
  matKey: "driedfish"
}, {
  id: "herbBag",
  label: "貓草包",
  icon: "🌿",
  image: "/assets/cat_equip/herbBag.jpg",
  stat: "def",
  matKey: "can"
}, {
  id: "potion",
  label: "貓薄荷藥水",
  icon: "🍵",
  image: "/assets/cat_equip/potion.jpg",
  stat: "hp",
  matKey: "potion"
}];

// 六品質：普通→稀有→精英→頭目→傳說→神話（史詩改為頭目/Boss）
const CAT_EQUIP_GRADE_NAMES = exports.CAT_EQUIP_GRADE_NAMES = ["普通", "稀有", "精英", "頭目", "傳說", "神話"];
const CAT_EQUIP_GRADE_COLORS = exports.CAT_EQUIP_GRADE_COLORS = ["#9ca3af", "#22c55e", "#3b82f6", "#f97316", "#f59e0b", "#ef4444"];
// 每品質顯示底色（帶透明度，用於UI背景）
const CAT_EQUIP_GRADE_BG = exports.CAT_EQUIP_GRADE_BG = ["rgba(156,163,175,0.15)",
// 普通 gray
"rgba(34,197,94,0.15)",
// 稀有 green
...
const CAT_EQUIP_MAX_PLUS = exports.CAT_EQUIP_MAX_PLUS = 9; // 每個品質內保留 +0～+9 的既有儲存格式
const CAT_EQUIP_MAX_ENHANCEMENT = exports.CAT_EQUIP_MAX_ENHANCEMENT = 50;
function catEquipEnhancement(grade, plusLevel) {
...
function calcCatEquipBonus(equip = {}) {
  ...
  for (const s of CAT_EQUIP_SLOTS) {
    ...
    if (s.stat === "atk") atkBonus += bonus;else if (s.stat === "def") defBonus += bonus;else if (s.stat === "hp") hpBonus += bonus * 5;
  }
  return {
    atkBonus,
    defBonus,
...
const CAT_FORGE_COSTS = exports.CAT_FORGE_COSTS = {
  plusUpgrades: [{
    tier: 1,
    amount: 5
  },
...
function calcForgeCost(slotId, currentGrade, currentPlus) {
...
// ── 貓貓技能觸發機率（0–0.25）──────────────────────────────
function calcCatSkillChance(catLevel, bondLv, catId = null) {
...
function calcCatSkillEffect(skillGroup, catLevel, bondLv, catId = null) {
  const lv = catLevel || 1;
  const bl = bondLv || 0;
