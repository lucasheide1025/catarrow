...
exports.THROW_POTIONS = exports.RAID_POTIONS = exports.POTION_CATALOG_VERSION = exports.POTIONS = exports.FRAGMENTS = exports.CHEST_TYPES = exports.CARRY_POTIONS = void 0;
exports.calcPotionBuffs = calcPotionBuffs;
exports.getExpansionChestMaterialPool = getExpansionChestMaterialPool;
exports.getFragment = getFragment;
exports.getMonsterCardPackPool = getMonsterCardPackPool;
exports.getPotion = getPotion;
exports.makeBossChest = makeBossChest;
exports.makeChest = makeChest;
exports.makeChests = makeChests;
exports.makeFamilyMaterialChest = makeFamilyMaterialChest;
exports.makeMiniBossChest = makeMiniBossChest;
exports.openChestContents = openChestContents;
exports.rollChestType = rollChestType;
var _monsterMaterials = require("./monsterMaterials");
...
// ── 寶箱類型 ─────────────────────────────────────────────
const CHEST_TYPES = exports.CHEST_TYPES = {
  wood: {
    id: "wood",
    name: "通用材料木箱",
    icon: "📦",
...
function rollChestType(tier, mode) {
  const table = (mode === "student" ? CHEST_ROLL_STUDENT : CHEST_ROLL)[tier] || CHEST_ROLL.common;
  let r = Math.random();
...
function makeChests(monster, mode) {
  const now = Date.now();
  ...
  const canon = _monsterExpansionCatalog.EXPANSION_MONSTER_BY_ID[monster.id] || null;
  const family = monster.family || canon?.family;
  const tierIndex = monster.tierIndex || canon?.tierIndex || MONSTER_TIER_ORDER.indexOf(monster.tier) + 1;
  const encounter = monster.encounter || canon?.encounter || (monster.isKing ? "boss" : "normal");
  const useFamilyChest = ALL_FAMILIES.includes(family) && tierIndex >= 1;
  const mkNew = () => encounter === "boss" ? makeBossChest(family, tierIndex, monster.name) : encounter === "miniBoss" ? makeMiniBossChest(family, tierIndex, monster.name) : makeFamilyMaterialChest(family, tierIndex, monster.name);
  const mkOld = (extra = 0) => ({
...
function makeChest(monster) {
  return makeChests(monster).mainChest;
}

// ── 章碎片定義 ────────────────────────────────────────────
// 10 個碎片 → 合成對應章
const FRAGMENTS = exports.FRAGMENTS = [{
...
const carry = ({
  ...
  kind: "carry",
  category: "carry",
  battleModes: ["monster", "party", "dungeon", "worldboss"],
  actionCost: "utility",
  effect,
  effectText,
  desc,
...
const throwable = ({
  ...
  kind: "throw",
  category: "throw",
  battleModes: ["monster", "party", "dungeon"],
  actionCost,
  effect,
  effectText,
  desc,
...
const POTION_CATALOG_VERSION = exports.POTION_CATALOG_VERSION = 2;
const POTIONS = exports.POTIONS = [
// ══ 攜帶型：HP恢復（3 級）════════════════════════════════
carry({
  id: "carry_heal_basic",
  family: "heal",
  ...
  gold: 100,
  craftYield: 3,
  futureFeature: "status_effects"
}), carry({
  id: "carry_cleanse_advanced",
  family: "cleanse",
  level: 2,
  name: "高級淨化藥",
  icon: "🌟",
  ...
  effect: {
    cleanseAll: true,
    statusResistRounds: 1
  },
  effectText: "清除全部異常並獲得抗性（預備）",
  desc: "異常狀態系統開放後可使用。",
  recipe: [{
    id: "potion_t2",
    count: 2
  ...
  effectText: "每大回合 ATK 40%，持續 3 回合",
  desc: "重複使用只刷新持續時間。",
  recipe: [{
    id: "potion_t2",
    count: 1
  ...
  effectText: "全隊對目標傷害 +10%",
  desc: "強力團隊用品，每個目標只保留一層。",
  recipe: [{
    id: "potion_t3",
    count: 1
  }, {
  ...
  effect: {
    skipRound: "big",
    bossCounterReducePct: 50
  },
  effectText: "停止一次反擊；頭目改為減傷",
  desc: "普通怪停止反擊，頭目下一次反擊減半。",
  recipe: [{
    id: "potion_t2",
    count: 1
  }, {
  ...
  effectText: "ATK 120%；低血量時 260%",
  desc: "世界王低於 25% HP 時發揮終結威力。",
  recipe: [{
    id: "potion_t3",
    count: 1
  }, {
...
// 攜帶型藥水分類輔助
const CARRY_POTIONS = exports.CARRY_POTIONS = POTIONS.filter(p => p.kind === "carry");
const THROW_POTIONS = exports.THROW_POTIONS = POTIONS.filter(p => p.kind === "throw");
const RAID_POTIONS = exports.RAID_POTIONS = POTIONS.filter(p => p.kind === "raid");
...
function getExpansionChestMaterialPool(chestType, family, tierIndex) {
  const kinds = EXPANSION_KINDS_BY_CHEST[chestType];
  if (!kinds) return [];
  return _monsterExpansionCatalog.EXPANSION_MATERIALS.filter(material => material.family === family && material.tierIndex === tierIndex && kinds.has(material.kind));
}
...
function makeFamilyMaterialChest(family, tierIndex, source = "掉落") {
...
function makeMiniBossChest(family, tierIndex, source = "掉落") {
  const name = FAMILY_NAMES[family] || family;
  const now = Date.now();
  return {
...
function makeBossChest(family, tierIndex, source = "掉落") {
  const name = FAMILY_NAMES[family] || family;
  const now = Date.now();
  return {
...
function getMonsterCardPackPool() {
  return _monsterExpansionCatalog.EXPANSION_MONSTERS.filter(monster => monster.encounter === "normal").map(monster => ({
...
function drawRandomCards(count = 3) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let tier = "common";
    for (const [t, w] of Object.entries(CARD_TIER_WEIGHT)) {
      r -= w;
      if (r <= 0) {
        tier = t;
        break;
...
const WB_RELIC_COIN_RANGE = {
  min: 400,
  max: 800
};
const WB_RELIC_MATERIAL_RANGE = {
  min: 3,
  max: 7
};
...
function openWorldBossRelic() {
  ...
  const wbMaterial = _monsterMaterials.MATERIALS.find(m => m.id === "wb_relic_shard");
  const materials = wbMaterial ? Array.from({
    length: matCount
  }, () => wbMaterial) : [];
  return {
    coins,
...
function openChestContents(chest) {
  ...
  if (chest.type === "potion") {
    ...
    for (const entry of POTION_CHEST_TABLE) {
      r -= entry.weight;
      if (r <= 0) {
        potionId = entry.id;
        break;
  ...
  if (chest.type === "family_mat") {
    ...
    const pool = _monsterExpansionCatalog.EXPANSION_MATERIALS.filter(m => m.family === family && m.tierIndex === tierIndex && m.kind === "normal");
  ...
  if (chest.type === "mini_boss_mat" || chest.type === "boss_mat") {
    ...
    const pool = _monsterExpansionCatalog.EXPANSION_MATERIALS.filter(m => m.family === family && m.tierIndex === tierIndex);
    const count = chest.type === "boss_mat" ? 3 + Math.floor(Math.random() * 4) // 大王 3~6 個
  ...
  for (const fam of ALL_FAMILIES) {
    const pool = _monsterExpansionCatalog.EXPANSION_MATERIALS.filter(m => m.family === fam && m.tierIndex === uniTierIndex && uniKinds.has(m.kind));
    if (!pool.length) continue;
...
//   used: [],           // 使用的藥水物件
// }
function calcPotionBuffs(potionIds) {
  const buffs = {
    hpPct: 0,
    atkPct: 0,
    defPct: 0,
