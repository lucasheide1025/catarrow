...
exports.WORLD_BOSS_CARD_DEX_GROUPS = exports.WORLD_BOSS_CARD_DEX_FAMILIES = exports.VILLAGE_BUILDING_DEX = exports.TIERED_ACHIEVEMENTS = exports.SPECIAL_GRANTS = exports.REPLACED_BY_TIERED = exports.RARITY_STYLE = exports.RANK_STYLE = exports.MONSTER_DEX_FAMILIES = exports.MONSTER_DEX_CATALOG = exports.MONSTER_CARD_PACK = exports.MONSTER_CARD_FAMILIES = exports.MONSTER_CARD_DEX_GROUPS = exports.EXTERNAL_COMP_FORMATS = exports.DEX_THEMES = exports.DEX_CATEGORY_THEME = exports.DEX_CATEGORIES = exports.CHEST_DEX_TYPES = exports.AUTO_ACHIEVEMENTS = void 0;
exports.buildAnnualCertificationAchievements = buildAnnualCertificationAchievements;
exports.buildArcherLevelAchievement = buildArcherLevelAchievement;
exports.buildArcheryTenureAchievement = buildArcheryTenureAchievement;
exports.buildCohortAchievement = buildCohortAchievement;
exports.buildCohortAchievements = buildCohortAchievements;
exports.buildExternalCompetitionAchievements = buildExternalCompetitionAchievements;
exports.buildRoundAchievements = buildRoundAchievements;
exports.computeDexStats = computeDexStats;
exports.computeTierProgress = computeTierProgress;
exports.countDefeatedCatalogMonsters = countDefeatedCatalogMonsters;
exports.describeKey = describeKey;
exports.getUnlockedKeys = getUnlockedKeys;
exports.isActiveAchievement = isActiveAchievement;
...
var _adventurerSystem = require("./adventurerSystem");
var _dungeonCollectibles = require("./dungeonCollectibles");
var _worldBossData = require("./worldBossData");
var _villageData = require("./villageData");
var _villageShop = require("./villageShop");
var _worldBossCards = require("./worldBossCards");
var _catData = require("./catData");
var _catLevel = require("./catLevel");
var _catCardData = require("./catCardData");
var _boardJourney = require("./boardJourney");
var _guildRank = require("../guild/domain/guildRank");
...
function catAchievementLevel(cats, catId) {
  ...
  return cat ? (0, _catLevel.catLevelFromXP)(cat.catXP || 0) : 0;
}
function catAchievementBondLevel(cats, catId) {
  const cat = findAchievementCat(cats, catId);
  return cat ? (0, _catData.getBondLevel)(cat.bond || 0) : 0;
}
function catAchievementEquipmentLevel(cats, catId) {
  const cat = findAchievementCat(cats, catId);
  if (!cat || _catData.CAT_EQUIP_SLOTS.length === 0) return 0;
  const total = _catData.CAT_EQUIP_SLOTS.reduce((sum, slot) => {
    const slotId = typeof slot === "string" ? slot : slot.id;
    const item = cat.equip?.[slotId];
    return sum + (0, _catData.catEquipEnhancement)(item?.grade || "普通", item?.plusLevel || 0);
  }, 0);
  return Math.floor(total / _catData.CAT_EQUIP_SLOTS.length);
}
const RARITY_STYLE = exports.RARITY_STYLE = {
  common: {
    ring: "#cbd5e1",
    glow: "none",
    label: "普通"
  ...
  uncommon: {
    ring: "#22c55e",
    glow: "0 0 8px rgba(34,197,94,.45)",
    label: "非凡"
  },
  rare: {
    ring: "#3b82f6",
    glow: "0 0 10px rgba(59,130,246,.5)",
    label: "稀有"
  },
  epic: {
    ring: "#a855f7",
    glow: "0 0 12px rgba(168,85,247,.6)",
    label: "史詩"
  },
  legendary: {
    ring: "#f59e0b",
    glow: "0 0 16px rgba(245,158,11,.7)",
    label: "傳說"
  },
  mythic: {
    ring: "#ef4444",
    glow: "0 0 20px rgba(239,68,68,.8)",
    label: "神話"
  }
};
const RANK_STYLE = exports.RANK_STYLE = {
  1: {
    ring: "#f59e0b",
    glow: "0 0 16px rgba(245,158,11,.7)",
    icon: "🥇",
  ...
  2: {
    ring: "#94a3b8",
    glow: "0 0 12px rgba(148,163,184,.6)",
    icon: "🥈",
    label: "亞軍"
  },
  3: {
    ring: "#b45309",
    glow: "0 0 12px rgba(180,83,9,.5)",
    icon: "🥉",
    label: "季軍"
  },
  0: {
    ring: "#0ea5e9",
    glow: "0 0 8px rgba(14,165,233,.4)",
    icon: "🎯",
    label: "參賽"
  }
};
const DEX_CATEGORIES = exports.DEX_CATEGORIES = [{
  id: "start",
  label: "🌱 啟程"
}, {
  id: "practice",
...
const DEX_THEMES = exports.DEX_THEMES = [{
  id: "career",
  label: "🎯 射手生涯",
  categories: ["start", "practice", "cohort", "archer_level", "archery_tenure", "cert", "level"]
}, {
  id: "honor",
  label: "🏆 榮耀紀錄",
  categories: ["collect", "physical", "point", "external", "special"]
}, {
  id: "combat",
  label: "⚔️ 戰鬥",
  categories: ["monster", "monster_miniboss", "monster_boss", "duel", "chest"]
}, {
  id: "worldboss",
  label: "🐲 世界王",
  categories: ["worldboss_participation", "worldboss_kill", "worldboss_rank"]
}, {
  id: "adventure",
  label: "🗺️ 冒險",
  categories: ["guild", "dungeon"]
}, {
  id: "collection",
  label: "🃏 收藏",
  categories: ["card", "wbcard"]
}, {
  id: "cat",
  label: "🐈 貓小隊",
  categories: ["cat"]
  ...
  id: "village",
  label: "🏘️ 貓貓村",
  categories: ["village", "shop"]
}, {
  id: "growth",
  label: "🛡️ 養成",
  categories: ["equip", "forge"]
}];
const DEX_CATEGORY_THEME = exports.DEX_CATEGORY_THEME = Object.freeze(Object.fromEntries(DEX_THEMES.flatMap(theme => theme.categories.map(cat => [cat, theme.id]))));
const EXTERNAL_COMP_FORMATS = exports.EXTERNAL_COMP_FORMATS = Object.freeze([Object.freeze({
  id: "qualification",
  label: "資格賽"
}), Object.freeze({
  id: "mixed",
...
const EXTERNAL_COMP_FORMAT_MAP = Object.freeze(Object.fromEntries(EXTERNAL_COMP_FORMATS.map(item => [item.id, item])));
...
function isActiveAchievement(achievement) {
  return !!achievement && achievement.retired !== true && achievement.futureData !== true;
}

// 一般怪物卡包的單一真本：擴充目錄中 encounter=normal 的怪物。
const MONSTER_CARD_PACK = exports.MONSTER_CARD_PACK = Object.freeze(_monsterExpansionCatalog.EXPANSION_MONSTERS.filter(monster => monster.encounter === "normal"));
const MONSTER_CARD_FAMILIES = exports.MONSTER_CARD_FAMILIES = Object.freeze([...new Set(MONSTER_CARD_PACK.map(card => card.family).filter(Boolean))]);
const WORLD_BOSS_CARD_DEX_FAMILIES = exports.WORLD_BOSS_CARD_DEX_FAMILIES = Object.freeze(Object.entries(_worldBossCards.WB_CARD_KEYS.reduce((groups, key) => {
  const family = _worldBossCards.WB_CARDS[key]?.family;
  if (!family) return groups;
  if (!groups[family]) groups[family] = [];
  groups[family].push(key);
  return groups;
}, {})).map(([family, cardKeys]) => Object.freeze({
  family,
...
const WORLD_BOSS_CARD_DEX_GROUPS = exports.WORLD_BOSS_CARD_DEX_GROUPS = Object.freeze(Object.entries(WORLD_BOSS_CARD_GROUP_META).map(([id, meta]) => Object.freeze({
  id,
  ...meta,
  cardKeys: Object.freeze(_worldBossCards.WB_CARD_KEYS.filter(key => (0, _worldBossData.getDropCategory)(_worldBossData.WORLD_BOSSES[key]) === id))
})));
const MONSTER_CARD_DEX_GROUPS = exports.MONSTER_CARD_DEX_GROUPS = Object.freeze(MONSTER_CARD_FAMILIES.flatMap(family => [1, 2, 3, 4, 5, 6].map(tierIndex => {
...
function villageDevelopmentTotal(buildings) {
  return _villageData.BUILDING_LIST.reduce((sum, buildingId) => sum + canonicalVillageBuildingLevel(buildings, buildingId), 0);
}
const VILLAGE_BUILDING_DEX = exports.VILLAGE_BUILDING_DEX = Object.freeze(_villageData.BUILDING_LIST.map(buildingId => Object.freeze({
  id: buildingId,
  name: _villageData.BUILDINGS[buildingId]?.name || buildingId,
  icon: _villageData.BUILDINGS[buildingId]?.emoji || "🏗️"
})));
function shopStatsFromContext(c) {
  return c.member?.village?.shop?.stats || {};
}
const SHOP_CUSTOMER_IDS = new Set(_villageShop.SHOP_CUSTOMERS.map(customer => customer.id));
...
const MONSTER_DEX_CATALOG = exports.MONSTER_DEX_CATALOG = _monsterExpansionCatalog.EXPANSION_MONSTERS;
const MONSTER_DEX_FAMILIES = exports.MONSTER_DEX_FAMILIES = Object.freeze([...new Set(MONSTER_DEX_CATALOG.map(monster => monster.family).filter(Boolean))]);
...
function countDefeatedCatalogMonsters(monsterDex, family = null) {
...
function sumCatalogMonsterWins(monsterDex, monsterIds) {
  return (monsterIds || []).reduce((sum, id) => sum + Math.max(0, Number(monsterDex?.[id]?.wins) || 0), 0);
}

// 地下城目前是七族；收藏品系統仍是舊六族，不能拿 FAMILY_COLLECTIBLES 當通關族群來源。
const DUNGEON_CLEAR_FAMILIES = Object.freeze(["ghost", "mountain", "insect", "workplace", "exam", "temple", "treasure"]);
function dungeonClearTotal(member) {
  const clears = member?.dungeonClears || {};
  return DUNGEON_CLEAR_FAMILIES.reduce((sum, family) => sum + Math.max(0, Math.floor(Number(clears?.[family]) || 0)), 0);
}
...
function bowAtLeast(certRecords, bowType, minLevel) {
  ...
  const lv = (0, _constants.getCertLevel)(bowType, best);
...
// ── AUTO_ACHIEVEMENTS ────────────────────────────────────────
const AUTO_ACHIEVEMENTS = exports.AUTO_ACHIEVEMENTS = [
// ══ 啟程 ══
{
  id: "checkin_1",
  cat: "start",
  ...
  rarity: "common",
  desc: "第一次參加年度檢定",
  check: c => (c.certRecords || []).length >= 1
},
...
// ══ 射手證 ══
{
  ...
  check: c => {
    ...
    return certTaskVal(c.certification, "blue", "task1", "hits") >= 6 && certTaskVal(c.certification, "blue", "task2", "score") >= 100;
  }
}, {
  id: "cert_blue_great",
  cat: "cert",
  ...
  check: c => {
    if (!certTaskPassed(c.certification, "blue", "task1")) return false;
    if (!certTaskPassed(c.certification, "blue", "task2")) return false;
    return certTaskVal(c.certification, "blue", "task1", "hits") >= 5 && certTaskVal(c.certification, "blue", "task2", "score") >= 90;
  }
}, {
  id: "cert_gold",
  cat: "cert",
  ...
  check: c => {
    ...
    return certTaskVal(c.certification, "gold", "task1", "hits") >= 6 && certTaskVal(c.certification, "gold", "task2", "score") >= 100;
  }
}, {
  id: "cert_gold_great",
  cat: "cert",
  ...
  check: c => {
    if (c.certification?.level !== "gold") return false;
    return certTaskVal(c.certification, "gold", "task1", "hits") >= 5 && certTaskVal(c.certification, "gold", "task2", "score") >= 90;
  }
},
// 射手證編號
{
...
// ══ 檢定 — 裸弓 ══
{
  ...
  rarity: "common",
  desc: "裸弓年度檢定達到入門",
  check: c => bowAtLeast(c.certRecords, "recurve_bare", "入門")
}, {
  id: "bare_basic",
  cat: "level",
  icon: "🏹",
  ...
  rarity: "common",
  desc: "裸弓年度檢定達到初級",
  check: c => bowAtLeast(c.certRecords, "recurve_bare", "初級")
}, {
  id: "bare_mid",
  cat: "level",
  icon: "🏹",
  ...
  rarity: "uncommon",
  desc: "裸弓年度檢定達到中級",
  check: c => bowAtLeast(c.certRecords, "recurve_bare", "中級")
}, {
  id: "bare_adv",
  cat: "level",
  icon: "🏹",
  ...
  rarity: "rare",
  desc: "裸弓年度檢定達到進階",
  check: c => bowAtLeast(c.certRecords, "recurve_bare", "進階")
}, {
  id: "bare_elite",
  cat: "level",
  icon: "🏹",
  ...
  rarity: "epic",
  desc: "裸弓年度檢定達到精英",
  check: c => bowAtLeast(c.certRecords, "recurve_bare", "精英")
},
// ══ 檢定 — 獵弓 ══
{
  id: "comp_entry",
  ...
  rarity: "common",
  desc: "獵弓年度檢定達到入門",
  check: c => bowAtLeast(c.certRecords, "compound", "入門")
}, {
  id: "comp_basic",
  cat: "level",
  icon: "🦅",
  ...
  rarity: "common",
  desc: "獵弓年度檢定達到初級",
  check: c => bowAtLeast(c.certRecords, "compound", "初級")
}, {
  id: "comp_mid",
  cat: "level",
  icon: "🦅",
  ...
  rarity: "uncommon",
  desc: "獵弓年度檢定達到中級",
  check: c => bowAtLeast(c.certRecords, "compound", "中級")
}, {
  id: "comp_adv",
  cat: "level",
  icon: "🦅",
  ...
  rarity: "rare",
  desc: "獵弓年度檢定達到進階",
  check: c => bowAtLeast(c.certRecords, "compound", "進階")
}, {
  id: "comp_elite",
  cat: "level",
  icon: "🦅",
  ...
  rarity: "epic",
  desc: "獵弓年度檢定達到精英",
  check: c => bowAtLeast(c.certRecords, "compound", "精英")
},
// ══ 檢定 — 傳統弓 ══
{
  id: "trad_entry",
  ...
  rarity: "common",
  desc: "傳統弓年度檢定達到入門",
  check: c => bowAtLeast(c.certRecords, "traditional", "入門")
}, {
  id: "trad_basic",
  cat: "level",
  icon: "🌿",
  ...
  rarity: "common",
  desc: "傳統弓年度檢定達到初級",
  check: c => bowAtLeast(c.certRecords, "traditional", "初級")
}, {
  id: "trad_mid",
  cat: "level",
  icon: "🌿",
  ...
  rarity: "uncommon",
  desc: "傳統弓年度檢定達到中級",
  check: c => bowAtLeast(c.certRecords, "traditional", "中級")
}, {
  id: "trad_adv",
  cat: "level",
  icon: "🌿",
  ...
  rarity: "rare",
  desc: "傳統弓年度檢定達到進階",
  check: c => bowAtLeast(c.certRecords, "traditional", "進階")
}, {
  id: "trad_elite",
  cat: "level",
  icon: "🌿",
  ...
  rarity: "epic",
  desc: "傳統弓年度檢定達到精英",
  check: c => bowAtLeast(c.certRecords, "traditional", "精英")
},
// ══ 檢定 — 跨弓成就 ══
{
  id: "multi_mid2",
  ...
  rarity: "rare",
  desc: "兩種弓以上達到中級",
  check: c => bowCountAtLeast(c.certRecords, "中級", 2)
}, {
  id: "multi_adv2",
  cat: "level",
  icon: "🔀",
  ...
  rarity: "epic",
  desc: "兩種弓以上達到進階",
  check: c => bowCountAtLeast(c.certRecords, "進階", 2)
}, {
  id: "multi_elite2",
  cat: "level",
  icon: "🔀",
  ...
  rarity: "legendary",
  desc: "兩種弓以上達到精英",
  check: c => bowCountAtLeast(c.certRecords, "精英", 2)
}, {
  id: "all_mid3",
  cat: "level",
  icon: "🎖️",
  ...
  rarity: "epic",
  desc: "三種弓以上達到中級",
  check: c => bowCountAtLeast(c.certRecords, "中級", 3)
}, {
  id: "all_adv3",
  cat: "level",
  icon: "🎖️",
  ...
  rarity: "legendary",
  desc: "三種弓以上達到進階",
  check: c => bowCountAtLeast(c.certRecords, "進階", 3)
}, {
  id: "all_elite3",
  cat: "level",
  icon: "🎖️",
...
// ══ 收藏 — 個別章 ══
{
  ...
  rarity: "common",
  desc: "獲得第一個肥貓銅章",
  check: c => (c.member?.fatCat?.bronze || 0) >= 1
}, {
  id: "fatcat_silver",
  cat: "collect",
  icon: "🐱",
  ...
  rarity: "uncommon",
  desc: "取得肥貓銀章一顆",
  check: c => (c.member?.fatCat?.silver || 0) >= 1
}, {
  id: "fatcat_gold",
  cat: "collect",
  icon: "👑",
  ...
  rarity: "epic",
  desc: "肥貓章累積達到金章",
  check: c => (c.member?.fatCat?.gold || 0) >= 1
}, {
  id: "score_bronze",
  cat: "collect",
  icon: "⭐",
  ...
  rarity: "common",
  desc: "獲得第一個積分銅章",
  check: c => (c.member?.score?.bronze || 0) >= 1
}, {
  id: "score_silver",
  cat: "collect",
  icon: "⭐",
  ...
  rarity: "uncommon",
  desc: "取得積分銀章一顆",
  check: c => (c.member?.score?.silver || 0) >= 1
}, {
  id: "score_gold",
  cat: "collect",
  icon: "🌠",
  ...
  rarity: "epic",
  desc: "積分章累積達到金章",
  check: c => (c.member?.score?.gold || 0) >= 1
}, {
  id: "ach_silver",
  cat: "collect",
  icon: "🏆",
  ...
  rarity: "common",
  desc: "獲得第一個成就銀章",
  check: c => (c.member?.achievement?.silver || 0) >= 1
}, {
  id: "ach_gold",
  cat: "collect",
  icon: "🏆",
  ...
  rarity: "uncommon",
  desc: "取得成就金章一顆",
  check: c => (c.member?.achievement?.gold || 0) >= 1
}, {
  id: "ach_black",
  cat: "collect",
  icon: "⬛",
  ...
  rarity: "legendary",
  desc: "成就章累積達到黑牌",
  check: c => (c.member?.achievement?.black || 0) >= 1
},
// ══ 收藏 — 組合章 ══
{
  id: "set_lowest",
  ...
  check: c => (c.member?.fatCat?.bronze || 0) >= 1 && (c.member?.score?.bronze || 0) >= 1 && (c.member?.achievement?.silver || 0) >= 1
}, {
  id: "set_mid",
  cat: "collect",
  icon: "💫",
  ...
  check: c => (c.member?.fatCat?.silver || 0) >= 1 && (c.member?.score?.silver || 0) >= 1 && (c.member?.achievement?.gold || 0) >= 1
}, {
  id: "set_top",
  cat: "collect",
  icon: "👑",
  ...
  check: c => (c.member?.fatCat?.gold || 0) >= 1 && (c.member?.score?.gold || 0) >= 1 && (c.member?.achievement?.black || 0) >= 1
},
// ══ 打怪模式 — 累積場數 ══
{
  id: "monster_first",
...
// 六族全圖鑑
{
  ...
  rarity: "epic",
  desc: "六大族各擊敗至少一隻",
  check: c => ["ghost", "mountain", "insect", "workplace", "exam", "temple"].every(fam => Object.entries(c.monsterDex || {}).some(([id, m]) => id.startsWith(fam + "_") && (m.wins || 0) > 0))
}, {
  id: "dex_all36",
  cat: "monster",
  icon: "📖",
...
// ══ 煉製 ══
{
  ...
  hidden: true,
  riddle: "工坊百味，缺一不可…",
  check: c => _itemData.POTIONS.every(p => (c.craftStats?.potionTypesCrafted?.[p.id] || 0) >= 1)
}, {
  id: "frag_forge_1",
  cat: "forge",
  icon: "✨",
...
// ══ 藥水使用 — 累積場數 ══
{
  ...
  name: "全種藥師",
  rarity: "epic",
  desc: "所有已開放消耗品各使用至少一次",
  hidden: true,
  riddle: "百味戰術，各試過一遍…",
  check: c => _itemData.POTIONS.filter(p => !p.futureFeature).every(p => (c.potionDex?.used?.[p.id] || 0) >= 1)
},
// ══ 怪物卡收藏 ══
{
  id: "card_1",
  ...
  name: "七族全收",
  rarity: "epic",
  desc: `${MONSTER_CARD_FAMILIES.length} 大族群各收集至少一張一般怪物卡`,
  hidden: true,
  riddle: "七種血脈，盡收囊中…",
...
// ══ 冒險者公會 ══
{
  ...
  rarity: "common",
  desc: "冒險者等級達到 Lv.10",
  check: c => (0, _adventurerSystem.levelFromXP)(c.member?.adventurerXP || 0) >= 10
}, {
  id: "guild_promo_bronze",
  cat: "guild",
  icon: "🥈",
  name: "白銀晉階",
  rarity: "uncommon",
  ...
  check: c => (0, _adventurerSystem.levelFromXP)(c.member?.adventurerXP || 0) >= 60
},
...
// 6族×(20普通+10稀有+5頭目+1超稀有)=216件+24首通紀念章=240件）。
{
  ...
  check: c => ["ghost", "mountain", "insect", "workplace", "exam", "temple"].every(fam => Object.keys(c.member?.dungeonCollectibles || {}).some(id => _dungeonCollectibles.COLLECTIBLE_MAP[id]?.family === fam))
}, {
  id: "collectible_master",
  cat: "dungeon",
  icon: "👑",
  name: "圖鑑大師",
  rarity: "mythic",
  hidden: true,
  riddle: `${Object.keys(_dungeonCollectibles.COLLECTIBLE_MAP).length} 件秘寶，一件不缺…`,
  desc: `收集全部地下城收藏品（${Object.keys(_dungeonCollectibles.COLLECTIBLE_MAP).length} 件）`,
  check: c => Object.keys(c.member?.dungeonCollectibles || {}).length >= Object.keys(_dungeonCollectibles.COLLECTIBLE_MAP).length
}];
...
for (const fam of ["ghost", "mountain", "insect", "workplace", "exam", "temple"]) {
  for (let t = 1; t <= 6; t++) {
    ...
    const monster = _monsterData.MONSTERS.find(m => m.id === monsterId);
...
for (const fam of ["ghost", "mountain", "insect", "workplace", "exam", "temple"]) {
  const pool = _dungeonCollectibles.FAMILY_COLLECTIBLES[fam];
  ...
  AUTO_ACHIEVEMENTS.push({
    id: `dungeon_${fam}_rare1`,
    cat: "dungeon",
    icon: FAM_ICONS[fam],
    name: `${FAM_LABELS[fam]}稀有獵人`,
    rarity: "rare",
    desc: `拾獲${FAM_LABELS[fam]}任一稀有收藏品`,
    check: c => rareIds.some(id => (c.member?.dungeonCollectibles?.[id] || 0) > 0)
  });
  AUTO_ACHIEVEMENTS.push({
    id: `dungeon_${fam}_rare_all`,
    cat: "dungeon",
    icon: FAM_ICONS[fam],
    name: `${FAM_LABELS[fam]}稀有大師`,
    rarity: "epic",
    hidden: true,
    riddle: "十件稀世珍寶，缺一不可…",
  ...
  AUTO_ACHIEVEMENTS.push({
    id: `dungeon_${fam}_boss1`,
    cat: "dungeon",
    icon: FAM_ICONS[fam],
    name: `${FAM_LABELS[fam]}王者遺物`,
    rarity: "legendary",
  ...
  if (superRareId) {
    AUTO_ACHIEVEMENTS.push({
      id: `dungeon_${fam}_superrare`,
      cat: "dungeon",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}至寶`,
      rarity: "mythic",
      hidden: true,
...
for (const item of Object.values(_dungeonCollectibles.COLLECTIBLE_MAP)) {
  if (item.rarity !== "exclusive") continue;
  AUTO_ACHIEVEMENTS.push({
    id: `dungeon_trophy_${item.dungeonId}`,
    cat: "dungeon",
    icon: item.icon,
    name: item.name,
    rarity: "epic",
...
for (const t of Object.values(_worldBossData.WB_TROPHY_MAP)) {
  AUTO_ACHIEVEMENTS.push({
    id: `wb_trophy_${t.id}`,
    cat: "worldboss_rank",
    icon: t.icon,
    name: t.name,
    rarity: t.kind === "lastHit" ? "mythic" : "legendary",
    desc: t.desc,
    hidden: true,
...
const TARGET_KILL_RARITIES = {
  1: "common",
  ...KILL_RARITIES
};
const TARGET_KILL_ICONS = {
  1: "⚔️",
  ...KILL_ICONS
};
...
for (const monster of _monsterData.MONSTERS) {
...
const CHEST_FAMILY_DEX = [["ghost", "👻", "鬼怪族"], ["mountain", "🏔️", "山林族"], ["insect", "🦂", "毒蟲族"], ["workplace", "💼", "職場族"], ["exam", "📝", "考試族"], ["temple", "🏰", "西方怪物族"], ["treasure", "📦", "寶箱族"]];
const CHEST_DEX_TYPES = exports.CHEST_DEX_TYPES = [{
  id: "wood",
  statKey: "wood",
  icon: "📦",
  name: "通用材料木箱",
  milestones: CHEST_10000_MILESTONES
}, {
  id: "iron",
  statKey: "iron",
  icon: "🧰",
  ...
  icon: "🎐",
  name: "貓貓箱",
  milestones: CHEST_100_MILESTONES
}, {
  id: "mimi_box",
  statKey: "mimi_box",
  icon: "😺",
  ...
  id: `family_mat_${family}`,
  statKey: `family_mat_${family}`,
  family,
  icon,
  name: `${label}素材箱`,
  milestones: CHEST_10000_MILESTONES
  ...
  id: "mini_boss_mat",
  statKey: "mini_boss_mat",
  icon: "🔶",
  name: "小王素材箱",
  milestones: CHEST_500_MILESTONES
}, {
  id: "boss_mat",
  statKey: "boss_mat",
  icon: "🔴",
...
const CHEST_OPEN_RARITIES = Object.fromEntries(CHEST_10000_MILESTONES.map(count => [count, chestMilestoneRarity(count, 10000)]));
...
for (const potion of _itemData.POTIONS.filter(item => !item.futureFeature)) {
...
// 但不再顯示、不提醒，也不進完成率分母。
const LEGACY_GUILD_AUTO_IDS = new Set(["guild_first_xp", "guild_lv10", "guild_promo_bronze", "guild_promo_silver", "guild_promo_gold", "guild_promo_plat", "guild_promo_legend", "guild_max"]);
const LEGACY_MONSTER_AUTO_IDS = new Set([...["ghost", "mountain", "insect", "workplace", "exam", "temple"].flatMap(fam => Array.from({
  length: 6
}, (_, index) => `dex_${fam}_t${index + 1}`)), ..._monsterData.MONSTERS.flatMap(monster => KILL_MILESTONES.map(count => `kill_${monster.id}_${count}`))]);
const RETIRED_AUTO_IDS = new Set([...LEGACY_GUILD_AUTO_IDS, ...LEGACY_MONSTER_AUTO_IDS, "drop_rare", "drop_epic", "drop_legendary", "drop_mythic", "dex_all6", "dex_all36", "mythic_all"]);
AUTO_ACHIEVEMENTS.forEach(achievement => {
  if (!RETIRED_AUTO_IDS.has(achievement.id)) return;
  achievement.retired = true;
  achievement.retiredReason = LEGACY_GUILD_AUTO_IDS.has(achievement.id) ? "舊冒險者等級系統已由新公會聲望／階級取代" : LEGACY_MONSTER_AUTO_IDS.has(achievement.id) ? "舊 36 怪個別討伐已由現行 252 怪目錄的普通怪族×T階／小王大王指定討伐取代" : achievement.id.startsWith("drop_") ? "目前沒有可靠的掉寶稀有度累計資料" : "舊 36 怪／六族終局條件已不符合現行擴充怪物目錄";
  if (achievement.id.startsWith("drop_")) achievement.futureData = true;
});
const SPECIAL_GRANTS = exports.SPECIAL_GRANTS = [{
  id: "beat_coach",
  cat: "special",
  icon: "⚔️",
  name: "擊敗主教練",
...
// ── 屆數成就（動態產生）────────────────────────────────────
function buildRoundAchievements(type, max, granted) {
  const list = [];
  for (let r = 1; r <= max; r++) {
...
// ── 期數成就 ───────────────────────────────────────────────
function buildCohortAchievement(joinDate) {
  const n = (0, _cohort.getCohort)(joinDate);
  return {
    id: `cohort_${n}`,
    cat: "cohort",
    icon: n === 1 ? "👑" : n === 0 ? "❓" : "🎓",
    name: (0, _cohort.cohortLabel)(n),
    rarity: (0, _cohort.cohortRarity)(n),
    title: (0, _cohort.cohortTitle)(n),
    desc: n === 0 ? "尚未設定加入日期，期數無法判定。請聯絡教練更新資料。" : `${(0, _cohort.cohortLabel)(n)}（${(0, _cohort.cohortTitle)(n)}）— 你在這個時期加入了貓小隊射箭場`,
    unlocked: true
  };
}
function buildCohortAchievements(joinDate, max = 20) {
  ...
  const current = (0, _cohort.getCohort)(joinDate);
  return Array.from({
    length: Math.max(0, Number(max) || 0) + 1
  }, (_, index) => {
    const n = index;
    return {
      id: `cohort_${n}`,
      cat: "cohort",
      icon: n === 0 ? "🌱" : n === 1 ? "👑" : "🎓",
      name: n === 0 ? "0期生" : (0, _cohort.cohortLabel)(n),
      rarity: (0, _cohort.cohortRarity)(n),
      title: n === 0 ? "創始期射手" : (0, _cohort.cohortTitle)(n),
      desc: n === 0 ? "2022/07/16 前加入貓小隊的 0 期生" : `${(0, _cohort.cohortLabel)(n)}（${(0, _cohort.cohortTitle)(n)}）`,
      unlocked: hasValidJoinDate && current === n
    };
  });
}
function buildArcherLevelAchievement() {
  const ranges = [{
    count: 100,
    name: "Lv.1～100",
    rarity: "rare",
  ...
  return {
    ...
    icon: "📈",
    name: "射手等級",
    desc: `射手等級 Lv.1～Lv.${_archerLevel.MAX_ARCHER_LEVEL}`,
    directDisplay: true,
    getValue: c => (0, _archerLevel.archerLevelFromXP)(c.member?.archerXP || 0),
...
function buildArcheryTenureAchievement(joinDate, now = new Date()) {
  return {
...
const ANNUAL_CERT_BOWS = Object.freeze([Object.freeze({
  id: "recurve_bare",
  label: "裸弓",
  icon: "🏹"
}), Object.freeze({
...
const ANNUAL_CERT_TIERS = Object.freeze([[1, "common", "入門"], [2, "uncommon", "初級"], [3, "rare", "中級"], [4, "epic", "進階"], [5, "legendary", "精英"]]);
...
function annualRecordValue(record, bowType) {
  ...
  return annualLevelValue((0, _constants.getCertLevel)(bowType, Number(record?.score) || 0));
}
...
function buildAnnualCertificationAchievements(certRecords = [], currentYear = new Date().getFullYear(), dexCompetitions = []) {
  ...
  if (catalog.length) {
    return catalog.flatMap(comp => {
      ...
      return ANNUAL_CERT_BOWS.map(bow => buildCertCard({
        year,
        half,
        bow,
        competition: comp
  ...
  return [...years].sort((a, b) => b - a).flatMap(year => ["first", "second"].flatMap(half => ANNUAL_CERT_BOWS.map(bow => buildCertCard({
    year,
    half,
    bow
  }))));
...
function buildExternalCompetitionAchievements(dexCompetitions = []) {
  return (dexCompetitions || []).filter(comp => comp?.dexCatalog === true && comp?.dexKind === "external" && comp?.id).map(comp => ({
...
const TIERED_ACHIEVEMENTS = exports.TIERED_ACHIEVEMENTS = [
// ══ 啟程 — 累積報到 ══
{
  id: "checkin",
  cat: "start",
...
// ══ 打怪 — 累積擊殺場數 ══
{
  ...
  tiers: [{
    ...
    count: MONSTER_DEX_CATALOG.length,
    rarity: "mythic",
    icon: "👑",
    name: "全圖鑑制霸",
    desc: `擊敗全部 ${MONSTER_DEX_CATALOG.length} 種正式怪物`
  }]
},
// ══ 打怪 — 頭目擊殺 ══
{
...
// ══ 怪物卡 — 卡片收集 ══
{
  ...
  tiers: [{
    ...
    icon: "🔰",
    name: "踏入公會",
    desc: "在新版冒險者公會獲得第一點聲望"
  }, ..._guildRank.GUILD_RANKS.filter(rank => rank.rep > 0).map((rank, index) => ({
    count: rank.rep,
  ...
  tiers: [{
    ...
    icon: "⚡",
    name: "險境獵人",
    desc: "累積贏得 5 次危險度 3 以上遠征"
  }, {
    count: 15,
    rarity: "legendary",
    icon: "🏆",
    name: "高危專家",
    desc: "累積贏得 15 次危險度 3 以上遠征"
  }]
}, {
  id: "guild_deadly_wins",
  cat: "guild",
  icon: "💀",
  name: "致命遠征",
  ...
  tiers: [{
    ...
    icon: "☠️",
    name: "絕境行者",
    desc: "累積贏得 5 次危險度 5 以上遠征"
  }, {
    count: 15,
    rarity: "mythic",
    icon: "🔥",
    name: "死境征服者",
    desc: "累積贏得 15 次危險度 5 以上遠征"
  }]
}, {
  id: "guild_mythic_wins",
  cat: "guild",
  icon: "👑",
  name: "神話遠征",
  ...
  tiers: [{
    ...
    icon: "🌋",
    name: "三破神話",
    desc: "累積贏得 3 次危險度 6 遠征"
  }, {
    count: 10,
    rarity: "mythic",
    icon: "🌠",
    name: "神話征服者",
    desc: "累積贏得 10 次危險度 6 遠征"
  }]
},
// ══ 決鬥 — 累積勝場 ══
{
...
for (const fam of MONSTER_DEX_FAMILIES) {
  for (let tierIndex = 1; tierIndex <= 6; tierIndex++) {
    const monsters = MONSTER_DEX_CATALOG.filter(monster => monster.family === fam && monster.tierIndex === tierIndex && monster.encounter === "normal");
...
for (const ct of CHEST_DEX_TYPES) {
  ...
  TIERED_ACHIEVEMENTS.push({
    ...
    tiers: milestones.map(n => ({
      count: n,
      rarity: chestMilestoneRarity(n, max),
      icon: ct.icon,
...
TIERED_ACHIEVEMENTS.push({
  ...
  tiers: LEGACY_CHEST_OPEN_MILESTONES.map(n => ({
    count: n,
    rarity: CHEST_OPEN_RARITIES[n],
    icon: "🐱",
    name: `舊版貓貓箱 ×${n}`,
...
for (const legacy of [{
  ...
  TIERED_ACHIEVEMENTS.push({
    ...
    tiers: CHEST_10000_MILESTONES.map(n => ({
      count: n,
      rarity: chestMilestoneRarity(n, 10000),
      icon: legacy.icon,
      name: `${legacy.name} ×${n}`,
      desc: `歷史累積開啟 ${n} 次`
    })),
    retired: true,
    retiredReason: legacy.id === "chest_family_mat" ? "族系素材箱已拆為七族各自統計" : "世界秘寶箱屬特殊道具，不列入現行寶箱圖鑑"
  });
}
...
TIERED_ACHIEVEMENTS.push({
  ...
  tiers: CHEST_CATALOG_MILESTONES.map((count, index) => ({
    count,
    rarity: CHEST_CATALOG_RARITIES[index],
    icon: "🗃️",
...
for (let danger = 1; danger <= 6; danger++) {
  TIERED_ACHIEVEMENTS.push({
    id: `guild_danger_${danger}_wins`,
    cat: "guild",
    icon: "⚔️",
    name: `公會 T${danger} 遠征勝場`,
    desc: `累積贏得危險度 ${danger}（T${danger}）公會遠征`,
    guildDanger: danger,
    getValue: c => Math.max(0, Number(c.guildExpeditionStats?.byDanger?.[danger]) || 0),
    tiers: [1, 5, 10, 25, 50, 100].map(count => ({
      count,
      rarity: longTermRarity(count),
      icon: "⚔️",
      name: `T${danger} 勝場 ×${count}`,
      desc: `累積贏得 T${danger} 公會遠征 ${count} 次`
    }))
  });
}
TIERED_ACHIEVEMENTS.push({
  id: "worldboss_participations",
  cat: "worldboss_participation",
  icon: "⚔️",
  name: "世界王參戰",
  desc: "累積參與世界王並完成參戰獎勵領取的次數",
  getValue: c => Number(c.member?.worldBossParticipations || 0),
  tiers: WORLD_BOSS_PARTICIPATION_MILESTONES.map(count => ({
    count,
    rarity: longTermRarity(count),
    icon: "⚔️",
    name: `參戰 ${count} 次`,
    desc: `累積參戰世界王 ${count} 次`
  }))
}, {
  id: "worldboss_kills",
  cat: "worldboss_kill",
  ...
  tiers: WORLD_BOSS_KILL_MILESTONES.map(count => ({
    count,
    rarity: longTermRarity(count),
    icon: "💀",
    name: `擊殺 ${count} 次`,
    desc: `累積擊倒世界王 ${count} 次`
  }))
});
for (const group of WORLD_BOSS_CARD_DEX_FAMILIES) {
  const familyName = group.family;
  TIERED_ACHIEVEMENTS.push({
    id: `worldboss_cards_${group.family}`,
    cat: "card",
    icon: "🃏",
    name: `世界王卡・${familyName}`,
    ...
    tiers: group.cardKeys.map((_, index) => ({
      count: index + 1,
      rarity: longTermRarity(index + 1),
      icon: "🃏",
      name: `${familyName} ${index + 1}/${group.cardKeys.length}`,
      desc: `收集 ${index + 1} 張不同的 ${familyName} 世界王卡`
    })),
    retired: true,
...
for (const group of WORLD_BOSS_CARD_DEX_GROUPS) {
  TIERED_ACHIEVEMENTS.push({
    id: `worldboss_card_group_${group.id}`,
    cat: "wbcard",
    icon: group.icon,
    name: `世界王卡・${group.label}`,
...
for (const group of MONSTER_CARD_DEX_GROUPS) {
  ...
  TIERED_ACHIEVEMENTS.push({
    id: `monster_cards_${group.id}`,
    cat: "card",
    icon: "🃏",
    name: `${label} T${group.tierIndex}`,
...
for (const building of VILLAGE_BUILDING_DEX) {
  TIERED_ACHIEVEMENTS.push({
    id: `village_building_${building.id}`,
    cat: "village",
    icon: building.icon,
    name: `${building.name}等級`,
    ...
    tiers: [1, 5, 10, 15, 20].map(count => ({
      count,
      rarity: longTermRarity(count),
      icon: building.icon,
      name: `${building.name} Lv.${count}`,
      desc: `${building.name}達到 Lv.${count}`
    }))
  });
}
TIERED_ACHIEVEMENTS.push({
  id: "village_cat_cards",
  cat: "village",
  icon: "🎴",
  name: "貓貓卡片收藏",
  desc: `收集貓貓卡片（目前共 ${_catCardData.CAT_CARDS.length} 張）`,
  getValue: c => Object.keys(c.member?.catCards || {}).filter(id => Boolean(c.member?.catCards?.[id])).length,
  tiers: [1, 10, 25, 50, 100, 150, _catCardData.CAT_CARDS.length].filter((count, index, all) => count <= _catCardData.CAT_CARDS.length && all.indexOf(count) === index).map(count => ({
    count,
    rarity: count === _catCardData.CAT_CARDS.length ? "legendary" : longTermRarity(count),
    icon: "🎴",
    name: count === _catCardData.CAT_CARDS.length ? `全收集 ${_catCardData.CAT_CARDS.length} 張` : `收集 ${count} 張`,
    desc: count === _catCardData.CAT_CARDS.length ? `收集全部 ${_catCardData.CAT_CARDS.length} 張貓貓卡片` : `累積收集 ${count} 張不同貓貓卡片`
  }))
});
for (const mapId of _boardJourney.JOURNEY_MAP_IDS) {
  const meta = _boardJourney.JOURNEY_MAP_META[mapId] || {};
  const label = meta.name || mapId;
  const icon = meta.icon || "🗺️";
  TIERED_ACHIEVEMENTS.push({
    id: `village_journey_${mapId}`,
    cat: "village",
    icon,
    name: `探索地圖・${label}`,
    desc: `完成${label}的旅程次數`,
    villageJourneyMapId: mapId,
    ...
    tiers: [1, 5, 10, 25, 50, 100].map(count => ({
      ...
      icon,
      name: `${label} ×${count}`,
      desc: `累積完成${label} ${count} 次`
    }))
  });
}
const shopCustomerMilestones = [...new Set([1, 3, 6, 12, 18, _villageShop.SHOP_CUSTOMERS.length].filter(count => count <= _villageShop.SHOP_CUSTOMERS.length))].sort((a, b) => a - b);
TIERED_ACHIEVEMENTS.push({
  id: "shop_level",
  cat: "shop",
  icon: "🏪",
  name: "商店等級",
  desc: "依累積營業額提升商店等級",
  getValue: c => (0, _villageShop.getShopLevel)(shopStatsFromContext(c).totalRevenue),
  tiers: [1, 5, 10, 15, 20, 25, _villageShop.MAX_SHOP_LEVEL].map(count => ({
    count,
    rarity: longTermRarity(count),
    icon: "🏪",
    name: `商店 Lv.${count}`,
  ...
  tiers: [1, 10, 50, 100, 250, 500, 1000, 2500, 5000].map(count => ({
    count,
    rarity: longTermRarity(count),
    icon: "🧾",
    name: `成交 ${count} 筆`,
  ...
  id: "shop_customers_served",
  cat: "shop",
  icon: "🐾",
  name: "服務顧客",
  desc: "累積服務顧客人次",
  getValue: c => Number(shopStatsFromContext(c).customersServed || 0),
  tiers: [1, 10, 50, 100, 250, 500, 1000].map(count => ({
    count,
    rarity: longTermRarity(count),
    icon: "🐾",
    name: `服務 ${count} 人次`,
    desc: `累積服務顧客 ${count} 人次`
  }))
}, {
  id: "shop_customer_catalog",
  cat: "shop",
  icon: "📖",
  name: "顧客圖鑑",
  desc: "發現不同的商店顧客",
  getValue: discoveredShopCustomerCount,
  tiers: shopCustomerMilestones.map(count => ({
    count,
    rarity: longTermRarity(count),
    icon: "📖",
    name: `發現 ${count} 位顧客`,
    desc: `累積發現 ${count} 位不同顧客`
  }))
});
for (const potion of _itemData.POTIONS.filter(item => !item.futureFeature)) {
  const milestones = POTION_RARITY_MILESTONES[potion.rarity] || POTION_RARITY_MILESTONES.common;
...
for (const fam of MONSTER_DEX_FAMILIES) {
  TIERED_ACHIEVEMENTS.push({
    ...
    tiers: [{
      ...
      rarity: "common",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}初遇`,
      desc: `擊敗第 1 種${FAM_LABELS[fam]}怪物`
    }, {
      count: 6,
      rarity: "uncommon",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}踏查`,
      desc: `擊敗 6 種${FAM_LABELS[fam]}怪物`
    }, {
      count: 12,
      rarity: "rare",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}獵手`,
      desc: `擊敗 12 種${FAM_LABELS[fam]}怪物`
    }, {
      count: 18,
      rarity: "rare",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}半制霸`,
      desc: `擊敗 18 種${FAM_LABELS[fam]}怪物`
    }, {
      count: 24,
      rarity: "epic",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}專家`,
      desc: `擊敗 24 種${FAM_LABELS[fam]}怪物`
    }, {
      count: 30,
      rarity: "legendary",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}征服者`,
      desc: `擊敗 30 種${FAM_LABELS[fam]}怪物`
    }, {
      count: 36,
      rarity: "mythic",
      icon: FAM_ICONS[fam],
      name: `${FAM_LABELS[fam]}全圖鑑`,
      desc: `擊敗全部 36 種${FAM_LABELS[fam]}怪物`
    }]
  });
}
...
// 🐈 貓咪（ctx.cats：cats 子集合陣列）
{
  ...
  getValue: c => (c.cats || []).reduce((m, x) => Math.max(m, (0, _adventurerSystem.levelFromXP)(x.catXP || 0)), 0),
  tiers: [{
    count: 10,
    rarity: "common",
    icon: "⭐",
...
// 🛡️ 裝備（member.rpgEquip）
{
  ...
  // +1 讓「無裝備」為 0、common 為 1…mythic 為 6
  tiers: [{
    count: 2,
    rarity: "uncommon",
    icon: "🔷",
...
const CAT_LEVEL_ACHIEVEMENT_TIERS = [[10, "common", "⭐"], [50, "uncommon", "🌟"], [100, "rare", "💫"], [200, "epic", "🔥"], [300, "legendary", "⚡"], [500, "mythic", "👑"]];
const CAT_BOND_ACHIEVEMENT_TIERS = [[5, "common", "💛"], [10, "uncommon", "💗"], [20, "rare", "💖"], [30, "epic", "💞"], [40, "legendary", "💝"], [50, "mythic", "🫶"]];
const CAT_EQUIP_ACHIEVEMENT_TIERS = [[5, "common", "🔧"], [10, "uncommon", "🛠️"], [20, "rare", "⚙️"], [30, "epic", "✨"], [40, "legendary", "💎"], [50, "mythic", "👑"]];
for (const [catId, cat] of Object.entries(_catData.CATS)) {
  const catName = cat?.name || catId;
  TIERED_ACHIEVEMENTS.push({
    id: `cat_${catId}_level`,
    cat: "cat",
    icon: "⭐",
    name: `${catName}・等級`,
    desc: `${catName}的成長等級`,
    catAchievementKind: "level",
    catId,
    ...
    id: `cat_${catId}_equipment`,
    cat: "cat",
    icon: "⚙️",
    name: `${catName}・裝備`,
    desc: `${catName}七件裝備的平均強化等級`,
    catAchievementKind: "equipment",
    catId,
...
AUTO_ACHIEVEMENTS.push({
  ...
  hidden: true,
  riddle: "九棟建築，全數登頂…",
  desc: "9 棟建築全部升到滿級 Lv.20",
  check: c => {
    const b = c.member?.village?.buildings || {};
    return _villageData.BUILDING_LIST.every(buildingId => canonicalVillageBuildingLevel(b, buildingId) >= 20);
  }
}, {
  id: "equip_full_mythic",
  cat: "equip",
  icon: "👑",
  name: "神裝完全體",
  rarity: "mythic",
...
function computeTierProgress(tieredAch, ctx) {
  ...
  const progress = range > 0 ? Math.min(1, Math.max(0, (value - prevThreshold) / range)) : 1;
...
const REPLACED_BY_TIERED = exports.REPLACED_BY_TIERED = new Set();
TIERED_ACHIEVEMENTS.filter(isActiveAchievement).forEach(t => (t.replacesIds || []).forEach(id => REPLACED_BY_TIERED.add(id)));
...
function getUnlockedKeys(ctx) {
  const keys = [];
  AUTO_ACHIEVEMENTS.forEach(a => {
    if (!isActiveAchievement(a)) return;
    if (REPLACED_BY_TIERED.has(a.id)) return;
    try {
      if (a.check(ctx)) keys.push(a.id);
    } catch {/* 資料未就緒時忽略 */}
  });
  const dynamicTiered = [buildArcherLevelAchievement(), buildArcheryTenureAchievement(ctx.member?.joinDate), ...buildAnnualCertificationAchievements(ctx.certRecords, new Date().getFullYear(), ctx.dexCompetitions), ...buildExternalCompetitionAchievements(ctx.dexCompetitions)];
...
function computeDexStats({
  ...
  const ctx = {
    ...
    duelStats: duelStats || {},
    cats: cats || [],
    guildRep: Math.max(0, Number(guildRep) || 0),
    guildExpeditionStats: guildExpeditionStats || {},
    externalComps: externalComps || [],
    dexCompetitions: dexCompetitions || []
  };
  let autoUnlocked = 0,
    autoTotal = 0;
  AUTO_ACHIEVEMENTS.forEach(a => {
  ...
  let tieredUnlocked = 0,
    tieredTotal = 0;
  const dynamicTiered = [buildArcherLevelAchievement(), buildArcheryTenureAchievement(member?.joinDate), ...buildAnnualCertificationAchievements(certRecords, new Date().getFullYear(), dexCompetitions), ...buildExternalCompetitionAchievements(dexCompetitions)];
  [...TIERED_ACHIEVEMENTS, ...dynamicTiered].forEach(t => {
    if (!isActiveAchievement(t)) return;
    const prog = computeTierProgress(t, ctx);
    if (!prog) return;
  ...
  SPECIAL_GRANTS.forEach(a => {
    if (grantedIds.has(a.id)) specialUnlocked++;
  });
  ...
  (granted || []).forEach(g => {
    if (g.type === "physical" || g.type === "point") {
      if (g.rank === 1) gold++;else if (g.rank === 2) silver++;else if (g.rank === 3) bronze++;
    }
  });
  return {
    totalUnlocked,
