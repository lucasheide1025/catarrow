...
exports.GATHERING_TIER_META = exports.GATHERING_TIERS = exports.GATHERING_SITE_MAP = exports.GATHERING_SITES = exports.GATHERING_ROUNDS = exports.GATHERING_ARROWS_PER_ROUND = void 0;
exports.buildGatheringRunContract = buildGatheringRunContract;
exports.calculateGatheringRewards = calculateGatheringRewards;
exports.calculateGatheringRound = calculateGatheringRound;
exports.getGatheringCompletion = getGatheringCompletion;
exports.getGatheringMaterialPool = getGatheringMaterialPool;
exports.getGatheringPartyBonus = getGatheringPartyBonus;
exports.getGatheringTierNumber = getGatheringTierNumber;
exports.getUnlockedGatheringTiers = getUnlockedGatheringTiers;
exports.rollGatheringMaterials = rollGatheringMaterials;
exports.scoreToGatheringProgress = scoreToGatheringProgress;
var _villageData = require("./villageData");
var _monsterEconomyCatalog = require("./monsterEconomyCatalog");
const GATHERING_ROUNDS = exports.GATHERING_ROUNDS = 3;
const GATHERING_ARROWS_PER_ROUND = exports.GATHERING_ARROWS_PER_ROUND = 6;
const GATHERING_TIERS = exports.GATHERING_TIERS = ["common", "rare", "elite", "fierce", "boss", "mythic"];
const GATHERING_TIER_META = exports.GATHERING_TIER_META = {
  common: {
    no: 1,
    label: "T1 基礎",
    color: "#64748b",
...
const GATHERING_SITES = exports.GATHERING_SITES = [{
  id: "mine",
  name: "星屑礦坑",
  buildingName: "礦坑",
  icon: "⛏️",
...
const GATHERING_SITE_MAP = exports.GATHERING_SITE_MAP = Object.fromEntries(GATHERING_SITES.map(site => [site.id, site]));
...
function getGatheringMaterialPool(race, tierNo) {
  if (!race) return [];
  const maxTier = Math.max(1, Math.min(6, Math.round(Number(tierNo) || 1)));
  return (0, _monsterEconomyCatalog.getNormalMaterialPool)({
    family: race,
    maxTier
  }).map(item => ({
    materialId: item.id,
...
function rollGatheringMaterials({
  race,
  tierNo,
  totalCount,
  random = Math.random
  ...
  return pool.filter(item => counts.has(item.materialId)).map(item => ({
...
function getGatheringTierNumber(tier) {
  return GATHERING_TIER_META[tier]?.no || 1;
}
function getUnlockedGatheringTiers(buildingLevel = 1) {
  const stage = Math.max(1, Math.min(5, (0, _villageData.getBuildingStage)(buildingLevel || 1)));
  return GATHERING_TIERS.slice(0, stage);
}
function scoreToGatheringProgress(label) {
  return SCORE_PROGRESS[String(label)] ?? 0;
}
function getGatheringCompletion(progressPct) {
  if (progressPct >= 180) return {
    key: "great",
    label: "大豐收",
    multiplier: 1.5,
    rareRolls: 2
  };
  if (progressPct >= 130) return {
    key: "harvest",
    label: "豐收",
    multiplier: 1.25,
    rareRolls: 1
...
function buildGatheringRunContract({
  buildingId,
  tier,
  buildingLevel = 1,
  seed = Date.now()
...
function calculateGatheringRound(arrows) {
...
function makeRareRewards({
  ...
  rewards.push({
    ...
    resourceKey: (0, _villageData.getResourceKey)("fur", Math.min(5, Math.max(1, tierNo))),
    name: "陪練貓毛",
  ...
  if (tierNo >= 3 || xCount >= 4) {
    rewards.push({
      type: "villageResource",
      resourceKey: (0, _villageData.getResourceKey)("potion", Math.min(5, Math.max(1, tierNo - 1))),
      name: "貓薄荷藥水",
      count: 1
    });
  }
  if (completion.key === "great" && tierNo >= 4) {
    ...
    const bonusPool = getGatheringMaterialPool(site.race, bonusTier).filter(item => item.tierIndex === bonusTier);
...
function getGatheringPartyBonus(partySize = 1) {
...
function calculateGatheringRewards({
  contract,
  rounds,
  partySize = 1
}) {
  ...
  const materials = rollGatheringMaterials({
    race: site.race,
    tierNo,
    totalCount: materialCount
  });
  const primary = materials.length ? materials.reduce((best, item) => item.tierIndex > best.tierIndex ? item : best, materials[0]) : null;
  const rewards = {
    progressPct,
    totalScore,
    xCount,
    ...
    materialCount,
    villageResources: {
      [(0, _villageData.getResourceKey)(site.resource, Math.min(5, tierNo))]: villageCount
    },
