...
exports.TARGET_ZONES = exports.TARGET_NAME = exports.STANDARD_ZONES = exports.RANKS = exports.PROMOTION_QUESTS = exports.DAILY_BOUNTY_TIER_MAP = exports.DAILY_BOUNTY_KILL_RANGE = void 0;
exports.checkTaskPass = checkTaskPass;
exports.generateBiWeeklyBounties = generateBiWeeklyBounties;
exports.generateDailyGeneralBounties = generateDailyGeneralBounties;
exports.getBiWeeklyPeriodKey = getBiWeeklyPeriodKey;
exports.getDailyGuildTasks = getDailyGuildTasks;
exports.isPromotionLevel = isPromotionLevel;
exports.levelFromXP = levelFromXP;
exports.levelInRank = levelInRank;
exports.makeSeedRand = makeSeedRand;
exports.rankFromLevel = rankFromLevel;
exports.rankIdxFromLevel = rankIdxFromLevel;
exports.taskDesc = taskDesc;
exports.xpProgress = xpProgress;
exports.xpToReachLevel = xpToReachLevel;
// src/lib/adventurerSystem.js — 冒險者公會等級系統

// ── 六大階級 ──────────────────────────────────────────────────
const RANKS = exports.RANKS = [{
  name: "青銅",
  icon: "🥉",
  color: "#b45309",
  mult: 1.0,
...
// 由等級取得階級索引（0-5）
function rankIdxFromLevel(level) {
...
function isPromotionLevel(level) {
  return [10, 20, 30, 40, 50].includes(level);
}

// ── 靶紙分數按鈕設定 ──────────────────────────────────────────
const TARGET_ZONES = exports.TARGET_ZONES = {
  cthulhu: [{
    label: "12",
    val: 12,
    color: "#fbbf24"
...
const TARGET_NAME = exports.TARGET_NAME = {
  cthulhu: "克蘇魯靶",
  hostage: "人質靶",
  zombie: "殭屍靶"
};
...
// ── 原野射箭標準分數按鈕 ────────────────────────────────────────
const STANDARD_ZONES = exports.STANDARD_ZONES = [{
  label: "X",
  val: 10,
  color: "#fbbf24"
}, {
...
const PROMOTION_QUESTS = exports.PROMOTION_QUESTS = {
  10: {
    level: 10,
    fromRank: "青銅",
    toRank: "白銀",
...
// ── 任務驗收 ──────────────────────────────────────────────────
function checkTaskPass(task, arrows) {
  const total = arrows.reduce((s, a) => s + a, 0);
  switch (task.type) {
    case "score":
      return total >= task.goal;
    case "hits":
      return arrows.filter(a => a > 0).length >= task.goal;
    case "protected_score":
      return total >= task.goal && !arrows.includes(-50);
    case "headshot":
      return arrows.filter(a => a >= 10).length >= task.goal;
    case "one_shot":
      return arrows[0] === 20;
    default:
      return total >= task.goal;
...
function taskDesc(task) {
  switch (task.type) {
    case "score":
      return `總分達 ${task.goal} 分`;
    case "hits":
      return `命中 ${task.goal} 箭以上`;
    case "protected_score":
      return `保護人質，得分達 ${task.goal} 分`;
    case "headshot":
      return `爆頭（10/11分區）${task.goal} 箭以上`;
    case "one_shot":
      return `一箭討伐（命中 20 分區）`;
    default:
      return `達到目標 ${task.goal}`;
  }
}
...
function rollBonus(difficulty, rand) {
  ...
  if (difficulty === 1) {
    ...
    if (r < 0.30) return {
      type: "coins",
      amount: 150 + Math.floor(rand() * 150),
      icon: "💰",
      label: "金幣寶箱"
    };
    ...
    if (r < 0.30) return {
      type: "coins",
      amount: 300 + Math.floor(rand() * 200),
      icon: "💰",
      label: "金幣寶箱"
    };
  } else {
    ...
    if (r < 0.35) return {
      type: "coins",
      amount: 500 + Math.floor(rand() * 300),
      icon: "💰",
      label: "金幣寶箱"
    };
  }
...
function getDailyGuildTasks(date) {
  ...
  // 難度二（挑戰）
  {
    ...
    coins: 150,
    arrowDew: ri(20, 40),
    questName: qn("hostage", "protected_score"),
    bonus: bon(2)
  }, {
    id: 5,
    target: "zombie",
...
// 懸賞難度 → 可選怪物 tier 對應表
const DAILY_BOUNTY_TIER_MAP = exports.DAILY_BOUNTY_TIER_MAP = {
  1: ["common"],
  2: ["common", "rare"],
  3: ["rare", "elite"],
...
// 懸賞難度 → 擊殺數範圍
const DAILY_BOUNTY_KILL_RANGE = exports.DAILY_BOUNTY_KILL_RANGE = {
  1: {
    min: 5,
    max: 10
  },
...
const BOUNTY_NAME_POOL = {
  ghost: ["靈界異變討伐令", "鬼怪掃蕩令", "陰間通道封鎖任務", "幽鏡巡查令"],
 ...
 */
function generateDailyGeneralBounties(dateKey, rewards, rand, monsters) {
  const ri = (min, max) => Math.floor(rand() * (max - min + 1)) + min;
  const pick = arr => arr[Math.floor(rand() * arr.length)];
  const results = [];
  [1, 2, 3, 4].forEach(difficulty => {
    const allowedTiers = DAILY_BOUNTY_TIER_MAP[difficulty] || ["common"];
    const killRange = DAILY_BOUNTY_KILL_RANGE[difficulty] || {
      min: 1,
      max: 5
    };

    // 從 MONSTERS 中篩選符合 tier 的怪物（排除 treasure 族，因為寶箱怪不是常規狩獵目標）
    const pool = monsters.filter(m => allowedTiers.includes(m.tier) && m.family !== "treasure");
    ...
    const namePool = familyNames && familyNames.length > 0 ? familyNames : BOUNTY_GENERIC_NAMES[difficulty] || ["討伐令"];
    const title = pick(namePool);

    // 生成任務描述
    const familyLabel = monster.family === "ghost" ? "鬼怪" : monster.family === "mountain" ? "野獸" : monster.family === "insect" ? "蟲族" : monster.family === "workplace" ? "職場" : monster.family === "exam" ? "考試" : monster.family === "temple" ? "西方魔物" : monster.family === "treasure" ? "寶箱" : "怪物";
    ...
    results.push({
      ...
      reward: {
        xp: r.xp,
        coins: r.coins,
        arrowDew: r.arrowDew || 0,
        gachaCoins: r.gachaCoins || 0
      },
...
function generateBiWeeklyBounties(periodKey, monsters) {
    ...
    for (let i = 0; i < cfg.count; i++) {
      ...
      quests.push({
        ...
        reward: {
          xp: cfg.xp,
          coins: cfg.coins,
          arrowDew: cfg.arrowDew,
          gachaCoins: cfg.gachaCoins
        },
        periodTag: periodKey,
        badgeReward: null,
        deadline: null,
