...
exports.WB_PASSIVE_PCT_PER_CARD = exports.TIER_CARD_BONUS = exports.STAR_UPGRADE_COST = exports.SLAYER_PCT_PER_STAR = exports.SLAYER_CYCLE = exports.SLAYER_BASE_PCT = exports.MAX_WB_EQUIPPED = exports.MAX_MONSTER_EQUIPPED = exports.MAX_EQUIPPED_PER_STAT = exports.MAX_EQUIPPED_BY_STAT = exports.MAX_DAMAGE_REDUCE_PCT = exports.FAMILY_STAT = exports.CAT_GROUP_STAT = void 0;
exports.calcCardBonus = calcCardBonus;
exports.calcEquippedBonus = calcEquippedBonus;
exports.canUpgradeStar = canUpgradeStar;
exports.familyPreyedBy = familyPreyedBy;
exports.familyPreysOn = familyPreysOn;
exports.getCardSlayerEffect = getCardSlayerEffect;
exports.getCardStat = getCardStat;
exports.getStatLabel = getStatLabel;
exports.getUpgradeCost = getUpgradeCost;
exports.maxEquippedForStat = maxEquippedForStat;
exports.resolveBattleBonus = resolveBattleBonus;
exports.resolveEquippedCards = resolveEquippedCards;
exports.resolveFamilyModifiers = resolveFamilyModifiers;
// src/lib/monsterCards.js
// 怪物卡片系統：收集、升星、裝備加成

// 族 → 加成屬性
const FAMILY_STAT = exports.FAMILY_STAT = {
  ghost: "atk",
  mountain: "def",
  insect: "hp",
  workplace: "atk",
...
// 貓貓分組 → 加成屬性（世界王貓貓卡用）
const CAT_GROUP_STAT = exports.CAT_GROUP_STAT = {
  atk: "atk",
  def: "def",
  heal: "hp"
};
...
// 階級 → 1★基礎值 & 每升一星追加值
const TIER_CARD_BONUS = exports.TIER_CARD_BONUS = {
  common: {
    base: 1,
    perStar: 1,
    label: "普通",
...
const STAR_UPGRADE_COST = exports.STAR_UPGRADE_COST = [1, 2, 3, 4, 5];

// 2026-07-19 使用者拍板：改回依屬性分槽，HP 5 張、ATK/DEF 各 3 張，四個區塊各自獨立。
const MAX_EQUIPPED_BY_STAT = exports.MAX_EQUIPPED_BY_STAT = Object.freeze({
  hp: 5,
  atk: 3,
  def: 3
});
function maxEquippedForStat(stat) {
  return MAX_EQUIPPED_BY_STAT[stat] ?? 3;
}
// 舊常數保留避免外部引用炸掉；總量＝各槽相加（5+3+3）
const MAX_EQUIPPED_PER_STAT = exports.MAX_EQUIPPED_PER_STAT = 3;
const MAX_MONSTER_EQUIPPED = exports.MAX_MONSTER_EQUIPPED = MAX_EQUIPPED_BY_STAT.hp + MAX_EQUIPPED_BY_STAT.atk + MAX_EQUIPPED_BY_STAT.def;
// 世界王卡：獨立的裝備欄位，最多 3 張（不分屬性，跟怪物卡的 9 格完全分開算）
const MAX_WB_EQUIPPED = exports.MAX_WB_EQUIPPED = 3;

// 世界王卡被動效果：同分類每張額外加成，封頂 3 張
const WB_PASSIVE_PCT_PER_CARD = exports.WB_PASSIVE_PCT_PER_CARD = 0.03;

// 計算某星級的加成數值（worldboss 固定值，忽略星級）
function calcCardBonus(tier, stars) {
...
function resolveEquippedCards(collection = {}) {
  const {
    cards = {},
    wbCards = {},
    equipped = []
...
function calcEquippedBonus(equippedCards = []) {
  const bonus = {
    hp: 0,
    atk: 0,
    def: 0,
...
const SLAYER_BASE_PCT = exports.SLAYER_BASE_PCT = Object.freeze({
  common: 3,
  rare: 5,
  elite: 7,
  fierce: 9,
...
const SLAYER_PCT_PER_STAR = exports.SLAYER_PCT_PER_STAR = 2;
...
const SLAYER_CYCLE = exports.SLAYER_CYCLE = Object.freeze(["temple", "ghost", "exam", "workplace", "treasure", "insect", "mountain"]);
...
// 這一族剋誰
function familyPreysOn(family) {
  return cycleNeighbor(family, 1);
}
// 誰剋這一族（天敵）
function familyPreyedBy(family) {
  return cycleNeighbor(family, -1);
}
function getCardSlayerEffect(card) {
  if (!card) return null;
  ...
  if (explicit?.targetFamily) {
    return {
      targetFamily: explicit.targetFamily,
      mode: explicit.mode === "reduce" ? "reduce" : "bonus",
      pct
    };
  }
...
const MAX_DAMAGE_REDUCE_PCT = exports.MAX_DAMAGE_REDUCE_PCT = 0.8; // 減傷封頂，避免堆到完全免疫

function resolveBattleBonus(bonus, monsterFamily) {
  const family = resolveFamilyModifiers(bonus, monsterFamily);
  return {
    dmgBonusPct: (bonus?.dmgBonusPct || 0) + family.damageBonusPct / 100,
    dmgReducePct: Math.min(MAX_DAMAGE_REDUCE_PCT, (bonus?.dmgReducePct || 0) + family.damageReducePct / 100),
    healBonusPct: bonus?.healBonusPct || 0
  };
}

// 依「本場怪物族系」把剋制加成攤平成單一數字，供戰鬥端直接使用
function resolveFamilyModifiers(bonus, monsterFamily) {
  if (!monsterFamily) return {
    damageBonusPct: 0,
    damageReducePct: 0
  };
  return {
...
// 判斷是否可以升星（世界王卡不可升星）
function canUpgradeStar(stars, duplicates, tier) {
