...
exports.SHOP_RUSH_SECONDS_PER_TEN_ARROWS = exports.SHOP_RUSH_SECONDS_CAP = exports.SHOP_LEVEL_THRESHOLDS = exports.SHOP_EXCHANGE_REWARDS = exports.SHOP_CUSTOMERS = exports.MAX_SHOP_LEVEL = exports.FURNITURE_DEFS = void 0;
exports.calcShopCap = calcShopCap;
exports.calcShopRate = calcShopRate;
exports.calcShopSlots = calcShopSlots;
exports.calcWaitingVisitors = calcWaitingVisitors;
exports.claimShopRushTime = claimShopRushTime;
exports.defaultShopState = defaultShopState;
exports.getExchangeRemaining = getExchangeRemaining;
exports.getExchangeRewardById = getExchangeRewardById;
exports.getExchangeUsed = getExchangeUsed;
exports.getFurniturePrice = getFurniturePrice;
exports.getFurnitureTotalPrice = getFurnitureTotalPrice;
exports.getLevelProgress = getLevelProgress;
exports.getLevelReward = getLevelReward;
exports.getShopCapBonus = getShopCapBonus;
exports.getShopLastVisitedMs = getShopLastVisitedMs;
exports.getShopLevel = getShopLevel;
exports.getShopSpeedBonus = getShopSpeedBonus;
exports.getUnlockedCustomers = getUnlockedCustomers;
exports.normalizeShop = normalizeShop;
exports.pickCustomer = pickCustomer;
exports.planQuickShopDisplay = planQuickShopDisplay;
exports.planShopExchange = planShopExchange;
exports.simulateServe = simulateServe;
exports.todayStr = todayStr;
exports.weekStr = weekStr;
var _shopGoodsCatalog = require("./shopGoodsCatalog");
var _shopData = require("./shopData");
...
// ────────────────────────────────────────────────────────────
const SHOP_LEVEL_THRESHOLDS = exports.SHOP_LEVEL_THRESHOLDS = [0,
// Lv1 開局
300,
// Lv2
700,
...
const MAX_SHOP_LEVEL = exports.MAX_SHOP_LEVEL = SHOP_LEVEL_THRESHOLDS.length; // 30
...
// 依累計營業額回傳商店等級（1~30）
function getShopLevel(totalRevenue) {
  const rev = Number(totalRevenue) || 0;
  let level = 1;
  for (let i = 0; i < SHOP_LEVEL_THRESHOLDS.length; i++) {
    if (rev >= SHOP_LEVEL_THRESHOLDS[i]) level = i + 1;else break;
  }
  return level;
}

// 累計至 level 的客速加成（%）
function getShopSpeedBonus(level) {
  let sum = 0;
...
// 累計至 level 的顧客上限加成
function getShopCapBonus(level) {
  let sum = 0;
...
// 下一級的門檻與進度（供進度條）
function getLevelProgress(totalRevenue) {
...
// ────────────────────────────────────────────────────────────
const FURNITURE_DEFS = exports.FURNITURE_DEFS = {
  cabinet: {
    id: "cabinet",
    name: "櫃子",
    icon: "🗄️",
  ...
  counter: {
    ...
    base: 80,
    maxLevel: 10,
    effect: "每級 +1 格位，料理吸引力 +15%",
    liveEffect: "即時營業：妹妹的收銀結帳會逐級加快。",
    desc: "寬敞的料理檯，擺料理特別吸引人。"
  },
  flower: {
    id: "flower",
    name: "花飾",
    ...
    maxLevel: 10,
    effect: "每級 +8% 客速",
    liveEffect: "即時營業：提升招客節奏，也讓排隊顧客更有耐心。",
    desc: "芬芳花飾吸引貓貓上門。"
  },
  flag: {
    id: "flag",
    ...
    maxLevel: 10,
    effect: "每級 +8 顧客上限",
    liveEffect: "即時營業：Lv.3 起店內可同時容納 3 位顧客。",
    desc: "飄揚的旗幟讓更多貓貓駐足。"
  },
  sign: {
    id: "sign",
    ...
    maxLevel: 10,
    effect: "每級 +10% 客速",
    liveEffect: "即時營業：讓下一位顧客更快推門進店。",
    desc: "醒目的招牌讓遠方的貓貓也聞名而來。"
  },
  luckyCat: {
    id: "luckyCat",
    name: "招財貓",
  ...
  starLamp: {
    ...
    maxLevel: 1,
    effect: "+10% 顧客上限",
    liveEffect: "即時營業：直接開放 3 人同場，並大幅提升排隊耐性。",
    desc: "星塵光芒映照，店內人氣高漲。"
  }
};
function getFurniturePrice(fid, currentLevel) {
...
function getFurnitureTotalPrice(fid, targetLevel) {
  const def = FURNITURE_DEFS[fid];
  if (!def) return 0;
  let sum = 0;
...
function calcShopSlots(furniture) {
  const f = furniture || {};
...
function planQuickShopDisplay(shop, goodId, category) {
  const capacity = calcShopSlots(shop?.furniture);
  const display = (Array.isArray(shop?.display) ? shop.display : []).slice(0, capacity).map(d => ({
    slot: d?.slot || "counter",
    goodId: d?.goodId || null
  }));
  while (display.length < capacity) display.push({
    slot: "counter",
    goodId: null
  });
...
function calcShopRate(furniture, level) {
  const f = furniture || {};
...
function calcWaitingVisitors(shop, now = Date.now()) {
...
// ────────────────────────────────────────────────────────────
const SHOP_CUSTOMERS = exports.SHOP_CUSTOMERS = [
// ── 常見（開局，weight 30）───────────────────────────────
{
  id: "小貓仔",
  name: "小貓仔",
...
// 加權抽一位顧客（rng 注入以便測試）
function pickCustomer(level, rng = Math.random) {
...
function pickGoodForCustomer(customer, displayed, rng) {
  ...
  const tierFiltered = displayed.filter(d => (!customer.minTier || d.good.tier >= customer.minTier) && (!customer.maxTier || d.good.tier <= customer.maxTier));
  const pool = tierFiltered.length ? tierFiltered : displayed;
  const byCat = {};
  pool.forEach(d => {
    (byCat[d.good.category] = byCat[d.good.category] || []).push(d);
  });
  if (customer.mode === "collector" || customer.mode === "collector2") {
    // 各類別都買：collector = 每類 1 件；collector2 = 武器+裝備各 1~2（古董商）或每類各 N
    const targets = customer.mode === "collector2" ? ["weapon", "armor"] : ["weapon", "armor", "food"];
    const picks = [];
    for (const cat of targets) {
      const list = byCat[cat] || [];
      if (!list.length) continue;
      const per = customer.mode === "collector2" ? rndInt(rng, customer.qtyMin, customer.qtyMax) : customer.qtyMin;
  ...
  for (let i = 0; i < qty; i++) {
    if (customer.mode === "random") {
      ...
      const weights = pool.map(d => {
        ...
        const counterBonus = d.slot === "counter" && d.good.category === "food" ? 1 + _shopGoodsCatalog.COUNTER_ATTRACTION_BONUS : 1;
...
const SHOP_SPECIAL_TICKET_REWARDS = Object.entries(_shopData.SPECIAL_TICKET_META).map(([ticketId, meta]) => ({
  id: ticketId,
  type: "special_ticket",
  ticketId,
  icon: meta.icon,
...
const SHOP_EXCHANGE_REWARDS = exports.SHOP_EXCHANGE_REWARDS = [...SHOP_MATERIAL_REWARDS, ...SHOP_SPECIAL_TICKET_REWARDS, {
  id: "potion",
  type: "potion",
  family: null,
  icon: "🧪",
...
function getExchangeRewardById(id) {
...
function getExchangeUsed(shop, rewardId) {
...
function getExchangeRemaining(shop, rewardId) {
  const reward = getExchangeRewardById(rewardId);
  if (!reward) return 0;
...
function planShopExchange(shop, rewardId, count = 1, specialItems = {}) {
  ...
  if (reward.type !== "special_ticket") return {
    reward,
    count,
    cost
  };
...
const SHOP_RUSH_SECONDS_PER_TEN_ARROWS = exports.SHOP_RUSH_SECONDS_PER_TEN_ARROWS = 60;
const SHOP_RUSH_SECONDS_CAP = exports.SHOP_RUSH_SECONDS_CAP = 30 * 60;

// 在「確認下課」成功後以官方累計箭數呼叫。回傳完整持久化欄位與本次差額，
// 讓呼叫端可在 transaction 內寫入 checkpoint，重送同一累計值時不會重複發放。
function claimShopRushTime(shop, officialArrowTotal) {
  ...
  const combinedArrows = Math.max(0, Math.min(9, Math.floor(Number(shop?.rushArrowRemainder) || 0))) + claimedArrowDelta;
  ...
  const rushSeconds = Math.min(SHOP_RUSH_SECONDS_CAP, currentSeconds + convertedSets * SHOP_RUSH_SECONDS_PER_TEN_ARROWS);
  return {
    rushSeconds,
...
// 預設商店狀態（新帳號 / 缺欄位補齊用）
// ────────────────────────────────────────────────────────────
function defaultShopState(now = Date.now()) {
  return {
    level: 1,
    tickets: 0,
    stock: {},
...
function normalizeShop(raw) {
  ...
  const display = Array.isArray(raw.display) && raw.display.length ? raw.display : base.display;
  const stats = {
    ...(base.stats || {}),
    ...(raw.stats || {})
  };
  ...
  const exchange = {
    ...rawExchange,
    date: todayStr(),
    counts: rawExchange.date === todayStr() ? {
      ...(rawExchange.counts || {})
    } : {},
    daily: {
      ...(rawExchange.daily || {}),
    ...
    week: weekStr(),
    weeklyCounts: rawExchange.week === weekStr() ? {
      ...(rawExchange.weeklyCounts || {})
    } : {}
  };
  const out = {
    ...base,
