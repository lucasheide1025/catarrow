...
exports.TIER_ORDER = exports.TIER_LABEL = exports.MONSTERS = exports.HONOR_BONUS_PER_BADGE = exports.FAMILIES = exports.CERT_BONUS = exports.BODY_PARTS = void 0;
exports.applyCertBonus = applyCertBonus;
exports.applyVariant = applyVariant;
exports.calcArcherPower = calcArcherPower;
exports.calcArcherStats = calcArcherStats;
exports.calcCounterDamage = calcCounterDamage;
exports.calcDamage = calcDamage;
exports.calcHonorBonus = calcHonorBonus;
exports.describeStatSources = describeStatSources;
exports.drawExpeditionBoss = drawExpeditionBoss;
exports.drawFloorMonsters = drawFloorMonsters;
exports.drawMatchedMonsters = drawMatchedMonsters;
exports.drawMixedMonsterPool = drawMixedMonsterPool;
exports.drawTreasureKing = drawTreasureKing;
exports.drawTreasureMonsterPool = drawTreasureMonsterPool;
exports.getTierPoolByPower = getTierPoolByPower;
exports.monsterTierNumber = monsterTierNumber;
exports.resolveHitPart = resolveHitPart;
exports.sumStatSources = sumStatSources;
var _constants = require("./constants");
var _equipmentRuneData = require("./equipmentRuneData");
var _archerLevel = require("./archerLevel");
// src/lib/monsterData.js
// 六族36隻怪物 + 射手數值公式 + 匹配系統

// ── 階級定義 ─────────────────────────────────────────────
const TIER_LABEL = exports.TIER_LABEL = {
  common: {
    label: "普通",
    color: "#6b7280",
    bg: "#f3f4f6"
...
// ── 36隻怪物 ─────────────────────────────────────────────
const MONSTERS = exports.MONSTERS = [
// ════ 鬼怪族 ════
{
  id: "ghost_1",
  family: "ghost",
...
// ── 身體部位（殭屍靶紙模式）────────────────────────────
const BODY_PARTS = exports.BODY_PARTS = [{
  id: "head",
  name: "頭部",
  icon: "💀",
  mult: 1.25,
...
function resolveHitPart(score, unlockedParts, isX = false) {
  const unlocked = unlockedParts instanceof Set ? unlockedParts : new Set(unlockedParts || []);
  if (score === 0) return _bp("miss");

  // X 環：保證命中頭部或頸部（一定爆擊）
  if (isX) {
    if (unlocked.has("chest") && Math.random() < 0.35) return _bp(Math.random() < 0.5 ? "heart" : "lung");
    return _bp(Math.random() < 0.5 ? "head" : "neck");
  }

  // 10 分：一定命中，不脫靶，命中高等部位
  if (score === 10) {
    if (unlocked.has("chest") && Math.random() < 0.30) return _bp(Math.random() < 0.5 ? "heart" : "lung");
    if (unlocked.has("groin") && Math.random() < 0.25) return _bp("balls");
    return _pick(["head", "neck", "groin", "chest"]);
  }

  // 8~9 分：中上部位
  if (score >= 8) {
    if (unlocked.has("chest") && Math.random() < 0.15) return _bp(Math.random() < 0.5 ? "heart" : "lung");
    if (unlocked.has("belly") && Math.random() < 0.12) return _bp("kidney");
 ...
 */
const HONOR_BONUS_PER_BADGE = exports.HONOR_BONUS_PER_BADGE = Object.freeze({
  fatCat: {
    bronze: 1,
    silver: 4,
    gold: 12
...
const badgeSum = (owned, table) => Object.entries(table).reduce((sum, [rank, value]) => sum + (Number(owned?.[rank]) || 0) * value, 0);

/** 三種章換算成三圍。回傳的是**要加在夾制之外**的量。 */
function calcHonorBonus(member) {
  return {
 ...
 */
const CERT_BONUS = exports.CERT_BONUS = Object.freeze({
  flat: {
    hp: 100,
    atk: 10,
    def: 10
...
function applyCertBonus(stats, certification) {
  const level = certification?.level;
  if (!hasCert(level)) return {
    ...stats
  };
  const {
    flat,
...
function calcArcherStats({
  ...
  const certLevelScore = (certRecords || []).reduce((s, r) => {
    ...
    const lvName = r.level || (0, _constants.getCertLevelByScores)(r.bowType, r.score) || null;
  ...
  // ── RPG 裝備加成 ─────────────────────────────────────────
  const equip = (0, _constants.calcEquipBonus)(member?.rpgEquip);
  hp = Math.min(800, hp + equip.hpBonus);
  atk = Math.min(160, atk + equip.atkBonus);
  def = Math.min(120, def + equip.defBonus);
  const runeBonus = (0, _equipmentRuneData.getAllEquipmentRuneBonus)(member?.rpgEquip);
...
function describeStatSources({
  ...
  const baseNoCert = calcArcherStats({
    member: stripped,
    certification: null,
    certRecords: [],
    dexStats
  });
  const baseWithCert = calcArcherStats({
    member: stripped,
    certification: null,
    certRecords,
    dexStats
  ...
  const beforeCert = {
    hp: baseWithCert.hp + honor.hp,
    atk: baseWithCert.atk + honor.atk,
    def: baseWithCert.def + honor.def
  };
  const afterCert = applyCertBonus(beforeCert, certification);
  const level = (0, _archerLevel.archerLevelBonus)(archerLevel);
  const rows = [{
    key: "base",
    label: "基礎（裝備・報到・射齡…）",
    ...baseNoCert
...
// ── 戰力評分（用於怪物匹配）─────────────────────────────
function calcArcherPower(stats) {
...
function getTierPoolByPower(power) {
...
// ── 七族各抽1隻（依射手戰力匹配，不烙單）────────────────
// 含寶箱族；王怪仍只會在王房出現。
function drawMatchedMonsters(archerPower) {
  ...
  families.forEach(family => {
    ...
    const candidates = MONSTERS.filter(m => m.family === family && tierPool.includes(m.tier) && !m.isKing);
    let monster;
    if (candidates.length === 0) {
      // fallback：取該族最低階
      const fallback = MONSTERS.filter(m => m.family === family && !m.isKing).sort((a, b) => TIER_ORDER.indexOf(a.tier) - TIER_ORDER.indexOf(b.tier));
...
const TIER_ORDER = exports.TIER_ORDER = ["common", "rare", "elite", "fierce", "boss", "mythic"];
...
function monsterTierNumber(monster) {
...
function drawMixedMonsterPool(count, variant, tier) {
  ...
  return selectedFamilies.map(family => {
    ...
    const candidates = MONSTERS.filter(m => m.family === family && m.tier === tierKey && !m.isKing);
    let monster;
    if (candidates.length === 0) {
      const fallback = MONSTERS.filter(m => m.family === family && !m.isKing).sort((a, b) => TIER_ORDER.indexOf(a.tier) - TIER_ORDER.indexOf(b.tier));
...
function drawFloorMonsters(floorIndex, difficultyTier, options = {}) {
  ...
  const elite = isTreasureRun ? drawTreasureMonsterPool(1, "strong", difficultyTier)[0] : drawMixedMonsterPool(1, "strong", difficultyTier)[0];
  const fixedBoss = options.fixedBoss ? options.fixedBoss.variant === "boss" ? {
    ...options.fixedBoss
  } : applyVariant(options.fixedBoss, "boss") : isTreasureRun ? drawTreasureKing(difficultyTier) : drawExpeditionBoss(difficultyTier, options.family);
  return {
