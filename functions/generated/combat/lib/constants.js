...
exports.EVENT_POINTS = exports.EQUIP_SLOT_DEFS = exports.EQUIP_GRADES = exports.COMP_TYPE_COLOR = exports.COMP_TYPES = exports.CERT_LEVEL_ORDER = exports.CERT_LEVELS = exports.CERT_HALF = exports.CERT_DEFAULT_SCORES = exports.BOW_TYPES = exports.BADGE_WEIGHTS = void 0;
exports.calcAge = calcAge;
exports.calcBadgePoints = calcBadgePoints;
exports.calcEquipBonus = calcEquipBonus;
exports.certLevelStyle = certLevelStyle;
exports.fmtDT = fmtDT;
exports.fmtDate = fmtDate;
exports.formatArcherNo = formatArcherNo;
exports.getCertLevel = getCertLevel;
exports.getCertLevelByScores = getCertLevelByScores;
exports.getEquipSlotBonus = getEquipSlotBonus;
exports.getMonthRange = getMonthRange;
exports.isHigherScore = isHigherScore;
exports.thisYear = thisYear;
exports.today = today;
// ─── 年度檢定預設分數 ──────────────────────────────────────
const CERT_DEFAULT_SCORES = exports.CERT_DEFAULT_SCORES = {
  recurve_full: {
    入門: 60,
    初級: 90,
    中級: 108,
...
// 依分數查詢達到的最高階級
function getCertLevelByScores(bowType, score, scoreTable) {
...
const COMP_TYPES = exports.COMP_TYPES = ["實體賽", "積分賽", "挑戰賽", "臨時任務賽", "年度檢定"];
const COMP_TYPE_COLOR = exports.COMP_TYPE_COLOR = {
  "實體賽": {
...
const EVENT_POINTS = exports.EVENT_POINTS = {
  0: 3,
  1: 2,
  2: 1
};
...
// 成就章：銀 1、金 2、黑 3
const BADGE_WEIGHTS = exports.BADGE_WEIGHTS = {
  fatCat: {
    bronze: 1,
    silver: 10,
    gold: 50
...
function calcBadgePoints(member, type) {
  const map = {
    fatcat: "fatCat",
    achieve: "achievement"
  };
...
const BOW_TYPES = exports.BOW_TYPES = {
  recurve_bare: {
    label: "競技反曲弓（裸弓）",
    short: "裸弓",
    icon: "🏹",
...
function getCertLevel(bowType, score) {
  if (!score || score === 0) return null;
...
const CERT_LEVEL_ORDER = exports.CERT_LEVEL_ORDER = ["入門", "初級", "中級", "進階", "精英", "菁英"];

// 級別 → 樣式（越高越華麗）。回傳 Tailwind class 字串。
// variant: "solid"（實心徽章）| "soft"（深色底頁用）| "softLight"（淺色底頁用，AdminApp 未遷移前使用）
function certLevelStyle(level, variant = "solid") {
  const styles = {
    入門: {
...
function isHigherScore(newScore, oldScore) {
  return Number(newScore || 0) > Number(oldScore || 0);
}

// ─── 裝備系統 ──────────────────────────────────────────────
const EQUIP_GRADES = exports.EQUIP_GRADES = [{
  id: "common",
  name: "普通",
  color: "#94a3b8",
  glow: "#64748b"
...
const EQUIP_SLOT_DEFS = exports.EQUIP_SLOT_DEFS = [{
  id: "bow",
  name: "弓",
  icon: "🏹",
  stat: "atk"
...
// HP 欄位再乘 5
//
// ⚠️ 這條公式刻意維持直線，不要改成凸曲線。曾試算過把神話+4 從 30 拉到 59，
// ATK 四格合計會變 236，但怪物防禦最高只有 200 出頭 —— 平衡直接崩掉。
// 要調精煉難度請改 equipData.js 的材料需求曲線。
function getEquipSlotBonus(slotOrStat, equipment) {
...
function calcEquipBonus(equipment) {
  let atkBonus = 0,
    defBonus = 0,
    hpBonus = 0;
  if (!equipment) return {
  ...
  for (const slot of EQUIP_SLOT_DEFS) {
    ...
    if (slot.stat === "atk") atkBonus += bonus;else if (slot.stat === "def") defBonus += bonus;else if (slot.stat === "hp") hpBonus += bonus;
  }
  return {
    atkBonus,
    defBonus,
