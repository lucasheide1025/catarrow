...
- [ ] 組隊成員看到相同的迷霧地圖、房間進度、第三層分支與寶藏房，且斷線重連後位置一致。
- [ ] 前衛／後衛與倒地後換位狀態會帶入下一個戰鬥房。
- [x] 正式 build 通過。
...
- 組隊遠征必須與單人遠征共用相同的三層探索流程：前兩層 5×5 迷霧地圖、功能房與樓梯，第三層分支、Boss 與寶藏房。
- 組隊地圖由房主控制並同步所有隊員；戰鬥仍使用多人 `DungeonBattleRoom`，保留前衛／後衛、倒地換位及跨房間 HP／buff。
