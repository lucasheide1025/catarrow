...
// 材料總倍率（2026-07-12）：鍛造成本維持高（一格 ~18,450），
// 遠征改成主力產出來源，所有 baseRewards 材料再 ×此值。
export const EXPEDITION_MATERIAL_BOOST = 4;

// 貓 XP/羈絆結算上限（與 db.js 採集結算一致）
...
export const EXPEDITION_MISSIONS = [
  ...
  },
  {
    ...
    baseRewards: [
      ...
      { resource: "ore",      tier: 3, min: 1, max: 2 },  // 打通弓/防具過精英
      { resource: "fish",     tier: 1, min: 1, max: 3 },
      { resource: "meat",     tier: 1, min: 1, max: 2 },
      { resource: "meat",     tier: 2, min: 1, max: 2 },
      { resource: "driedfish",tier: 1, min: 1, max: 2 },  // 讓貓草包終於能開工
    ],
    bonusChance: { arrowdew: 0.3, gachaToken: 0.3 },
  ...
  },
  {
    ...
    baseRewards: [
      ...
      { resource: "ore",      tier: 4, min: 1, max: 2 },  // 高階礦：弓/防具過頭目
      { resource: "fish",     tier: 2, min: 1, max: 3 },
      { resource: "meat",     tier: 2, min: 1, max: 3 },
      { resource: "meat",     tier: 3, min: 1, max: 2 },
  ...
  },
  {
    ...
    baseRewards: [
      ...
      { resource: "ore",      tier: 5, min: 1, max: 2 },  // 頂階礦：弓/防具衝神話
      { resource: "fish",     tier: 3, min: 1, max: 3 },
      { resource: "meat",     tier: 3, min: 1, max: 3 },
      { resource: "meat",     tier: 5, min: 1, max: 2 },  // 頂階肉：箭衝神話
      { resource: "driedfish",tier: 2, min: 1, max: 2 },
      { resource: "driedfish",tier: 3, min: 1, max: 2 },  // 高階小魚乾：貓草包過精英
...
// 回傳物件可含：材料 key（fur_t1…）、arrowdew、gachaToken、catXP、catBond
export function calcExpeditionRewards(missionTier, catData) {
  ...
  for (const r of mission.baseRewards) {
    ...
    // 材料 = 隨機量 × 貓戰力倍率 × 全域材料倍率
    const count = Math.max(1, Math.round(raw * mult * EXPEDITION_MATERIAL_BOOST));
    const key = `${r.resource}_t${r.tier}`;
    rewards[key] = (rewards[key] || 0) + count;
  }

  // 貓咪 XP（×戰力倍率，撞上限）與羈絆（固定值）
  if (mission.catXP) {
    rewards.catXP = Math.min(EXPEDITION_CATXP_CAP, Math.round(mission.catXP * mult));
  }
  if (mission.catBond) {
    rewards.catBond = Math.min(EXPEDITION_CATBOND_CAP, mission.catBond);
  }
