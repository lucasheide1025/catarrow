          ...
          <div className="rounded-2xl p-5 flex flex-col gap-3"
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {activeQuest.bountySource === "daily_general" && (
                <span className="text-xs font-black text-amber-300">📋 每日懸賞・{BOUNTY_DIFF_LABEL[activeQuest.bountyDifficulty] || ""}{activeQuest.autoGenerated ? " 🤖" : ""}</span>
              )}
            </div>
                  ...
                  <button key={q.id} onClick={() => !isSubmitted && lock.ok && handleOpenQuest(q)}
                    ...
                    <div className="p-3 flex flex-col gap-1.5">
                      {q.bountySource === "daily_general" && (
                        <div className="self-start text-[10px] font-black px-1.5 py-0.5 rounded-md flex items-center gap-1" style={{ background:"rgba(180,120,40,0.25)", color:"#fbbf24", border:"1px solid rgba(180,120,40,0.4)" }}>
                          <span>{BOUNTY_DIFF_LABEL[q.bountyDifficulty] || "每日懸賞"}</span>
                          {q.autoGenerated && <span className="text-[9px] opacity-70">🤖</span>}
                        </div>
                      )}
                      {(() => { const chip = questRequirementChip(q); return chip ? (
