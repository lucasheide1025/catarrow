...
function SlotDetailModal({ slot, bookings, blocked, onClose, onChanged, toast }) {
  ...
  async function handleCancel(b) {
    ...
    if ((started || done) && !window.confirm(
      `這筆預約${done ? "已結帳完成" : "課程時間已開始"}，確定要強制取消嗎？`
    )) return;
    setBusy(true);
    const res = await cancelBooking(b.id, { force: true });
    setBusy(false);
  ...
  async function handleReschedule(newSlot) {
    ...
    if (bookingHasStarted(rescheduleTarget) && !window.confirm(
      "這筆預約課程時間已開始，確定要強制改期嗎？"
    )) return;
    setBusy(true);
    const res = await rescheduleBooking(
      rescheduleTarget.id, newSlot.date, newSlot.startTime, newSlot.endTime,
      { force: true, durationHours: newSlot.durationHours, planType: newSlot.planType },
    );
    setBusy(false);
            ...
            {bookings.map(b => (
              <Card key={b.id} className="p-3 flex flex-col gap-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {b.status !== "cancelled" && <Btn v="danger" size="sm" onClick={() => handleCancel(b)} disabled={busy}>{bookingHasStarted(b) || b.status === "completed" ? "強制取消" : "取消"}</Btn>}
                </div>
        ...
        <Modal open onClose={() => setRescheduleTarget(null)} title={`改期：${rescheduleTarget.memberName || "顧客"}`} wide>
          <div className="flex flex-col gap-4">
            <div className="text-slate-400 text-xs">
              ...
              （{PLAN_TYPES.find(p => p.id === rescheduleTarget.planType)?.label || rescheduleTarget.planType}・{durationLabel(rescheduleTarget.durationHours || 1)}，可一併變更方案/時數）
            </div>
            <RescheduleSlotPicker
              initialPlanType={rescheduleTarget.planType}
              initialDuration={rescheduleTarget.durationHours || 1}
...
// 教練改期：可一併變更方案/時數。時數改變會驅動 DateSlotPicker 重新檢查連續時段，
// 並清掉已選時段（舊選擇的連續格需求已不同）。endTime 一律由「新起始 + 新時數」重算，
// 不信任 picker 回傳的 endTime，確保跟新時數一致。
function RescheduleSlotPicker({ initialPlanType = "general", initialDuration = 1, onConfirm }) {
  const [planType, setPlanType] = useState(initialPlanType);
  const [durationHours, setDurationHours] = useState(initialDuration);
  const [slot, setSlot] = useState(null);

  function handlePlanChange({ planType: pt, durationHours: dh }) {
    setPlanType(pt);
    if (dh !== durationHours) { setDurationHours(dh); setSlot(null); }
  }

  return (
    <div className="flex flex-col gap-4">
      <PlanDurationPicker planType={planType} durationHours={durationHours} onChange={handlePlanChange} />
      <DateSlotPicker selected={slot} onSelect={setSlot} durationHours={durationHours} />
      <Btn v="primary" disabled={!slot} onClick={() => onConfirm({
        date: slot.date,
        startTime: slot.startTime,
        endTime: computeEndTime(slot.startTime, durationHours),
        planType, durationHours,
      })}>確認改期到此時段</Btn>
    </div>
  );
}
