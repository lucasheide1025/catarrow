...
// src/lib/monsterEconomyCatalog.js
var monsterEconomyCatalog_exports = {};
__export(monsterEconomyCatalog_exports, {
  MATERIAL_BY_ID: () => MATERIAL_BY_ID,
  NORMAL_DROP_MULTIPLIER: () => NORMAL_DROP_MULTIPLIER,
  NORMAL_MATERIALS: () => NORMAL_MATERIALS,
  SAME_TIER_CONVERSION: () => SAME_TIER_CONVERSION,
  TIER_BASE_MATERIAL: () => TIER_BASE_MATERIAL,
  UPGRADE_CONVERSION: () => UPGRADE_CONVERSION,
  VARIANT_MATERIAL_MULTIPLIER: () => VARIANT_MATERIAL_MULTIPLIER,
  getNormalMaterialPool: () => getNormalMaterialPool,
  getNormalMonsterMaterialDrop: () => getNormalMonsterMaterialDrop,
  previewSameTierConversion: () => previewSameTierConversion,
  previewTierUpgrade: () => previewTierUpgrade,
  validateMonsterEconomyCatalog: () => validateMonsterEconomyCatalog
});
module.exports = __toCommonJS(monsterEconomyCatalog_exports);
...
// src/lib/monsterEconomyCatalog.js
var TIER_BASE_MATERIAL = Object.freeze([0, 1, 1, 2, 2, 3, 3]);
var NORMAL_DROP_MULTIPLIER = Object.freeze({
  solo: 3,
  party: 5,
  dungeonNormal: 3,
  dungeonElite: 4,
...
var VARIANT_MATERIAL_MULTIPLIER = Object.freeze({ weakened: 0.8, normal: 1, strong: 1.2 });
var SAME_TIER_CONVERSION = Object.freeze({
  1: { input: 3, coins: 100 },
  2: { input: 3, coins: 200 },
  3: { input: 3, coins: 400 },
  4: { input: 4, coins: 800 },
  5: { input: 4, coins: 1500 },
  6: { input: 5, coins: 3e3 }
});
var UPGRADE_CONVERSION = Object.freeze({
  1: { input: 5, coins: 300 },
  2: { input: 5, coins: 600 },
  3: { input: 5, coins: 1200 },
  4: { input: 5, coins: 2500 },
  5: { input: 5, coins: 5e3 }
});
var NORMAL_MATERIALS = Object.freeze(EXPANSION_MATERIALS.filter((item) => item.kind === "normal"));
var MATERIAL_BY_ID = Object.freeze(Object.fromEntries(EXPANSION_MATERIALS.map((item) => [item.id, item])));
function positiveInteger(value, code = "invalid_quantity") {
  if (!Number.isInteger(value) || value < 1) throw new Error(code);
}
function getNormalMonsterMaterialDrop({ monsterId, mode, variant = "normal" }) {
...
function previewSameTierConversion({ sourceMaterialId, targetMaterialId, batches }) {
...
function previewTierUpgrade({ sourceMaterialId, targetMaterialId, batches }) {
  positiveInteger(batches, "invalid_batches");
  const source = MATERIAL_BY_ID[sourceMaterialId];
  const target = MATERIAL_BY_ID[targetMaterialId];
  if (!source || !target || source.kind !== "normal" || target.kind !== "normal") throw new Error("normal_material_required");
...
function getNormalMaterialPool({ maxTier = 6, family, exactTier }) {
  if (!Number.isInteger(maxTier) || maxTier < 1 || maxTier > 6) throw new Error("invalid_max_tier");
  return NORMAL_MATERIALS.filter(
    (item) => item.tierIndex <= maxTier && (!exactTier || item.tierIndex === exactTier) && (!family || item.family === family)
  );
}
