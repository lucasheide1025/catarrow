...
var dungeonExpansionMonsters_exports = {};
__export(dungeonExpansionMonsters_exports, {
  DUNGEON_DIFFICULTY_TIER_POOL: () => DUNGEON_DIFFICULTY_TIER_POOL,
  MAX_DUNGEON_TIER: () => MAX_DUNGEON_TIER,
  drawDungeonFallbackMonster: () => drawDungeonFallbackMonster,
  drawDungeonFloorMonsters: () => drawDungeonFloorMonsters,
  drawExpansionDungeonFloorMonsters: () => drawExpansionDungeonFloorMonsters,
  drawExpansionDungeonMonster: () => drawExpansionDungeonMonster,
  getDungeonTierPool: () => getDungeonTierPool
});
module.exports = __toCommonJS(dungeonExpansionMonsters_exports);
...
// src/lib/monsterExpansionAdapter.js
var SOLO_HUNT_FAMILIES = Object.freeze(["ghost", "mountain", "insect", "workplace", "exam", "temple"]);
var TIER_ORDER = Object.freeze(["common", "rare", "elite", "fierce", "boss", "mythic"]);
var TIER_HP_MULTIPLIER = Object.freeze({ common: 0.95, rare: 1, elite: 1.05, fierce: 1.1, boss: 1, mythic: 1.1 });
var FAMILY_ICONS = Object.freeze({ ghost: "\u{1F47B}", mountain: "\u{1F3D4}\uFE0F", insect: "\u{1F982}", workplace: "\u{1F4BC}", exam: "\u{1F4DD}", temple: "\u{1F3F0}", treasure: "\u{1F4E6}" });
function toLegacyBattleMonster(monster) {
...
var SOLO_CHALLENGE_LEVELS = Object.freeze({
  easy: { id: "easy", label: "\u{1F60C} \u8F15\u9B06", statMult: 0.8, materialQty: 3, coinMult: 0.8, coinChestChance: 0.2, cardChance: 0.12, desc: "\u602A\u7269 -20%\uFF5C\u7D20\u6750\xD73\uFF5C\u91D1\u5E63\xD70.8\uFF5C\u91D1\u5E63\u5BF6\u7BB1 20%\uFF5C\u6389\u5361 12%" },
  standard: { id: "standard", label: "\u2694\uFE0F \u6A19\u6E96", statMult: 1, materialQty: 5, coinMult: 1, coinChestChance: 0.5, cardChance: 0.2, desc: "\u602A\u7269\u539F\u503C\uFF5C\u7D20\u6750\xD75\uFF5C\u91D1\u5E63\xD71.0\uFF5C\u91D1\u5E63\u5BF6\u7BB1 50%\uFF5C\u6389\u5361 20%" },
  hard: { id: "hard", label: "\u{1F525} \u6311\u6230", statMult: 1.2, materialQty: 7, coinMult: 1.5, coinChestChance: 1, cardChance: 0.3, desc: "\u602A\u7269 +20%\uFF5C\u7D20\u6750\xD77\uFF5C\u91D1\u5E63\xD71.5\uFF5C\u91D1\u5E63\u5BF6\u7BB1\u5FC5\u6389\uFF5C\u6389\u5361 30%" }
});
var PARTY_COUNT_BONUS = Object.freeze({
  monsterStatPctPerExtra: 10,
  materialQtyPerExtra: 1,
  cardChancePctPerExtra: 5,
  coinChestChancePctPerExtra: 15
});
function applySoloVariant(monster, variant, roll) {
  const ranges = {
    weak: [[0.78, 0.92], [0.78, 0.92], [0.78, 0.92]],
    strong: [[1.15, 1.4], [1.1, 1.3], [1.1, 1.3]]
  };
  const range = ranges[variant];
  const multipliers = range ? range.map(([low, high]) => low + (high - low) * roll) : [1, 1, 1];
  return {
    ...monster,
    variant,
...
// src/lib/monsterExpansionFeature.js
function isMonsterExpansionEnabled() {
  return true;
}

// src/lib/equipmentRuneData.js
var EQUIPMENT_RUNE_TYPES = {
  atk: { name: "\u653B\u64CA\u7B26\u6587", icon: "\u2694\uFE0F", stat: "atk" },
  def: { name: "\u9632\u79A6\u7B26\u6587", icon: "\u{1F6E1}\uFE0F", stat: "def" },
  hp: { name: "\u751F\u547D\u7B26\u6587", icon: "\u2764\uFE0F", stat: "hp" },
  cat: { name: "\u8C93\u9748\u7B26\u6587", icon: "\u{1F431}", stat: "all" }
};
var EQUIPMENT_RUNE_TIERS = [
  ...
  { tier: 4, name: "\u738B\u7D1A", bonus: 0.16, fragmentCost: 36, goldCost: 6e3 }
];
var EQUIPMENT_RUNES = Object.fromEntries(
  Object.entries(EQUIPMENT_RUNE_TYPES).flatMap(
    ([type, meta]) => EQUIPMENT_RUNE_TIERS.map((config) => {
...
var BODY_PARTS = [
  { id: "head", name: "\u982D\u90E8", icon: "\u{1F480}", mult: 1.25, locked: false },
  { id: "neck", name: "\u9838\u90E8", icon: "\u{1F3AF}", mult: 1.2, locked: false },
  { id: "chest", name: "\u80F8\u8154", icon: "\u2764\uFE0F", mult: 1.1, locked: false },
  { id: "belly", name: "\u8179\u90E8", icon: "\u{1FAC1}", mult: 1.05, locked: false },
  { id: "arm", name: "\u624B\u81C2", icon: "\u{1F4AA}", mult: 1, locked: false },
  { id: "groin", name: "\u9F20\u8E4A", icon: "\u26A1", mult: 1.15, locked: false },
  { id: "heart", name: "\u5FC3\u81DF", icon: "\u2764\uFE0F\u200D\u{1F525}", mult: 1.5, locked: true },
  // 需先命中胸腔
  { id: "lung", name: "\u80BA\u8449", icon: "\u{1FAC1}", mult: 1.35, locked: true },
  // 需先命中胸腔
  { id: "kidney", name: "\u814E\u81DF", icon: "\u{1FAD8}", mult: 1.3, locked: true },
  // 需先命中腹部
  { id: "balls", name: "\u8981\u5BB3", icon: "\u{1F4A5}", mult: 1.4, locked: true },
  // 需先命中鼠蹊
  { id: "miss", name: "\u812B\u9776", icon: "\u{1F4A8}", mult: 0, locked: false }
];
var _BP_MAP = {};
...
var VARIANT_RANGE = {
  ...
  strong: { hp: [1.15, 1.4], atk: [1.1, 1.3], def: [1.1, 1.3] }
};
var VARIANT_FIXED = {
  normal: { hp: 1, atk: 1, def: 1 },
  boss: { hp: 2, atk: 1.6, def: 1.6 }
};
var TIER_COMBAT_HP_MULTIPLIER = {
  common: 0.95,
  rare: 1,
  elite: 1.05,
  fierce: 1.1,
...
function applyVariant(monster, variant) {
...
var TIER_ORDER2 = ["common", "rare", "elite", "fierce", "boss", "mythic"];
var FAMILY_KEYS = ["ghost", "mountain", "insect", "workplace", "exam", "temple"];
function drawMixedMonsterPool(count, variant, tier) {
  const tierKey = TIER_ORDER2[Math.max(0, Math.min(5, (tier || 1) - 1))];
  ...
  return selectedFamilies.map((family) => {
    ...
    const candidates = MONSTERS.filter(
      (m) => m.family === family && m.tier === tierKey && !m.isKing
    );
    let monster;
    if (candidates.length === 0) {
      const fallback = MONSTERS.filter((m) => m.family === family && !m.isKing).sort((a, b) => TIER_ORDER2.indexOf(a.tier) - TIER_ORDER2.indexOf(b.tier));
...
function drawTreasureMonsterPool(count, variant, tier) {
  const tierKey = TIER_ORDER2[Math.max(0, Math.min(5, (tier || 1) - 1))];
...
function drawTreasureKing(difficultyTier) {
  const tierKey = TIER_ORDER2[Math.max(0, Math.min(5, (difficultyTier || 1) - 1))];
...
function drawExpeditionBoss(difficultyTier, family = null) {
  const tierKey = TIER_ORDER2[Math.max(0, Math.min(5, (difficultyTier || 1) - 1))];
  const familyKey = FAMILY_KEYS.includes(family) ? family : FAMILY_KEYS[Math.floor(Math.random() * FAMILY_KEYS.length)];
  const monster = MONSTERS.find((m) => m.family === familyKey && m.tier === tierKey) || MONSTERS.find((m) => m.tier === tierKey) || MONSTERS[0];
  return applyVariant(monster, "boss");
}
function drawFloorMonsters(floorIndex, difficultyTier, options = {}) {
  const isTreasureRun = options.family === "treasure";
  if (floorIndex === 0) {
    const count = 2 + Math.floor(Math.random() * 2);
    return {
      monsters: isTreasureRun ? drawTreasureMonsterPool(count, "weak", difficultyTier) : drawMixedMonsterPool(count, "weak", difficultyTier),
      elite: null,
      boss: null
    };
  }
  if (floorIndex === 1) {
    ...
    const elite2 = isTreasureRun ? drawTreasureMonsterPool(1, "strong", difficultyTier)[0] : drawMixedMonsterPool(1, "strong", difficultyTier)[0];
    return {
      monsters: isTreasureRun ? drawTreasureMonsterPool(count, "normal", difficultyTier) : drawMixedMonsterPool(count, "normal", difficultyTier),
      elite: elite2 || null,
      boss: null
    };
  }
  const elite = isTreasureRun ? drawTreasureMonsterPool(1, "strong", difficultyTier)[0] : drawMixedMonsterPool(1, "strong", difficultyTier)[0];
  const fixedBoss = options.fixedBoss ? options.fixedBoss.variant === "boss" ? { ...options.fixedBoss } : applyVariant(options.fixedBoss, "boss") : isTreasureRun ? drawTreasureKing(difficultyTier) : drawExpeditionBoss(difficultyTier, options.family);
  return {
    monsters: isTreasureRun ? drawTreasureMonsterPool(3, "strong", difficultyTier) : drawMixedMonsterPool(3, "strong", difficultyTier),
    elite: elite || null,
    boss: fixedBoss || null
  };
}
...
// src/lib/dungeonExpansionMonsters.js
var FAMILY_ALIASES = Object.freeze({
  ghost: "ghost",
  forest: "mountain",
  mountain: "mountain",
  poison: "insect",
...
var DUNGEON_DIFFICULTY_TIER_POOL = Object.freeze({
  1: ["common"],
  // T1
  2: ["rare"],
  // T2
...
var MAX_DUNGEON_TIER = 6;
function getDungeonTierPool(difficulty) {
...
function pickExpansionCandidate(family, difficulty, random) {
  ...
  const candidates = EXPANSION_MONSTERS.filter(
    (monster) => monster.family === normalizedFamily && monster.encounter === "normal" && tierPool.includes(monster.tier)
  );
  if (!candidates.length) return null;
  return candidates[Math.min(candidates.length - 1, Math.floor(random() * candidates.length))];
}
function drawExpansionDungeonMonster(variant, difficulty, { family, random = Math.random } = {}) {
...
function drawExpansionDungeonFloorMonsters(floorIndex, difficulty, { family, fixedBoss, random = Math.random } = {}) {
  if (floorIndex === 0) {
...
function drawDungeonFloorMonsters(floorIndex, difficulty, options = {}) {
...
function drawDungeonFallbackMonster(variant, difficulty, { family } = {}) {
  if (isMonsterExpansionEnabled()) {
    const monster = drawExpansionDungeonMonster(variant, difficulty, { family });
    if (monster) return monster;
  }
  const legacy = family === "treasure" ? drawTreasureMonsterPool(1, variant, difficulty) : drawMixedMonsterPool(1, variant, difficulty);
