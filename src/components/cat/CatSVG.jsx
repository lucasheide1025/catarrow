// src/components/cat/CatSVG.jsx — 貓咪像素小圖示
// 使用簡單色塊組成可愛貓臉，依花色上色
...
// 通用貓臉形狀（16×16 格子）
// b = base color, p = patch color, l = light color
...
export default function CatSVG({ catId, size = 64, deceased = false }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}
      ...
      {deceased && (
        <circle cx={size/2} cy={size/2} r={size/2} fill="#312e81" opacity={0.3}/>
      )}
      <Render sz={cellSz}/>
      {deceased && (
        <>
