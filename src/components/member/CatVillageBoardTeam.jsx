        ...
        <div className="fixed inset-0 z-[215] bg-black/85 flex items-center justify-center p-4" onClick={confirmCard}>
          <div className="[perspective:1000px]">
            <div className="relative w-64 h-96 transition-transform duration-500" style={{ transformStyle:"preserve-3d", transform: card.flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
              <div className="absolute inset-0 rounded-3xl border-4 flex items-center justify-center [backface-visibility:hidden]" style={{ borderColor: card.event.deck==="fate"?"#f59e0b":"#38bdf8", background: card.event.deck==="fate"?"linear-gradient(160deg,#7c2d12,#431407)":"linear-gradient(160deg,#075985,#0c4a6e)" }}><div className="text-4xl">{card.event.deck==="fate"?"🎴 命運":"🎴 機會"}</div></div>
              <div className="absolute inset-0 rounded-3xl border-4 flex flex-col items-center justify-center p-5 text-center [backface-visibility:hidden]" style={{ transform:"rotateY(180deg)", borderColor: card.event.deck==="fate"?"#f59e0b":"#38bdf8", background: card.event.deck==="fate"?"linear-gradient(160deg,#fed7aa,#fdba74)":"linear-gradient(160deg,#bae6fd,#7dd3fc)" }}>
