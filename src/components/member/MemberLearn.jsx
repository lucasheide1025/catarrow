    ...
    <Card className="p-4">
      ...
      {log.studentNote && <div className="mb-3">
        <div className="flex items-center gap-2 mb-1.5">
          <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-xs font-bold">我</div>
      ...
      {log.coachNote ? <>
        ...
        {log.studentReply && !replying ? (
          <div>
            ...
            <div className="bg-blue-50 rounded-xl p-3 text-gray-700 text-sm whitespace-pre-wrap leading-relaxed mb-2">{log.studentReply}</div>
          ...
          <button onClick={() => setReplying(true)}
            className="w-full text-sm text-blue-600 font-bold border border-blue-200 rounded-xl py-2 bg-blue-50 active:scale-98 transition-transform">
            ＋ 回覆教練
          </button>
        )}
      </> : <div className="text-gray-300 text-xs text-center py-2 border border-dashed border-gray-200 rounded-lg">等待教練回饋…</div>}
    </Card>
  );
}
