...
// ── 半透明卡片樣式 ────────────────────────────────────────────
const CS  = { background:"rgba(255,255,255,0.6)", backdropFilter:"blur(10px)" };
const CS2 = { background:"rgba(255,255,255,0.45)", backdropFilter:"blur(8px)" };
...
function SetupPhase({ initial, equipSets, onStart }) {
  ...
  function selectFormat(id) {
    ...
    if (f?.isTriple) up("arrowCount", 3); // 預設三連靶每靶1箭
  }
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <Card className="p-4" style={CS}>
        <div className="text-xs font-bold text-gray-600 mb-2">弓種</div>
        <div className="flex flex-wrap gap-2">
          {(equipSets.length > 0 ? equipSets : null)?.map(s => {
            const cat = s.bowCategory || "recurve_bare";
            const lbl = s.label || bowLabel(cat);
            return (
              <button key={s.id} onClick={() => up("bowType", cat)}
                className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all
                  ${form.bowType===cat ? "bg-blue-600 text-white border-blue-600" : "bg-white/60 text-gray-700 border-gray-300"}`}>
                {lbl}
            ...
            <button key={b.value} onClick={() => up("bowType", b.value)}
              className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all
                ${form.bowType===b.value ? "bg-blue-600 text-white border-blue-600" : "bg-white/60 text-gray-700 border-gray-300"}`}>
              {b.label}
            </button>
          ))}
        </div>
      ...
      {/* 距離 */}
      <Card className="p-4" style={CS}>
        <div className="text-xs font-bold text-gray-600 mb-2">距離</div>
        <div className="flex flex-wrap gap-2 mb-3">
          {DISTANCES.map(d => (
            <button key={d} onClick={() => selectDist(d)}
              className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all
                ${form.distance===d ? "bg-blue-600 text-white border-blue-600" : "bg-white/60 text-gray-700 border-gray-300"}`}>
              {d}m
            </button>
          ))}
        </div>
        <div className="flex gap-2">
          <input type="number" min="1" max="200" step="0.5" value={customDist}
            ...
            className="flex-1 border border-gray-300 rounded-lg px-3 py-1.5 text-sm bg-white/60" />
        ...
        {!DISTANCES.includes(form.distance) && form.distance > 0 && (
          <div className="text-xs text-blue-600 mt-1">已選：{form.distance}m（自訂）</div>
        )}
      </Card>

      {/* 靶紙格式 */}
      <Card className="p-4" style={CS}>
        <div className="text-xs font-bold text-gray-600 mb-2">靶紙格式</div>
        <div className="grid grid-cols-2 gap-2">
          {TARGET_FORMATS.map(f => (
            <button key={f.id} onClick={() => selectFormat(f.id)}
              className={`py-2 rounded-xl text-xs font-bold border transition-all
                ${form.targetFormat===f.id ? "bg-green-600 text-white border-green-600" : "bg-white/60 text-gray-700 border-gray-300"}`}>
              {f.label}
              {f.isTriple && <div className="text-xs opacity-70 mt-0.5">上·中·下各 N 箭</div>}
            </button>
          ))}
        </div>
        {/* 三連靶：每靶幾箭 */}
        {fmt.isTriple && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            <div className="text-xs font-bold text-gray-600 mb-2">每靶箭數</div>
            <div className="flex gap-2">
              {[1,2].map(n => (
                <button key={n} onClick={() => up("arrowCount", n*3)}
                  className={`flex-1 py-2 rounded-xl text-xs font-bold border transition-all
                    ${arrowsPerSpot===n ? "bg-green-600 text-white border-green-600" : "bg-white/60 text-gray-700 border-gray-300"}`}>
                  每靶 {n} 箭（共 {n*3} 箭/組）
                </button>
              ))}
            </div>
      ...
      <Card className="p-4" style={CS}>
        <div className="grid grid-cols-2 gap-3">
          {!fmt.isTriple && (
            <div>
              <div className="text-xs font-bold text-gray-600 mb-2">每組箭數</div>
              <div className="flex gap-1.5">
                {[3,6,12].map(n => (
                  <button key={n} onClick={() => up("arrowCount", n)}
                    className={`flex-1 py-1.5 rounded-lg text-xs font-bold border transition-all
                      ${form.arrowCount===n ? "bg-blue-600 text-white border-blue-600" : "bg-white/60 text-gray-700 border-gray-300"}`}>
                    {n}
                  </button>
                ))}
              </div>
          ...
          {fmt.isTriple && (
            <div>
              <div className="text-xs font-bold text-gray-600 mb-2">每組箭數</div>
              <div className="py-1.5 px-3 rounded-lg text-xs font-bold bg-green-50 text-green-700 border border-green-200 text-center">{form.arrowCount} 箭（鎖定）</div>
            </div>
          )}
          <div>
            <div className="text-xs font-bold text-gray-600 mb-2">組數</div>
            <div className="flex gap-1.5">
              {[3,6,10,12].map(n => (
                <button key={n} onClick={() => up("roundCount", n)}
                  className={`flex-1 py-1.5 rounded-lg text-xs font-bold border transition-all
                    ${form.roundCount===n ? "bg-blue-600 text-white border-blue-600" : "bg-white/60 text-gray-700 border-gray-300"}`}>
                  {n}
                </button>
              ))}
            </div>
      ...
      <Card className="p-4" style={CS}>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <div className="text-xs font-bold text-gray-600 mb-1">日期</div>
            <input type="date" value={form.date} onChange={e => up("date", e.target.value)}
              className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm bg-white/60" />
          </div>
          <div>
            <div className="text-xs font-bold text-gray-600 mb-1">備注</div>
      ...
      <div className="text-center text-xs text-blue-600 bg-blue-50/80 border border-blue-100/50 rounded-xl p-3 backdrop-blur-sm">
        {equipSets.find(s=>s.bowCategory===form.bowType)?.label || bowLabel(form.bowType)} · {form.distance}m · {fmt.label}
      ...
      <button onClick={() => onStart(form)}
        className="w-full py-4 bg-blue-600 text-white font-black text-lg rounded-xl">
        🎯 開始練習
      </button>
    </div>
  );
    ...
    <div className="flex flex-col gap-4 p-4">
      <Card className="p-4" style={CS}>
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-gray-600">第 {round+1} / {form.roundCount} 組</span>
          <span className="text-xs text-gray-500">{bowLabel(form.bowType)} · {form.distance}m · {fmt.label}</span>
        </div>
        <div className="w-full bg-gray-200/60 rounded-full h-1.5 mb-3">
          <div className="bg-blue-500 h-1.5 rounded-full transition-all"
            style={{ width:`${(round/form.roundCount)*100}%` }} />
        </div>
        {/* 三連靶分組標示 */}
        {fmt.isTriple && form.arrowCount >= 3 && (
          <div className="flex gap-2 mb-2 text-xs text-gray-500 justify-center">
            {["上","中","下"].map((s,i) => (
              <span key={i} className={`px-2 py-0.5 rounded-full ${Math.floor(cur.length/(form.arrowCount/3))===i ? "bg-blue-100 text-blue-700 font-bold" : "bg-gray-100"}`}>
                {s}靶
              </span>
            ))}
          </div>
        ...
        <div className="flex gap-2 justify-center flex-wrap mb-3">
          {Array.from({length:form.arrowCount},(_,i)=>{
            ...
            if (s===undefined) return (
              <div key={i} className="w-10 h-10 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-300 text-xs">●</div>
            );
        ...
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">本組：<strong className="text-blue-600">{curNumeric}</strong></span>
          <span className="text-gray-600">累計：<strong className="text-green-600">{prevTotal+curNumeric}</strong></span>
        </div>
      </Card>
      ...
      <Card className="p-4" style={CS}>
        <div className="grid gap-3" style={{ gridTemplateColumns:`repeat(${cols},1fr)` }}>
          {btns.map(s => {
            const c = scoreColor(s, fmt);
            return (
              <button key={s} onClick={()=>addArrow(s)}
                disabled={cur.length>=form.arrowCount}
      ...
      <div className="flex gap-3">
        <button onClick={()=>setCur(p=>p.slice(0,-1))} disabled={cur.length===0}
          className="flex-1 py-3 rounded-xl border border-gray-300/70 text-gray-600 font-bold text-sm disabled:opacity-30 bg-white/40">
          ← 刪除
        </button>
      ...
      {allR.length>0 && (
        <Card className="p-4" style={CS2}>
          <div className="text-xs text-gray-500 font-bold mb-2">已完成各組</div>
          {allR.map((r,i)=>(
            <div key={i} className="flex items-center gap-2 py-1.5 border-b border-gray-100 last:border-0 text-xs">
              <span className="text-gray-500 w-8">第{i+1}組</span>
              <div className="flex gap-1 flex-1">
                {r.map((v,j)=>{
                  const c=scoreColor(v,fmt);
                  return <span key={j} className="w-6 h-6 rounded-full flex items-center justify-center font-bold"
                    style={{ background:c.bg, color:c.text, fontSize:10 }}>{v}</span>;
                })}
              </div>
              <span className="font-bold text-gray-700">{numericArr([r]).reduce((a,b)=>a+b,0)}</span>
            </div>
          ))}
        </Card>
      )}
      <button onClick={onCancel} className="text-center text-gray-500 text-xs py-2">取消練習</button>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-4 p-4">
      <Card className="p-5" style={CS}>
        <div className="text-center mb-4">
          <div className="text-gray-500 text-xs mb-1">{bowLabel(form.bowType)} · {form.distance}m · {fmt.label}</div>
          <div className="font-black text-5xl text-gray-800">{stats.total}</div>
          <div className="text-gray-500 text-sm mt-1">總環數 / {stats.arrows} 箭</div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          {[["均分/箭",stats.avgPerArrow.toFixed(1)],["命中率",`${stats.hitRate}%`],["失誤",stats.misses]].map(([k,v])=>(
            <div key={k} className="bg-white/60 rounded-xl p-2.5 text-center">
              ...
              <div className="font-black text-lg text-gray-800">{v}</div>
            </div>
          ))}
        </div>
      </Card>
      <Card className="p-4" style={CS}>
        <div className="text-xs text-gray-600 font-bold mb-3">各組成績</div>
        <div className="flex items-end gap-1" style={{ height:80 }}>
          {roundSubs.map((s,i)=>(
            <div key={i} className="flex flex-col items-center flex-1 gap-1">
              <div className="text-xs text-gray-500">{s}</div>
              <div className="w-full bg-blue-500 rounded-t-sm" style={{ height:`${(s/maxSub)*48}px` }} />
              <div className="text-gray-400" style={{ fontSize:9 }}>{i+1}</div>
            </div>
          ))}
        </div>
      </Card>
      <div className="flex gap-3">
        <button onClick={onRetry} className="flex-1 py-3 rounded-xl border border-gray-300/70 text-gray-600 font-bold text-sm bg-white/40">重新練習</button>
...
const TYPE_BADGE = {
  practice:  { label:"練習",    bg:"#eff6ff", text:"#1d4ed8" },
  monster:   { label:"⚔️ 打怪", bg:"#fef2f2", text:"#b91c1c" },
  party:     { label:"👥 組隊", bg:"#f5f3ff", text:"#7c3aed" },
  worldboss: { label:"🌍 世界王",bg:"#fff7ed", text:"#c2410c" },
};
...
function HistoryTab({ logs, monsterLogs }) {
  ...
  const distsUsed = useMemo(()=>[
    ...new Set([...logs.filter(l=>!l.source),...monsterLogs.filter(l=>l.distance)].map(l=>Number(l.distance)))
  ].filter(Boolean).sort((a,b)=>a-b),[logs,monsterLogs]);
  ...
  if (!logs.length&&!monsterLogs.length) return <div className="p-8 text-center text-gray-500 text-sm">還沒有歷史紀錄</div>;
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
        ...
        {SOURCE_OPTS.map(opt=>(
          <button key={opt.id} onClick={()=>setSourceFilter(opt.id)}
            className={`px-3 py-1 rounded-full text-xs font-bold border flex-shrink-0 transition-all
              ${sourceFilter===opt.id?"bg-blue-600 text-white border-blue-600":"bg-white/50 text-gray-600 border-gray-300"}`}>
            {opt.label}
          </button>
        ))}
      </div>

      {(sourceFilter==="all"||sourceFilter==="practice")&&(bowsUsed.length>0||distsUsed.length>0)&&(
        <div className="flex gap-2 overflow-x-auto pb-1">
          {bowsUsed.map(b=>(
            <button key={b} onClick={()=>setBowFilter(f=>f===b?"":b)}
              className={`px-3 py-1 rounded-full text-xs font-bold border flex-shrink-0
                ${bowFilter===b?"bg-blue-600 text-white border-blue-600":"bg-white/50 text-gray-600 border-gray-300"}`}>
              {bowLabel(b)}
            </button>
          ))}
          {distsUsed.map(d=>(
            <button key={d} onClick={()=>setDistFilter(f=>f===d?"":d)}
              className={`px-3 py-1 rounded-full text-xs font-bold border flex-shrink-0
                ${distFilter===d?"bg-green-600 text-white border-green-600":"bg-white/50 text-gray-600 border-gray-300"}`}>
              {d}m
            </button>
          ))}
        </div>
      ...
      {filtered.length===0&&<div className="text-center text-gray-500 text-sm py-6">沒有符合的紀錄</div>}
                ...
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      <span className="font-bold text-gray-800 text-sm">{shortDate(log.date)}</span>
                    </div>
                    <div className="text-xs text-gray-500">{bowLabel(log.bowType)} · {log.distance}m · {fmt.label}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-black text-xl text-gray-800">{log.total??stats.total}</div>
                    <div className="text-xs text-gray-500">{(log.avgPerArrow??stats.avgPerArrow).toFixed(1)}/箭</div>
                  </div>
                </div>
                <div className="flex gap-3 text-xs text-gray-500 flex-wrap">
                  <span>命中 {stats.hitRate}%</span><span>失誤 {stats.misses}</span>
                  <span>{log.arrowCount}箭×{log.roundCount}組</span>
                  {log.note&&<span className="text-blue-600 truncate max-w-24">{log.note}</span>}
                  <span className="ml-auto text-gray-400">{isOpen?"▲":"▼"}</span>
                </div>
              </button>
              {/* 教練留言 */}
              {log.coachNote&&(
                <div className="mt-2 text-xs text-amber-700 bg-amber-50/70 border border-amber-200/60 rounded-lg p-2 backdrop-blur-sm">
                  💬 教練：{log.coachNote}
                </div>
              )}
                ...
                <div className="mt-3 pt-3 border-t border-gray-200/60">
                  {(log.rounds||[]).map((r,i)=>(
                    <div key={i} className="flex items-center gap-2 py-1.5 border-b border-gray-100 last:border-0">
                      <span className="text-xs text-gray-500 w-8">第{i+1}組</span>
                      <div className="flex gap-1 flex-1">
                      ...
                      <span className="font-bold text-gray-700 text-sm">{numericArr([r]).reduce((a,b)=>a+b,0)}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <button className="w-full text-left" onClick={toggle}>
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background:badge.bg,color:badge.text }}>{badge.label}</span>
                      <span className="font-bold text-gray-800 text-sm">{shortDate(log._date)}</span>
                    </div>
                    <div className="text-xs text-gray-500">{log.monsterName||"怪物"}</div>
                    {log.distance&&<div className="text-xs text-gray-400">{log.distance}m</div>}
                  </div>
                  <div className="text-right flex flex-col items-end gap-1">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${won?"bg-green-100 text-green-700":"bg-gray-100 text-gray-500"}`}>
                      {won?"勝利":"落敗"}
                    </span>
                    <div className="font-black text-xl text-gray-800">{total}</div>
                  </div>
                </div>
                <div className="text-xs text-gray-500">{rounds.length} 組 <span className="ml-1 text-gray-400">{isOpen?"▲":"▼"}</span></div>
              </button>
              {log.coachNote&&(
                <div className="mt-2 text-xs text-amber-700 bg-amber-50/70 border border-amber-200/60 rounded-lg p-2">
                  💬 教練：{log.coachNote}
                </div>
              )}
        ...
        return (
          <Card key={log.id} className="p-4" style={CS}>
            <button className="w-full text-left" onClick={toggle}>
              ...
              <div className="text-xs text-gray-500">{(log.rounds||[]).length} 組 <span className="ml-1 text-gray-400">{isOpen?"▲":"▼"}</span></div>
            </button>
            {log.coachNote&&(
              <div className="mt-2 text-xs text-amber-700 bg-amber-50/70 border border-amber-200/60 rounded-lg p-2">
                💬 教練：{log.coachNote}
...
function TrendView({ logs }) {
  ...
  if (!logs.length) return <div className="text-center text-gray-500 text-sm py-6">還沒有練習紀錄</div>;
  return (
    <div className="flex flex-col gap-4">
        ...
        {combos.map(c=>{ const key=`${c.bowType}_${c.distance}`; return(
          <button key={key} onClick={()=>setSelected(key)}
            className={`px-2.5 py-1 rounded-full text-xs font-bold border transition-all
              ${sel===key?"bg-green-600 text-white border-green-600":"bg-white/50 text-gray-600 border-gray-300"}`}>
      ...
      <Card className="p-4" style={CS}>
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs text-gray-600 font-bold">均分／箭 趨勢</span>
...
function DistanceView({ logs }) {
  ...
  if (!combos.length) return <div className="text-center text-gray-500 text-sm py-6">還沒有練習紀錄</div>;
  return (
    <Card className="p-4" style={CS}>
      <div className="grid grid-cols-4 text-xs text-gray-500 font-bold px-1 pb-2 border-b border-gray-200/60">
      ...
      {combos.map(c=>{
        ...
        return (
          <div key={`${c.bowType}_${c.distance}`} className="grid grid-cols-4 items-center py-2.5 border-b border-gray-100 last:border-0">
            <div><div className="text-gray-800 font-bold text-xs">{bowLabel(c.bowType)}</div><div className="text-gray-500 text-xs">{c.distance}m</div></div>
            <div className="text-center text-gray-600 font-bold text-sm">{c.logs.length}</div>
            <div className="text-center text-amber-500 font-black text-sm">{best.toFixed(1)}</div>
            <div className="text-center">
              <span className={`font-black text-sm ${last>=best?"text-green-600":"text-blue-600"}`}>{last.toFixed(1)}</span>
              {last>=best&&<div className="text-green-500 leading-none" style={{ fontSize:9 }}>PB!</div>}
            </div>
          </div>
        );
      })}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end" onClick={e=>e.target===e.currentTarget&&onCancel()}>
      <div className="w-full rounded-t-2xl p-5 flex flex-col gap-4" style={{ background:"rgba(15,23,42,0.92)",backdropFilter:"blur(12px)",maxHeight:"85vh",overflowY:"auto" }}>
        ...
        <div>
          <div className="text-xs font-bold text-gray-400 mb-2">目標類型</div>
          <div className="flex gap-2">
            {[["simple","簡單"],["complete","完整"]].map(([m,l])=>(
              <button key={m} onClick={()=>up("mode",m)}
                className={`flex-1 py-2 rounded-xl text-xs font-bold border transition-all
                  ${form.mode===m?"bg-blue-600 text-white border-blue-600":"bg-white/10 text-gray-300 border-white/20"}`}>{l}</button>
            ))}
          </div>
        </div>
        <div>
          <div className="text-xs font-bold text-gray-400 mb-2">弓種</div>
          <div className="flex flex-wrap gap-2">
            {BOW_OPTIONS.map(b=>(
              <button key={b.value} onClick={()=>up("bowType",b.value)}
                className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all
                  ${form.bowType===b.value?"bg-blue-600 text-white border-blue-600":"bg-white/10 text-gray-300 border-white/20"}`}>{b.label}</button>
            ))}
          </div>
        </div>
        <div>
          <div className="text-xs font-bold text-gray-400 mb-2">距離</div>
          <div className="flex flex-wrap gap-2">
            {DISTANCES.map(d=>(
              <button key={d} onClick={()=>up("distance",d)}
                className={`px-2.5 py-1 rounded-full text-xs font-bold border transition-all
                  ${form.distance===d?"bg-blue-600 text-white border-blue-600":"bg-white/10 text-gray-300 border-white/20"}`}>{d}m</button>
            ))}
          </div>
        </div>
        <div>
          <div className="text-xs font-bold text-gray-400 mb-2">靶紙格式</div>
          <div className="grid grid-cols-2 gap-2">
            {TARGET_FORMATS.map(f=>(
              <button key={f.id} onClick={()=>up("targetFormat",f.id)}
                className={`py-2 rounded-xl text-xs font-bold border transition-all
                  ${form.targetFormat===f.id?"bg-green-600 text-white border-green-600":"bg-white/10 text-gray-300 border-white/20"}`}>{f.label}</button>
            ))}
          </div>
        </div>
        <div>
          <div className="text-xs font-bold text-gray-400 mb-1">目標均分/箭</div>
        ...
        {form.mode==="complete"&&(
          <>
            <div>
              <div className="text-xs font-bold text-gray-400 mb-1">每週目標場次</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="text-xs font-bold text-gray-400 mb-1">週期起</div>
                <input type="date" value={form.cycleStart} onChange={e=>up("cycleStart",e.target.value)}
                  className="w-full border border-white/20 rounded-lg px-3 py-2 text-sm bg-white/10 text-white" />
              </div>
              <div>
                <div className="text-xs font-bold text-gray-400 mb-1">週期訖</div>
...
function GoalView({ logs, profile }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {goals.length===0&&<div className="text-center text-gray-500 text-sm py-6">還沒有設定目標</div>}
      {goals.map(g=>{
        ...
        return (
          <Card key={g.id} className="p-4" style={CS}>
            <div className="flex justify-between items-start mb-3">
              <div>
                <div className="font-bold text-gray-800 text-sm">{bowLabel(g.bowType)} · {g.distance}m</div>
                <div className="text-xs text-gray-500">{fmt.label}{g.mode==="complete"?" · 完整模式":""}</div>
              </div>
              <div className="flex gap-3">
                <button onClick={()=>setEditGoal(g)} className="text-xs text-blue-600">編輯</button>
            ...
            {!!g.targetAvg&&(
              <div className="mb-3">
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-gray-600">目標均分 {g.targetAvg}/箭</span>
                  <span className="font-bold text-blue-600">{prog.recentAvg.toFixed(1)} / {g.targetAvg}</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {!!g.weeklyTarget&&(
                  <div className="bg-white/60 rounded-xl p-2.5 text-center">
                    <div className="text-xs text-gray-500">本週場次</div>
                    <div className="font-black text-lg text-gray-800">{prog.thisWeek}<span className="text-xs text-gray-500">/{g.weeklyTarget}</span></div>
                  </div>
                )}
                {prog.cycle&&(
                  <div className="bg-white/60 rounded-xl p-2.5 text-center">
                    <div className="text-xs text-gray-500">週期進度</div>
                    <div className="font-black text-lg text-gray-800">{prog.cycle.elapsed}<span className="text-xs text-gray-500">%</span></div>
                    <div className="text-xs text-gray-500">{prog.cycle.sessions} 場</div>
                  </div>
                )}
              </div>
            )}
      ...
      <button onClick={()=>setShowAdd(true)}
        className="w-full py-3 border-2 border-dashed border-white/40 rounded-xl text-gray-500 text-sm font-bold bg-white/20 backdrop-blur-sm">
        + 新增目標
      </button>
...
function AnalysisTab({ logs, profile }) {
  ...
  return (
    <div className="flex flex-col">
      <div className="flex border-b border-white/20 sticky top-0 z-10" style={{ background:"rgba(255,255,255,0.5)",backdropFilter:"blur(8px)" }}>
        {[["trend","📈 趨勢"],["distance","🗺️ 距離"],["goals","🎯 目標"]].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)}
            className={`flex-1 py-2.5 text-xs font-bold border-b-2 transition-all
              ${subTab===id?"border-blue-500 text-blue-600":"border-transparent text-gray-600"}`}>{label}</button>
        ))}
      </div>
      <div className="p-4">
...
export default function MemberPractice() {
  ...
  const [form, setForm]=useState(()=>({
    date: today(),
    bowType: getDefaultBowType(profile?.equipment),
    distance: 18, targetFormat:"full_110",
    arrowCount:6, roundCount:6, note:"",
  }));
          ...
          {[["practice","🎯 練習"],["history","📋 歷史"],["analysis","📈 分析"]].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)}
              className={`flex-1 py-3 text-xs font-bold border-b-2 transition-all
                ${tab===id?"border-blue-500 text-blue-600":"border-transparent text-gray-600"}`}>{label}</button>
          ))}
        </div>
      )}
