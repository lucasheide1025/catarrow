...
export default function MemberPerformance({ profileOverride = null, coachView = false }) {
  ...
  const filterOptions = useMemo(() => ({
    ...
    // ⚠️ 月份選單原本**只有「深度分析」分頁裡有**，其他分頁只能選相對期間。
    //    而「月」＝**本月**，所以一跨月就整片空白，過去的資料完全看不到。
    //    篩選邏輯本來就支援 filters.month，缺的只是外層的 UI。（2026-08-04 作者回報）
    //    新的月份排前面（最近的最常看）。
    month:[{ value:ALL, label:"不限月份" },
      ...[...new Set(sourceSessions.map(monthKeyOf).filter(Boolean))].sort().reverse()
  ...
  return <div className="p-4 flex flex-col gap-4" style={{ minHeight:"100dvh", backgroundColor:"#0a0c14", backgroundImage:"linear-gradient(180deg, rgba(9,11,18,0.90), rgba(9,11,18,0.96)), url(/ui/page-bg.webp)", backgroundSize:"cover", backgroundPosition:"top center", backgroundAttachment:"local" }}>
    ...
    {tab === "overview" && <>
      ...
      <section><ST>近期趨勢</ST><div className="mb-2 flex gap-1 overflow-x-auto pb-1">{PERIOD_OPTIONS.map(([value, label]) => <button key={value} type="button" onClick={() => setFilters(current => ({ ...current, period:value }))} className={`shrink-0 rounded px-3 py-1.5 text-xs font-bold ${filters.period === value ? "bg-emerald-500 text-white" : "bg-white/10"}`} style={filters.period === value ? undefined : { color:"var(--text-secondary)" }}>{label}</button>)}<span className="shrink-0 self-center text-[10px]" style={{ color:"var(--text-secondary)" }}>或指定</span><FilterSelect value={filters.month || ALL} onChange={value => setFilters(current => ({ ...current, month:value }))} options={filterOptions.month} /></div><p className="mt-1 mb-3 text-[10px]" style={{ color:"var(--text-secondary)" }}>期間以場次完成時間分類；<b>指定月份後就不再套用相對期間</b>，可以直接翻看過去任何一個月。</p>
