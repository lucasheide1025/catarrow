...
export default function MemberHome({ onPageChange }) {
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      ...
      {/* ✅ 補傳 memberCreatedAt，修復新會員看到舊通知 + 修 HonorCelebration 空白 BUG */}
      <HonorCelebration
        memberId={profile?.id}
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-3 gap-3 mt-1">
          {CERT_SHOW.map(bk => {
            ...
            const frame = level
              ? (frameByLevel[level] || "bg-gray-50 border-gray-200")
      ...
      {recentResults.length > 0 && (
        <Card className="p-4">
          ...
          <button onClick={() => onPageChange("history")} className="text-blue-600 text-xs font-semibold mt-2">
            查看全部成績 →
          </button>
        </Card>
      )}
...
function ClaimBtn({ logId, memberId }) {
  ...
  return (
    <button onClick={claim} className="text-xs bg-green-600 text-white font-bold px-2 py-1 rounded-lg">
      確認領取
    </button>
  );
}
...
function DisputeBtn({ logId, memberId }) {
  ...
  if (show) return (
    <div className="flex gap-1">
      <input value={reason} onChange={e => setReason(e.target.value)}
        placeholder="說明原因" className="text-xs border border-gray-300 rounded px-2 py-1 w-24" />
  ...
  return (
    <button onClick={() => setShow(true)} className="text-xs text-red-400 font-medium">有誤？</button>
  );
}
