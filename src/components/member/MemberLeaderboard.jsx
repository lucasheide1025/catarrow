...
import { getMembers, getCompetitions, getAllCertRecords } from "../../lib/db";
...
export default function MemberLeaderboard() {
  ...
  useEffect(() => {
    Promise.all([getMembers(), getCompetitions(), getAllCertRecords()]).then(([ms, cs, certs]) => {
      setMembers(ms); setComps(cs);
  ...
  function calcEventPts(memberId) {
    let pts = 0;
    comps.forEach(c => {
      if (!c.results || c.results.length === 0) return;
      const sorted = [...c.results].sort((a, b) => b.total - a.total);
      const idx = sorted.findIndex(r => r.memberId === memberId);
      if (idx === 0) pts += 3;
      else if (idx === 1) pts += 2;
      else if (idx === 2) pts += 1;
    });
  ...
  // 檢定排行：今年度，該弓種每人最高分（讀 certRecords，後台改即同步）
  function certRanking(bowType) {
    const map = {};   // memberId -> total（取該年度上下半年的最高）
    certRecords
  ...
  function BadgeSummary({ member, type }) {
    if (type === "fatcat") {
      ...
      return <div className="flex gap-1 text-xs">
  ...
  const settledComps = comps.filter(c => c.results && c.results.length > 0);
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      ...
      {isCertTab ? (
        <Card className="p-4">
          {certList.length === 0 ? <Empty message="今年尚無已審核的檢定成績" /> :
            certList.map((m, i) => {
              ...
              return (
                <div key={m.memberId}
                  ...
                  <div className="w-8 text-center flex-shrink-0">
                    {["🥇","🥈","🥉"][i] ? <span className="text-2xl">{["🥇","🥈","🥉"][i]}</span> : <span className="text-gray-400 font-bold text-sm">{i+1}</span>}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {level && <span className={`inline-block mt-0.5 text-xs font-bold px-2 py-0.5 rounded-full ${certLevelStyle(level, "soft")}`}>{level} 級</span>}
                  </div>
          ...
          {ranked.map((m, i) => {
            ...
            return (
              <div key={m.id}
                className={`flex items-center gap-3 py-3 border-b border-gray-100 last:border-0 ${isMe ? "bg-blue-50 -mx-4 px-4 rounded-xl" : ""}`}>
                <div className="w-8 text-center flex-shrink-0">
                  {medal ? <span className="text-2xl">{medal}</span> : <span className="text-gray-400 font-bold text-sm">{i+1}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <div className={`font-bold text-sm ${isMe ? "text-blue-700" : "text-gray-800"}`}>
                    {m.nickname || m.name}{isMe && <span className="ml-1 text-xs text-blue-500">（我）</span>}
                  ...
                  {tab === "event" && <div className="text-gray-400 text-xs">賽事積分 {m.pts} 分</div>}
                </div>
                <div className="text-right flex-shrink-0">
                ...
                {sorted.map((r, i) => {
                  ...
                  return (
                    <div key={r.memberId}
                      ...
                      <span className="flex-1 text-sm font-medium">{r.nickname || r.name}{isMe && " (我)"}</span>
                      <span className="font-black text-lg">{r.total}</span>
                      <span className="text-xs text-gray-400 w-12 text-right">{i===0?"+3":i===1?"+2":i===2?"+1":""}</span>
                    </div>
                  );
                })}
              </Card>
