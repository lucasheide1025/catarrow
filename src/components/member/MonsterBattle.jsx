cat > /home/claude/catarrow/MonsterBattle_v4.jsx << 'ENDOFFILE'
...
import {
  MONSTERS, FAMILY_LABEL, TIER_LABEL,
  calcArcherStats, calcArcherPower, matchMonsters,
...
import { drawMaterials, getMaterialCount } from "../../lib/monsterMaterials";
...
@keyframes mb-star   { 0%,100%{transform:scale(1)} 50%{transform:scale(1.3)} }
`;
  ...
  return (
    <span className="flex gap-0.5">
      {Array.from({length:cfg.stars}).map((_,i)=>(
        <span key={i} className="text-xs" style={{ color:cfg.color }}>★</span>
      ))}
    </span>
  );
}
...
export default function MonsterBattle({ onBack, isGuest=false }) {
  ...
  // ✅ 六族匹配：依射手戰力決定，archerStats 算好後才產生
  ...
  useEffect(() => {
    if (isGuest) {
      // 訪客：預設戰力，固定出普通/稀有怪
      const stats = { hp:100, atk:10, def:10 };
      setArcherStats(stats);
      setMatchedMonsters(matchMonsters(stats));
      return;
    }
  ...
  function reroll() {
    ...
    setMatchedMonsters(matchMonsters(archerStats));
  ...
  async function submitRound() {
    ...
    for (let i=0;i<ARROWS_PER_ROUND;i++) {
      ...
      if (curMonHP<=0) {
        const ra=[...arrows];
        setAllArrows(prev=>[...prev,...ra]);
        setRoundScores(prev=>[...prev,{round,scores:ra,total:ra.reduce((s,v)=>s+v,0)}]);
      ...
      if ((i+1)%ARROWS_PER_COUNTER===0) {
        ...
        if (!skipCtr) {
          ...
          const counterTxt = isCrit?`${monster.icon} 爆擊！${monster.name} 猛烈反擊！受到 ${cdmg} 傷害`
    ...
    const ra=[...arrows];
    setAllArrows(prev=>[...prev,...ra]);
    setRoundScores(prev=>[...prev,{round,scores:ra,total:roundTotal}]);
  ...
  async function startBattle() {
    ...
    setMonsterHP(boosted.hp);
    setRound(1); setDistance(DISTANCE_START);
    setAllArrows([]); setRoundScores([]);
    setLog([
      {type:"system",text:`⚔️ 戰鬥開始！${FAMILY_LABEL[boosted.family]?.icon||""} ${FAMILY_LABEL[boosted.family]?.label||""}・${TIER_LABEL[boosted.tier]?.label||""} — ${boosted.name}`},
      {type:"system",text:`🎯 ${battleMode==="zombie"?"殭屍靶紙":"分數靶紙"}　${mode==="veteran"?`⚠️ 老手（HP:${boosted.hp} ATK:${boosted.atk} DEF:${boosted.def}）`:"🟢 新手"}`},
  ...
  async function endBattle(result, finalArchHP, finalMonHP) {
    if (result==="win") {
      ...
      setLoot(lootItem);
      const mats = drawMaterials(monster);
      ...
      if (profile?.id) {
        setRoundScores(rs=>{
          saveMonsterLog(profile.id,{
            monsterName:monster.name,monsterId:monster.id,
            result:"win",rounds:round,
            ...
            monsterTier:monster.tier,monsterFamily:monster.family,
          }).catch(()=>{});
          return rs;
        });
      }
      ...
      if (profile?.id) {
        setRoundScores(rs=>{
          saveMonsterLog(profile.id,{
            monsterName:monster.name,monsterId:monster.id,
            result:"lose",rounds:round,mode,battleMode,materials:[],roundScores:rs,
            monsterTier:monster.tier,monsterFamily:monster.family,
          }).catch(()=>{});
          return rs;
        });
        ...
        <div className="rounded-2xl p-4 text-white" style={{background:"linear-gradient(135deg,#7c3aed,#1e3a8a)"}}>
          <div className="flex justify-between items-start mb-3">
            <div>
              ...
              <div className="text-xl font-black">今日對手</div>
            </div>
            <div className="text-right">
              <div className="text-purple-200 text-xs mb-0.5">射手戰力</div>
              <div className="font-black text-2xl text-amber-300">{power}<span className="text-sm text-purple-200 font-normal">/100</span></div>
            </div>
          </div>
          {archerStats&&(
            <div className="flex gap-2 text-xs flex-wrap">
              ...
              {!isGuest&&dailyLeft!==null&&(
                <span className={`px-2 py-0.5 rounded-full font-bold ${dailyLeft>0?"bg-emerald-500/80":"bg-red-500/80"}`}>
                  今日剩 {dailyLeft}/{dailyMax} 次
                </span>
              )}
              {isGuest&&<span className="bg-amber-500/80 px-2 py-0.5 rounded-full font-bold">⭐ 體驗</span>}
            </div>
          )}
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              {matchedMonsters.map(m=>{
                ...
                const family = FAMILY_LABEL[m.family]||{};
                ...
                return (
                  <button key={m.id} onClick={()=>setPickedMonster(m)}
                    ...
                    style={{background:isPicked?tier.bg||"#ede9fe":"white",border:`2px solid ${isPicked?tier.color||"#7c3aed":"#e2e8f0"}`}}>
                    {/* 族群標籤 */}
                    ...
                    <div className="text-xs font-bold mt-0.5" style={{color:tier.color}}>【{tier.label}】</div>
            ...
            <button onClick={reroll}
              className="w-full py-2.5 rounded-xl text-sm font-bold border border-gray-200 bg-white text-gray-600 active:scale-95 transition-transform">
              🎲 重新配對（換一批怪物）
            </button>
    ...
    return (
      <div className="p-4 flex flex-col gap-4">
        ...
        <button onClick={()=>{setMode("novice");setPhase("prebattle");}}
          ...
          <div className="text-gray-500 text-sm">每2箭怪物反擊一次，傷害穩定。適合剛開始練習的射手。</div>
        </button>
        <button onClick={()=>{setMode("veteran");setPhase("prebattle");}}
          ...
          <div className="text-gray-500 text-sm">怪物數值 ×1.5，每回合需實際移動靶位，距離越近爆擊率越高。</div>
  ...
  if (phase==="prebattle") {
    ...
    const family     = FAMILY_LABEL[pickedMonster.family]||{};
    const tierCfg    = TIER_LABEL[pickedMonster.tier]||{};
    return (
      <div className="p-4 flex flex-col gap-4">
        <style>{BATTLE_CSS}</style>
        ...
        <div className="rounded-2xl p-6 text-white text-center" style={{background:"linear-gradient(135deg,#7c3aed,#1e3a8a)"}}>
          <div className="flex justify-center gap-2 mb-2">
            <span className="text-xs font-bold px-2 py-1 rounded-full" style={{background:family.color+"44",color:"white"}}>
              {family.icon} {family.label}
            </span>
            <span className="text-xs font-bold px-2 py-1 rounded-full" style={{background:tierCfg.color+"44",color:"white"}}>
              {tierCfg.label}
            </span>
          </div>
          ...
          {mode==="veteran"&&<div className="bg-orange-500/30 text-orange-200 text-xs font-bold px-3 py-1.5 rounded-full mb-3 inline-block">⚠️ 老手：數值增強 + 每回合縮短距離</div>}
          {archerStats&&(
  ...
  if (phase==="loot") {
    ...
    const family = monster?FAMILY_LABEL[monster.family]:{};
    const tierCfg= monster?TIER_LABEL[monster.tier]:{};
    return (
      <div className="p-4 flex flex-col gap-4 items-center">
        <style>{BATTLE_CSS}</style>
        <div className="text-center mt-4">
          <div className="text-xs font-bold mb-1" style={{color:family.color}}>{family.icon} {family.label}</div>
        ...
        {history.length===0?(
          ...
          <div className="flex flex-col gap-2">
            {history.map(h=>{
              ...
              const family = FAMILY_LABEL[h.monsterFamily||m?.family]||{};
              const tier   = TIER_LABEL[h.monsterTier||m?.tier]||{};
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className="font-bold text-gray-800 text-sm">{h.monsterName}</span>
                        </div>
                        <div className="text-gray-400 text-xs mt-0.5">
                          {h.mode==="veteran"?"老手":"新手"}·{h.battleMode==="zombie"?"殭屍":"分數"}　{h.rounds}回合
                          {tier.label&&<span className="ml-1" style={{color:tier.color}}>【{tier.label}】</span>}
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
