...
import { subscribeAdventurerProgress, completeGuildTask } from "../../lib/db";
import {
...
export default function AdventurerGuild({ onBack }) {
  ...
  const [view, setView]         = useState("list"); // "list" | "shoot"
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#0f172a" }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 pb-10">
        ...
        <div className="rounded-2xl p-5 relative overflow-hidden" style={{ background: rank.gradient }}>
          ...
          {isPromotionLevel(level) && (
            <div className="mt-3 rounded-xl px-3 py-2 border border-amber-400/40" style={{ background: "rgba(251,191,36,0.1)" }}>
              <div className="text-amber-300 font-black text-xs">⚔️ 晉階任務已解鎖！完成今日原野射箭挑戰可晉升下一階級</div>
            </div>
          )}
        </div>
