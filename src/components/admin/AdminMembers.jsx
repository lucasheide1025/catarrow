  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <Btn v="primary" size="sm" onClick={() => setAddModal(true)}>+ 新增會員</Btn>
      </div>
      ...
      {totalDisputes > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center gap-2">
          ...
          <span className="text-red-600 text-sm font-bold">
            有 {totalDisputes} 筆徽章被回報，請於下方有 🔴 記號的會員處理
          </span>
        </div>
      )}
      ...
      <Card className="p-4 flex flex-col gap-3">
        ...
        <Sel label="排序方式" value={sortBy} onChange={e => setSortBy(e.target.value)} options={SORT_OPTIONS} />
        <div className="text-gray-400 text-xs">共 {displayed.length} 位會員</div>
      </Card>
...
function MemberCard({ member: m, disputeList, onEdit, onBadge, onHistory, onCert, onCertExam, onDispute, onDelete }) {
  const lastLogin = m.lastLoginAt?.toDate?.() ? fmtDT(m.lastLoginAt) : "未登入";
  const equipSets = normalizeEquipment(m.equipment);
  const armorSets = (m.armorSets || []);
  const accessorySets = (m.accessorySets || []);
  const hasDispute = disputeList.length > 0;
  const fatPts = calcBadgePoints(m, "fatCat");
  const scrPts = calcBadgePoints(m, "score");
  const achPts = calcBadgePoints(m, "achievement");

  return (
    <Card className={`p-4 ${hasDispute ? "ring-2 ring-red-300" : ""}`}>
      <div className="flex justify-between mb-2">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            {hasDispute && (
              <button onClick={onDispute} title="有徽章被回報，點擊處理"
                className="text-red-500 text-sm animate-pulse">🔴</button>
            )}
            <span className="text-gray-800 font-black text-base">{m.name}</span>
            {m.nickname && <span className="text-gray-500 text-sm">「{m.nickname}」</span>}
            <span className="text-xs bg-blue-50 text-blue-600 font-bold px-2 py-0.5 rounded-full">
              {formatArcherNo(m.archerNo)}
            </span>
          </div>
          <div className="text-gray-400 text-xs mt-0.5">
            @{m.username} 射齡 {calcAge(m.joinDate)} 最近登入：{lastLogin}
          </div>
        </div>
        <button onClick={onDelete} className="text-red-300 hover:text-red-500 text-xs ml-2 flex-shrink-0">刪除</button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {[["肥貓分", fatPts, "text-orange-600", "bg-orange-50"],
          ["積分分", scrPts, "text-blue-600",   "bg-blue-50"],
          ["成就分", achPts, "text-purple-600", "bg-purple-50"]].map(([k,v,c,bg]) => (
      ...
      {/* 裝備標籤：弓組 + 防具 + 飾品 */}
      {(equipSets.length > 0 || armorSets.length > 0 || accessorySets.length > 0) && (
        <div className="flex gap-1 flex-wrap mb-3">
          {equipSets.map((set, i) => (
            <span key={i} className="text-xs bg-blue-50 text-blue-500 px-2 py-0.5 rounded-full">
              🏹 {set.label || set.bowCategory}
            </span>
          ))}
          {armorSets.map((set, i) => (
          ...
          {accessorySets.map((set, i) => (
            <span key={i} className="text-xs bg-purple-50 text-purple-500 px-2 py-0.5 rounded-full">
              ✨ {set.label || `飾品${i+1}`}
            </span>
          ))}
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        <Btn v="secondary" size="sm" onClick={onEdit}>✏️ 編輯資料</Btn>
        ...
        <Btn v="secondary" size="sm" onClick={onHistory}>📊 成績歷程</Btn>
        {hasDispute && (
          <Btn v="danger" size="sm" onClick={onDispute}>🔴 處理回報（{disputeList.length}）</Btn>
        )}
      </div>
    </Card>
  );
    ...
    <Modal open wide onClose={onClose} title="新增會員">
      <div className="flex flex-col gap-5">
        ...
        <Section title="🎒 裝備清單">
          ...
          {equipTab === "bow" && (
            <EquipmentEditor value={form.equipment}
              onChange={eq => setForm(p => ({ ...p, equipment: eq }))} />
          )}
          {equipTab === "armor" && (
            <ArmorManager value={form.armorSets}
              onChange={sets => setForm(p => ({ ...p, armorSets: sets }))} />
          )}
          {equipTab === "accessory" && (
            <AccessoryManager value={form.accessorySets}
              onChange={sets => setForm(p => ({ ...p, accessorySets: sets }))} />
          )}
        </Section>
...
function EditMemberModal({ member, onClose, onDone, operatorId, toast }) {
  ...
  return (
    <Modal open wide onClose={onClose} title={`編輯會員 — ${member.name}`}>
      <div className="flex flex-col gap-5">
        <Section title="👤 基本資料">
          <div className="grid grid-cols-2 gap-3">
            {f("name","姓名")}{f("nickname","暱稱")}
    ...
    <div className="flex gap-2 mb-3">
      {tabs.map(t => (
        <button key={t.key} onClick={() => onChange(t.key)}
          className={`px-3 py-1.5 rounded-full text-xs font-bold border ${active === t.key ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-200"}`}>
          {t.label}
        </button>
      ))}
    </div>
...
function BadgeModal({ member, onClose, onDone, operatorId, toast }) {
  ...
  return (
    <Modal open wide onClose={onClose} title={`徽章管理 — ${member.name}`}>
      <div className="flex flex-col gap-5">
        <div>
          <ST>目前持有</ST>
          <div className="flex flex-col gap-3">
            {[["🐱 肥貓章","fatCat",member.fatCat,["gold","silver","bronze"],["金","銀","銅"]],
              ...
              <div key={lbl}>
                <div className="text-gray-500 text-xs mb-1.5">{lbl}</div>
        ...
        <div className="bg-blue-50 rounded-xl p-4 flex flex-col gap-3">
          <ST>新增 / 發放徽章</ST>
          <p className="text-xs text-gray-500">新增後系統會自動計算升級（十銅換一銀、五銀換一金），並通知射手確認領取。</p>
          ...
          <Inp label="備註（原因）" value={adding.note} placeholder="例如：2025夏季積分賽第一名"
            onChange={e => setAdding(p=>({...p,note:e.target.value}))} />
          <Btn v="primary" className="w-full" onClick={save} disabled={saving}>{saving?"發放中…":"發放徽章"}</Btn>
        </div>
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col gap-3">
          <ST>⚙️ 直接修正數量</ST>
          <p className="text-xs text-amber-700">發錯時用這裡直接改正確數量。此操作會直接覆寫，不經升級換算。</p>
          {[["🐱 肥貓章","fatCat",["gold","silver","bronze"],["金","銀","銅"]],
            ["⭐ 積分章","score",["gold","silver","bronze"],["金","銀","銅"]],
            ["🏆 成就章","achievement",["black","gold","silver"],["黑","金","銀"]]].map(([lbl,type,keys,names])=>(
            <div key={type}>
              <div className="text-gray-500 text-xs mb-1.5">{lbl}</div>
              <div className="grid grid-cols-3 gap-2">
                {keys.map((k,i)=>(
                  <Inp key={k} label={names[i]} type="number" min="0"
                    value={counts[type][k]}
                    onChange={e=>setCount(type,k,e.target.value)} />
                ))}
              </div>
            </div>
          ))}
          <Btn v="secondary" className="w-full" onClick={saveFix} disabled={savingFix}>
            {savingFix?"修正中…":"💾 直接修正數量"}
          </Btn>
        </div>
        <Btn v="secondary" className="w-full" onClick={onClose}>關閉</Btn>
      </div>
    </Modal>
  );
}
...
function DisputeModal({ member, disputeList, onClose, onDone, operatorId, toast }) {
  ...
  async function resolveAll(applyCounts) {
    ...
    try {
      ...
      for (const log of disputeList) {
        await resolveBadgeDispute(log.id, operatorId, 0, note || (applyCounts ? "已修正數量" : "已確認無誤"));
      }
  ...
  return (
    <Modal open wide onClose={onClose} title={`處理徽章回報 — ${member.name}`}>
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          <ST>射手回報內容</ST>
          {disputeList.map(log => {
            const def = BADGE_DEF[log.badgeType] || {};
            return (
              <div key={log.id} className="bg-red-50 border border-red-200 rounded-xl p-3">
                <div className="text-gray-700 text-sm font-bold mb-1">
                  {def.label || log.badgeType}　<span className="text-gray-400 font-normal">發放時：{log.note || "（無備註）"}</span>
                </div>
                <div className="text-red-600 text-sm">回報原因：{log.disputeReason || "（射手未填寫原因）"}</div>
                <div className="text-gray-400 text-xs mt-1">回報時間：{fmtDT(log.disputedAt)}</div>
              </div>
            );
          })}
        </div>
          ...
          {Object.entries(BADGE_DEF).map(([type, def]) => (
            <div key={type} className={disputedTypes.includes(type) ? "" : "opacity-60"}>
              <div className="text-gray-500 text-xs mb-1.5">
                {def.label}{disputedTypes.includes(type) && <span className="text-red-500 ml-1">← 被回報</span>}
              </div>
              <div className="grid grid-cols-3 gap-2">
                {def.keys.map((k, i) => (
        ...
        <Inp label="處理備註（選填）" value={note} placeholder="例如：已與射手確認，補回 1 銀章"
          onChange={e => setNote(e.target.value)} />
        <div className="flex gap-2">
          <Btn v="secondary" className="flex-1" onClick={onClose}>取消</Btn>
          <Btn v="secondary" className="flex-1" onClick={() => resolveAll(false)} disabled={saving}>
            {saving ? "處理中…" : "確認無誤結案"}
          </Btn>
          <Btn v="primary" className="flex-1" onClick={() => resolveAll(true)} disabled={saving}>
            {saving ? "處理中…" : "修正並結案"}
          </Btn>
        </div>
      </div>
...
function HistoryModal({ member, onClose }) {
  ...
  return (
    <Modal open wide onClose={onClose} title={`成績歷程 — ${member.name}`}>
      <div className="flex flex-col gap-4">
            ...
            {logs.map(l => (
              <div key={l.id} className="flex items-center gap-3 py-2.5 border-b border-gray-100 last:border-0">
                <div className="flex-1">
                  <div className="text-gray-700 text-sm font-medium">{l.compTitle || "—"}</div>
...
function AuditSection({ targetId }) {
  ...
  useEffect(() => {
    if (!show) return;
    getAuditLogs(targetId).then(setLogs);
  }, [show, targetId]);
  return (
    <div>
...
function CertModal({ member, onClose, onDone, operatorId, toast }) {
  ...
  async function save() {
    ...
    try {
      for (const [bowType, scoreStr] of Object.entries(editing)) {
        ...
        if (score <= 0) {
          await deleteCertRecord(member.id, Number(year), half, bowType, operatorId);
        } else {
          await upsertCertRecord(member.id, Number(year), half, bowType, score, operatorId);
        }
      }
      toast("檢定成績已儲存 ✓");
  ...
  return (
    <Modal open wide onClose={onClose} title={`年度檢定 — ${member.name}`}>
      <div className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-amber-700 text-xs">
          後台為最高權限：填入分數直接覆寫；填 0 會刪除該弓種檢定；留空則不變動。
        </div>
        <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">
                      {score > 0 ? `目前成績：${score}分 階級：${level || "未通過"}` : "尚未參加"}
                    </div>
                  </div>
                  {score > 0 && level && (
                    <span className="text-xs font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{level}</span>
                  )}
                </div>
                <Inp label={`${year}年成績（留空=不變動）`} type="number" min="0"
        ...
        <div className="flex gap-2">
          <Btn v="secondary" className="flex-1" onClick={onClose}>取消</Btn>
          <Btn v="primary" className="flex-1" onClick={save} disabled={saving}>{saving ? "儲存中…" : "儲存成績"}</Btn>
        </div>
      </div>
    </Modal>
  );
