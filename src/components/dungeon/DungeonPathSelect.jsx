...
export default function DungeonPathSelect({ roomId, room, isHost }) {
  ...
  return (
    <DungeonEventStage tone="path">
      {/* Header */}
      <div className="dungeon-stage-header shrink-0 text-center py-6 border-b border-white/10">
      ...
      {/* Path options */}
      <div className="dungeon-stage-main px-4 pt-8 pb-[calc(7rem+env(safe-area-inset-bottom))] flex flex-col justify-center gap-4">
        {["A","B"].map(key => {
          ...
          return (
            <SpotlightCard as="button" key={key} onClick={() => handleSelect(key)}
