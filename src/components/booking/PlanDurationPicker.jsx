...
import { BOOKING_DURATIONS, bookingTotalPrice } from "../../lib/bookingPricing";

// 並排小卡樣式（使用者反映原本的直向長條列表很難看）——每張卡片顯示方案名稱+金額，
// 選中要有明顯的視覺區別（邊框+底色），比照 BillingSystem.jsx/CheckoutModal 既有的卡片語言。
export default function PlanDurationPicker({ planType, durationHours, participantBreakdown, onChange }) {
  if (participantBreakdown) {
    return (
      <div>
        <div className="text-slate-400 text-xs font-bold mb-1.5">時數</div>
        <div className="grid grid-cols-3 gap-2">
          {BOOKING_DURATIONS.map(duration => {
            const active = duration.value === durationHours;
            return (
              <button key={duration.value} type="button"
                onClick={() => onChange({ durationHours: duration.value })}
                ...
                <span className={`text-xs font-bold ${active ? "text-blue-300" : "text-slate-200"}`}>{duration.label}</span>
                <span className={`text-sm font-black ${active ? "text-blue-300" : "text-slate-400"}`}>
                  NT$ {bookingTotalPrice(participantBreakdown, duration.value)}
                </span>
              </button>
            );
          })}
