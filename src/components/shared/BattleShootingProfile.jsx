...
export default function BattleShootingProfile({ memberId, onChange, showDistance = true }) {
  const { profile: authProfile } = useAuth();
  const [shootingProfile, setShootingProfile] = useState(() => loadBattleShootingProfile(memberId));

  function update(patch) {
    const next = saveBattleShootingProfile(memberId, patch);
    setShootingProfile(next);
    onChange?.(next);
  }
  ...
  return (
    <div className="rounded-xl border border-blue-400/20 bg-blue-500/10 p-3">
      ...
      <div className={`grid gap-2 ${showDistance ? "grid-cols-2" : "grid-cols-1"}`}>
        ...
        {showDistance && <label className="text-[10px] font-bold text-white/50">
          實際距離
