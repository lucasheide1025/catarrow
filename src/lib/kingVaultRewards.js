...
export function rollKingVaultReward(dungeonTier = 1, family = "treasure") {
  ...
  const pool = EXPANSION_MATERIALS.filter(material => material.family === family && material.tierIndex === tier);
  const choices = pool.length ? pool : EXPANSION_MATERIALS.filter(material => material.tierIndex === tier);
