...
function attackDamageVariance(rand = Math.random) {
  return 0.9 + rand() * 0.2;
}

export function calcStandardArrowDmg(score, atk, def, partMult = 1, label = null, rand = Math.random) {
  const scoreMult = scoreDamageMultiplier(label, score);
  if (scoreMult === 0 || partMult === 0) return 0;
  const base = 8 + (atk || 10) * 0.7 - (def || 0) * 0.35;
  return Math.max(1, Math.round(base * scoreMult * partMult * attackDamageVariance(rand)));
}
