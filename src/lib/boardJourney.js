...
import { BOARD_MODES, MAX_CAMP_MULT, MAX_SHOOT_MULT, MAX_CATMATE_STACKS, MAX_DICE_COUNT } from "./boardData";

// ── 7 張地圖（沿用採集點＝建築 id）────────────────────────
export const JOURNEY_MAP_IDS = BOARD_MODES.map(m => m.id);
export const JOURNEY_MAP_META = Object.fromEntries(BOARD_MODES.map(m => [m.id, m]));
// 每日補滿（全地圖共用）。⚠️ 與 villageBoardDb.js 的 DAILY_DICE 同值——
// 這裡不 import 它是因為 villageBoardDb 綁 firebase（純模組會拖進依賴）；改值時兩處要同步。
export const JOURNEY_DAILY_DICE = 15;
...
export const JOURNEY_SHOOTING_TILES = new Set(["monster", "boss"]);
...
export const BASE_TILE_WEIGHTS = {
  material: 18, mining: 10, monster: 8, arrowdew: 7, coins: 8, gacha: 3,
  potion: 3, chest: 6, catbond: 4, fate: 4, opp: 4,
  camp: 4, empower: 3, catmate: 3, trap: 3, shortcut: 2, market: 2, scenery: 8,
  fork: 3,   // 分岔路口：玩家二選一（穩妥素材路／冒險怪物路）
  cardgacha: 2,   // 🃏 抽卡房（08-08）：稀有格，踩到開抽卡（免費 1 張／付費 3 張）
};
const MODE_WEIGHT_TWEAKS = {
  mine:      { mining: 16 },                    // 星屑礦坑：礦多
  farm:      { material: 22, scenery: 6 },      // 月芽農田：物產豐饒
  harbor:    { potion: 5, arrowdew: 9 },        // 霧潮港口：水氣與藥水
  hunting:   { monster: 11, trap: 4 },          // 巡林狩獵場：獵物與陷阱
  market:    { coins: 12, gacha: 4, market: 3 },// 喧鬧市集：金幣與轉蛋
  warehouse: { chest: 9, material: 16 },        // 古罐倉庫：藏寶多
  archery:   { arrowdew: 10 },                  // 藏金靶場：箭露豐沛
};
export function tileWeights(modeId) {
  return { ...BASE_TILE_WEIGHTS, ...(MODE_WEIGHT_TWEAKS[modeId] || {}) };
}
...
export function rollTileType(rnd, modeId) {
  ...
  let r = rnd() * total;
  for (const [type, v] of entries) { r -= v; if (r <= 0) return type; }
  return entries[entries.length - 1][0];
}

// 分岔路口：從 from 之後找前方最近的目標格（跳過去）。
// 找不到就回傳 null（呼叫端退回固定步數）。maxLookahead 上限避免跑到終點外。
export function findNextTile(cells, from, targetTypes, maxLookahead = 15) {
...
export function windingPath(length, rnd) {
  const path = [];
  let x = 0, row = 0, remaining = length;
  while (remaining > 0) {
    ...
    for (let k = 0; k < segLen; k += 1) { path.push({ x, y: rowY + jitter }); x += 1; }
    remaining -= segLen;
  }
  return path;
}
...
export function nextPos(pos = 0, roll = 0, length = 0) {
  return Math.max(0, Math.min(Math.max(0, length - 1), pos + roll));
}
export function applyTrapPos(pos = 0, length = 0, back = 0) {
  return Math.max(0, Math.min(Math.max(0, length - 1), pos - back));
}
export function applyShortcutPos(pos = 0, length = 0, ahead = 0) {
...
export function rollDice() { return 1 + Math.floor(Math.random() * 15); }
// 多骰（強化格 diceCount buff）：一次擲 count 顆骰子，回傳每顆與總和（單人/組隊共用）。
// count 夾在 1~MAX_DICE_COUNT（防呆）；單顆＝既有 rollDice 行為。
export function rollJourneyDice(count = 1) {
  const n = Math.min(MAX_DICE_COUNT, Math.max(1, Number(count) || 1));
  const rolls = Array.from({ length: n }, () => rollDice());
  return { rolls, total: rolls.reduce((a, b) => a + b, 0) };
}
export function randomSeed() { return 1 + Math.floor(Math.random() * 0x7ffffffe); }
...
export function mergeBuffs(existing = {}, reward = {}) {
  const out = { ...existing };
  const b = reward.buffs || {};
  if (b.campMult != null) out.campMult = round2(Math.min(MAX_CAMP_MULT, (Number(out.campMult) || 1) * b.campMult));
  if (b.catmate === true) out.catmate = Math.min(MAX_CATMATE_STACKS, (Number(out.catmate) || 0) + 1);
  if (reward.nextShootMult != null) out.nextShootMult = Math.min(MAX_SHOOT_MULT, (Number(out.nextShootMult) || 1) * reward.nextShootMult);
  // 多骰：跟其他加成一樣「已有就疊加」（08-07 玩家要求）——骰子數相加、上限 MAX_DICE_COUNT。
  // 例：已有 2 骰再抽到 2 骰 → 下一次擲 4 顆；抽到 3 骰 → 下一次擲 5 顆但夾到 4。
  if (reward.diceCount != null) out.diceCount = Math.min(MAX_DICE_COUNT, (Number(out.diceCount) || 0) + (Number(reward.diceCount) || 0));
  return out;
}

// 套用旅程 buff 倍率：shootMult（強化，含金幣/箭露/貓XP/資源）、
// campMult（營地，只乘村資源）。返回值為新的 reward（不原地修改）。
export function applyJourneyMultipliers(reward, { shootMult = 1, campMult = 1 } = {}) {
...
export function combineRewards(a = {}, b = {}) {
  const out = {
    ...
    villageResources: { ...(a.villageResources || {}) },
    familyMaterials: { ...(a.familyMaterials || {}) },
...
export function normalizeVillageBoard(vb = {}) {
