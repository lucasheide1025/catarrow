import { CAT_BATTLE_MODE_POLICIES, getCatBattleArchetype, getCatBondScaling, getCatStrongSkillChance } from "./catBattleArchetypes";

export function createCatBattleState(){return{strongSkillMisses:0,personalShield:0,guardAtkBuff:null,deathGuardReady:false,deathGuardUsed:false,combo:0,pulse:0,catStatuses:[]};}

const scoreValue=s=>String(s).toUpperCase()==="X"?10:Math.max(0,Number(s)||0);
const clamp=(n,min,max)=>Math.max(min,Math.min(max,n));

export function getCatGuardAtkBonus(state,round){
  const buff=state?.guardAtkBuff;
  return buff&&Number(buff.expiresAfterRound)>=Number(round)?Math.max(0,Number(buff.value)||0):0;
}

export function describeCatOutcome(outcome={}){
  const parts=[];
...
export function recordCatShieldAbsorption(state,input={}){
  const cat=getCatBattleArchetype(input.catId);
  const absorbed=Math.max(0,Number(input.absorbed)||0);
  if(cat.type!=="defense"||absorbed<=0)return state||createCatBattleState();
  const bond=getCatBondScaling(input.bondLevel);
  const catAtk=Math.max(1,Number(input.catAtk)||1);
  const conversion=input.catId==="diandian"?.45:input.catId==="xiaoan"?.2:.3;
  return{...createCatBattleState(),...(state||{}),guardAtkBuff:{
    value:Math.round(clamp(absorbed*conversion*bond.powerMultiplier,1,catAtk*.8)),
    expiresAfterRound:(Number(input.round)||1)+1,
  }};
}

export function consumeCatDeathGuard(state,input={}){
  const current={...createCatBattleState(),...(state||{})};
  const policy=CAT_BATTLE_MODE_POLICIES[input.mode]||CAT_BATTLE_MODE_POLICIES.normal;
  if(input.catId!=="xiaoan"||!policy.allowDeathGuard||!current.deathGuardReady||current.deathGuardUsed)return{triggered:false,state:current,hp:0};
  return{triggered:true,state:{...current,deathGuardReady:false,deathGuardUsed:true},hp:Math.max(1,Math.round((Number(input.maxHp)||1)*.15))};
}

export function resolveCatRound(input={}){
  const cat=getCatBattleArchetype(input.catId);
  const bond=getCatBondScaling(input.bondLevel);
  const policy=CAT_BATTLE_MODE_POLICIES[input.mode]||CAT_BATTLE_MODE_POLICIES.normal;
  const state={...createCatBattleState(),...(input.state||{})};
  const events=[];
  const random=typeof input.random==="function"?input.random:Math.random;
  const strongTriggered=state.strongSkillMisses>=3||random()<getCatStrongSkillChance(input.catId,input.bondLevel);
  const next={...state,strongSkillMisses:strongTriggered?0:state.strongSkillMisses+1};
  if(next.guardAtkBuff&&Number(next.guardAtkBuff.expiresAfterRound)<=Number(input.round||1)){
    events.push({kind:"guard_atk_expired",name:"守護反攻已消耗"});
  ...
  let monsterDamage=0,playerHeal=0,playerShield=0,teamShield=0,teamHeal=0,teamCleanseCount=0,playerDefBonusPct=0,monsterStatus=null;
  ...
  if(cat.type==="heal"){
    ...
    next.catStatuses=activeStatuses.map(status=>({...status,rounds:status.rounds-1})).filter(status=>status.rounds>0);
    ...
    next.catStatuses=[...next.catStatuses.filter(status=>status.id!==cat.status.id),{id:cat.status.id,name:cat.status.name,damage:appliedDamage,rounds:duration}];
    if(statusTickDamage>0)events.push({kind:"cat_status_tick",name:"貓咪異常持續傷害",damage:statusTickDamage});
    events.push({kind:"monster_status",statusId:cat.status.id,name:cat.status.name,damage:appliedDamage,duration});
    if(strongTriggered){
      playerHeal+=Math.round(baseHeal*.8);
      teamHeal=Math.max(1,Math.round(baseHeal*.3));
      if(input.catId==="gege")teamCleanseCount=1;
      if(input.catId==="gege")monsterStatus={id:"weaken",name:"弱化",icon:"🫥",kind:"statDown",strength:Math.min(25,10+Math.floor(bond.level/5)),duration:2};
      if(input.catId==="meimei"){
        ...
        next.catStatuses=[];
        events.push({kind:"cat_status_detonation",name:"脈衝引爆",damage:Math.round(detonation*.6)});
      }
      events.push({kind:"strong_skill",name:cat.strongSkill.name,monsterStatus});
    }
  }else if(cat.type==="attack"){
    const scores=(input.scores||[]).map(scoreValue);
    const accurate=scores.filter(s=>s>=9).length;
    let mult=.8,hitCount=1;
    if(input.catId==="niuniu"){
      mult+=accurate*.12;
      if(strongTriggered){const precisionPierce=1+Math.min(.35,.12+accurate*.05);mult*=precisionPierce;monsterStatus={id:"defBreak",name:"破防",icon:"🔨",kind:"statDown",strength:Math.min(25,10+accurate*3+Math.floor(bond.level/10)),duration:2};}
    }
    if(input.catId==="haji"){
      next.combo=scores.every(s=>s>0)?Math.min(6,(state.combo||0)+1):0;
      mult+=next.combo*.08;
      if(strongTriggered){hitCount=Math.min(4,2+Math.floor(next.combo/2));mult*=1+.22*hitCount;monsterStatus={id:"defBreak",name:"破防",icon:"🔨",kind:"statDown",strength:Math.min(18,6+next.combo*2),duration:2};}
    }
    ...
    events.push({kind:"cat_attack",damage:monsterDamage,hitCount,name:strongTriggered?cat.strongSkill.name:cat.passive.name});
    if(monsterStatus)events.push({kind:"monster_status",...monsterStatus});
  }else{
    ...
    if(absorbed>0){
      next.guardAtkBuff=recordCatShieldAbsorption(next,{catId:input.catId,bondLevel:input.bondLevel,catAtk,absorbed,round:input.round}).guardAtkBuff;
    }
    events.push({kind:"player_shield",amount:playerShield,name:cat.passive.name});
    if(strongTriggered){
      ...
      events.push({kind:"strong_skill",name:cat.strongSkill.name,blockScale:policy.blockScale,teamShield,deathGuardReady:next.deathGuardReady});
    }
  }
  return{state:next,events,strongTriggered,monsterDamage,playerHeal,playerShield,teamShield,teamHeal,teamCleanseCount,playerDefBonusPct,monsterStatus};
}

export function deterministicCatRoll(...parts){
...
export function resolveAuthoritativeCatRound({member={},monster={},round=1,mode="normal",state=null,random=null,battleId="",memberId=""}={}){
  const stableRandom=typeof random==="function"
    ? random
    : ()=>deterministicCatRoll(battleId,memberId,member.catId,round,"strong-skill");
  return resolveCatRound({
    catId:member.catId,catLevel:member.catLevel||1,bondLevel:member.catBond||0,
    catAtk:member.catAtk||member.catATK||0,catMaxHp:member.catMaxHP||200,
    companionAttackPct:member.catModifiers?.companionAttackPct||0,companionHealingPct:member.catModifiers?.companionHealingPct||0,
    playerHp:member.hp||0,playerMaxHp:member.maxHP||member.hp||1,
    monsterHp:monster.currentHp??monster.hp,monsterMaxHp:monster.maxHp??monster.hp,
    monsterBossTagged:!!(monster.bossTagged||["boss","mythic"].includes(monster.tier)),
    round,scores:(member.arrows||[]).map(a=>a?.label??a?.score??a),mode,state:state||member.catBattleState||createCatBattleState(),random:stableRandom,
  });
}
