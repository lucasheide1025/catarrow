...
const FAMILY_CARD_META = {
  ...
  treasure:{typeLabel:"寶箱族王",rarity:"ANCIENT",frame:"#fbbf24"},
};

const V2_EFFECTS = {
  head_coach: [{ kind:"damage_pct", pct:.12 }, { kind:"armor_pierce_pct", pct:.15 }],
  wife: [{ kind:"damage_pct", pct:.10 }, { kind:"damage_reduce_pct", pct:.10 }],
  yumi: [{ kind:"damage_pct", pct:.10 }, { kind:"burn", atkPct:.20, duration:3 }],
  cat_daming: [{ kind:"heal_pct", pct:.12 }],
  cat_gege: [{ kind:"damage_pct", pct:.06 }, { kind:"heal_pct", pct:.06 }],
  cat_meimei: [{ kind:"damage_pct", pct:.08 }, { kind:"heal_pct", pct:.03 }],
  cat_niuniu: [{ kind:"boss_damage_pct", pct:.10 }],
  cat_haji: [{ kind:"damage_pct", pct:.10 }],
  cat_baobao: [{ kind:"damage_pct", pct:.07 }, { kind:"damage_reduce_pct", pct:.04 }],
  cat_youyou: [{ kind:"damage_reduce_pct", pct:.10 }],
  cat_xiaoan: [{ kind:"damage_pct", pct:.05 }, { kind:"damage_reduce_pct", pct:.07 }],
  cat_diandian: [{ kind:"damage_reduce_pct", pct:.06 }, { kind:"heal_pct", pct:.06 }],
};

for (const family of ["ghost", "forest", "poison", "office", "exam", "western","treasure"]) {
  V2_EFFECTS[`${family}_boss_small`] = [{ kind:"family_damage_pct", family:WB_FAMILY_TO_DUNGEON_FAMILY[family], pct:.12 }];
  V2_EFFECTS[`${family}_boss`] = [
    { kind:"family_damage_pct", family:WB_FAMILY_TO_DUNGEON_FAMILY[family], pct:.08 },
    { kind:"family_reduce_pct", family:WB_FAMILY_TO_DUNGEON_FAMILY[family], pct:.08 },
  ];
}

const pct = value => `${Math.round(Math.abs(value) * 100)}%`;
const FAMILY_LABEL = { ghost:"鬼怪", mountain:"山林", insect:"昆蟲", workplace:"職場", exam:"考試", temple:"神殿" };
export function describeWorldBossEffect(effect) {
  if (!effect) return "";
...
export function resolveWorldBossCardEffects({ equippedCardKeys = [], enemyFamily = null, enemyClass = null } = {}) {
  const modifiers = { damagePct:0, damageReducePct:0, healPct:0, armorPiercePct:0, burn:null };
  let generalDamagePct = 0;
  let scopedDamagePct = 0;
  const activeEffects = [], inactiveEffects = [];
  [...new Set(equippedCardKeys)].forEach(key => {
    (V2_EFFECTS[key] || []).forEach(effect => {
      const active = effect.kind === "family_damage_pct" || effect.kind === "family_reduce_pct"
        ? effect.family === enemyFamily
        : effect.kind === "boss_damage_pct" ? enemyClass === "boss" : true;
      (active ? activeEffects : inactiveEffects).push({ key, ...effect });
      if (!active) return;
      if (effect.kind === "damage_pct") generalDamagePct += effect.pct;
      else if (effect.kind === "family_damage_pct" || effect.kind === "boss_damage_pct") scopedDamagePct += effect.pct;
      else if (effect.kind === "damage_reduce_pct" || effect.kind === "family_reduce_pct") modifiers.damageReducePct += effect.pct;
      else if (effect.kind === "heal_pct") modifiers.healPct += effect.pct;
      else if (effect.kind === "armor_pierce_pct") modifiers.armorPiercePct += effect.pct * 100;
      else if (effect.kind === "burn") modifiers.burn = { chancePct:100, strengthPct:effect.atkPct * 100, duration:effect.duration };
    });
  });
  ...
  return { modifiers, activeEffects, inactiveEffects };
}
...
function buildWbCard(bossKey) {
  ...
  return {
    bossKey, name: boss.name, icon: boss.family === "coach" ? "👑" : boss.family === "cat" ? "🐱" : "🗡️", title: boss.title,
    flavor: boss.desc, family: boss.family, statMode: "passive", stat: null,
    ...makeCardMeta(bossKey, boss), version:2, effects:V2_EFFECTS[bossKey] || [],
  };
}
...
export const WB_CARDS = Object.keys(WORLD_BOSSES).reduce((acc, key) => {
  ...
  if (card) acc[key] = { ...card, effectText:(card.effects || []).map(describeWorldBossEffect).join("｜") };
  return acc;
}, {});

export const WB_CARD_KEYS = Object.keys(WB_CARDS);

export function buildWorldBossCardSnapshot(collection = {}) {
  return {
    effectVersion:2,
    equippedCardKeys:(collection.equipped || [])
