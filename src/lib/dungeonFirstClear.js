...
export const DUNGEON_DIFFICULTY_TIERS = { normal:1, advanced:3, hard:4, hell:5 };

export function normalizeDungeonDifficultyTier(value) {
...
export function buildDungeonFirstClearKey(family, difficultyTier) {
...
export function getDungeonFirstClearState(profile, dungeon) {
  const key = buildDungeonFirstClearKey(dungeon?.family, dungeon?.difficulty);
  if (!key) return { key:null, eligible:false, completed:false, known:true, reason:"此地下城不列入六族首次通關" };
  ...
  if (!profile) return { key, eligible:true, completed:false, known:false, reason:"首次通關資料讀取中" };
...
export function normalizeWorldFirstClearInput({ family, difficultyTier, hostId, hostName, teamMemberIds = [], teamNames = [], runId }) {
