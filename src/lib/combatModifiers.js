...
import { getSpecializationEffect } from "./equipmentSpecializationEngine";

const num = (v, d = 0) => (Number.isFinite(Number(v)) ? Number(v) : d);
const pct = v => Math.max(0, num(v)) / 100;

/** 受擊減傷的總上限——多來源疊加不能讓玩家無敵 */
export const MAX_DAMAGE_REDUCTION_PCT = 80;
/** 卡片「連擊」觸發時的爆擊倍率（跟 BattleScreen 一致） */
export const TALENT_CRIT_MULT = 1.3;
/** 幾環以上算「高品質命中」（精準／精研都吃這條線） */
export const HIGH_QUALITY_SCORE = 8;

function specValue(slot, trackId, key) {
  if (!slot || slot.trackId !== trackId) return 0;
  try { return num(getSpecializationEffect(trackId, slot.level)[key]); } catch { return 0; }
}
 ...
 */
export function buildCombatModifiers({ cardFx = null, equipSpec = null } = {}) {
  const fx = cardFx || {};
  ...
  return {
    ...
    // ── 狀態抗性 ──
    statusStrengthReductionPct:
      specValue(a, "immunity", "statusStrengthReductionPct") + num(fx.statusStrengthReductionPct),
    statusDurationReduction:
      specValue(a, "immunity", "statusDurationReduction") + num(fx.statusDurationReduction),
...
export function applyBattleStart({ playerMaxHp = 0, monsterAtk = 0, monsterDef = 0, mods }) {
 ...
 */
export function effectiveDefense(monsterDef, mods) {
  const m = mods || buildCombatModifiers();
  return Math.max(0, num(monsterDef)
    * (1 - pct(m.defIgnoreSpecPct))
    * (1 - pct(m.defIgnoreCardPct)));
}
...
export function applyOutgoing({
  ...
  return { damage: Math.round(dmg), crit, highQuality };
}

/** 受擊：堅韌／守護／堅盾，合計有上限 */
export function applyIncoming({ damage, currentHp = 1, maxHp = 1, mods }) {
  const m = mods || buildCombatModifiers();
 ...
 */
export function applyStatusResist(status, mods) {
  if (!status) return null;
  const m = mods || buildCombatModifiers();
  const strengthScale = Math.max(0, 1 - pct(m.statusStrengthReductionPct));
  let strength = typeof status.strength === "number"
    ? Math.round(status.strength * strengthScale * 10) / 10
    : status.strength;
...
export function applyRoundEnd({ currentHp = 0, maxHp = 1, mods, alive = true }) {
  const m = mods || buildCombatModifiers();
  if (!alive || m.endRoundHeal <= 0) return { hp: num(currentHp), healed: 0 };
  const hp = Math.min(num(maxHp), num(currentHp) + m.endRoundHeal);
  return { hp, healed: hp - num(currentHp) };
}

/** 反彈：對方打你多少，彈回去多少（有上限，見呼叫端） */
export function reflectDamage(incomingDamage, mods) {
  const m = mods || buildCombatModifiers();
...
/** 貓貓：支援專精放大陪練的攻擊與治療 */
export function applyCompanion({ attack = 0, healing = 0, mods }) {
  const m = mods || buildCombatModifiers();
  return {
...
export function describeModifiers(mods) {
  ...
  const add = (icon, label, value, unit = "%") => {
    if (value > 0) rows.push({ icon, label, text: `${label} +${Math.round(value * 10) / 10}${unit}` });
  };
