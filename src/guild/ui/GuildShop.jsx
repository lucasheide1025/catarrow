...
function itemView(item) {
  if (item.kind === "supply") {
    return { icon: item.icon, name: item.name, color: "#fde68a", note: `${item.qty} 份・購入後存入公會倉庫` };
  }
  if (item.kind === "equip") {
...
export default function GuildShop({ profile, memberCoins = 0, onBuy, onClose }) {
  const [msg, setMsg] = useState("");
  const [coinBalance, setCoinBalance] = useState(memberCoins);
  const [section, setSection] = useState("supply");
  const [matFamily, setMatFamily] = useState("ghost");
  const [matTier, setMatTier] = useState(1);
  const { shopTier, rank } = rankUnlocks(profile);
  useEffect(() => { setCoinBalance(memberCoins); }, [memberCoins]);
  ...
  const buy = async item => {
    ...
    if (res.ok && res.coinsSpent) setCoinBalance(v => Math.max(0, v - res.coinsSpent));
      ...
      {section !== "material" && [1, 2, 3].map(tier => {
        ...
        return (
          <div key={tier} style={{ ...card, opacity: locked ? 0.55 : 1 }}>
            ...
            {items.map(item => {
              const v = itemView(item);
              const afford = item.kind === "supply" ? coinBalance >= item.costCoins : profile.catCoins >= item.costCat;
              return (
                <div key={item.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
                  <span style={{ flex: 1, fontSize: 12, minWidth: 0 }}>
                    {v.icon} <b style={{ color: v.color }}>{v.name}</b>
