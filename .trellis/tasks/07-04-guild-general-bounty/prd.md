...
5. ✅ 每天從範本池抽選數量：**每難度固定抽 1 個，共 4 個**上架
6. ✅ 範本池數量不足時：**允許重複抽取**（同一範本可能連續上架）
7. ✅ 舊任務隔天新一批上架後：**直接下架失效**（不給補做寬限期，比照 `getDailyGuildTasks` 精神）
8. ✅ 各難度實際獎勵數字（金幣/經驗/箭露/轉蛋幣/寶箱）：**先用合理預設值上線**，之後教練再進後台調整。寶箱固定對應難度（例如 難度1→木寶箱、2→鐵寶箱、3→黃金寶箱、4→史詩/更高階寶箱），不做機率骰或保底掉落的額外設計，維持簡單。
