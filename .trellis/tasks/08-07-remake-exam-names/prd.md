...
沿用鬼怪族／山林族已驗證的獨立身份規則，重製考試族全部 36 隻怪物的顯示名稱、招牌技能名稱（含 `signatureSummary` 前綴）、掉落素材名稱，以及**卡片系統美術**（卡圖＋戰鬥立繪）。名稱與美術一併重設計，兩者共用同一套世界語言。

## Confirmed Facts

- 正式資料有 36 個 stable IDs：6 階 × 每階 6 隻（3 普通、2 小王、1 大王）；legacy 6 隻為 `exam_1~6`。
...
- 世界語言（取自 `07-22-local-comfyui-image-generation` 核准方向）：**校園與考試**——試卷、書卷、筆墨、學術制度；保留既有現代生活題材方向。
- 現行技能名多為考場梗直譯（如「筆尖突刺」「臨時抽考」「全科突襲」），可保留考場語感但需與新角色身份對齊。
...
- 36 個彼此獨立的怪物身分，不沿用跨階角色成長；同族跨 Tier 只共享校園考試世界語言。
