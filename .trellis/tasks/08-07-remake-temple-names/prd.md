...
沿用鬼怪族／山林族已驗證的獨立身份規則，重製西方怪物族全部 36 隻怪物的顯示名稱、招牌技能名稱（含 `signatureSummary` 前綴）、掉落素材名稱，以及**卡片系統美術**（卡圖＋戰鬥立繪）。名稱與美術一併重設計，兩者共用同一套世界語言。

## Confirmed Facts

- 正式資料有 36 個 stable IDs：6 階 × 每階 6 隻（3 普通、2 小王、1 大王）；legacy 6 隻為 `temple_1~6`。
...
- 世界語言：**西方奇幻**——吸血鬼、狼人、龍、騎士、神話生物與古城堡氛圍（族名「temple／西方怪物族」代表西方奇幻陣營）。
- 現行技能名多為西幻直譯（如「蘑菇彈跳」「月光劍舞」「幼龍翔擊」），可保留西幻語感但需與新角色身份對齊。
...
- 36 個彼此獨立的怪物身分，不沿用跨階角色成長；同族跨 Tier 只共享西方奇幻世界語言。
