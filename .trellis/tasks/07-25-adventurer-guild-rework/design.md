...
推進途中觸發**多重事件**（遭遇/陷阱/天候/休息點…）各自消耗食/水，有些消耗大
   → 食或水歸零 → 減益升級 → 完全耗盡 → **強迫撤退（委託失敗）** → 備包策略：帶多少糧/水/裝備要抓好（受負重限制）
全 wave 通過 → 凱旋回報 settleExpedition：聲望 + 材料(打怪/貓村通用) + 雜貨(帶回評估→金幣+CAT幣) + 公會裝掉落機率
```
...
## 7. 生存/補給事件（推進過程）
- 推進中觸發**多重事件**：戰鬥、陷阱、惡劣天候、遭遇、休息點…每種各自消耗**食物/水**（有些消耗大）。
- **食或水歸零 → 減益逐步升級**（如無法回血 → ATK/DEF↓ → 每回合掉 HP）→ **完全耗盡＝強迫撤退（委託失敗）**。
- 因此**出發前備包是核心策略**：糧、水、裝備共用背包負重上限，帶多少要精算（重用 `backpackEngine` 的重量/容量）。

## 8. 公會 RPG 系統：六維屬性 + 裝備 + 怪物 + 經濟（＝貓村×打怪融合）

> **定位（作者拍板）**：冒險者模式 = **貓貓村（材料/經濟/CAT幣）× 打怪（怪物/戰鬥/掉裝）** 的融合冒險。是獨立的一套 RPG 數值，與主線隔離。

### 8.1 六維屬性（`src/guild/domain/guildStats.js`）
角色有六維，每維影響不同（**提案值，實作可調**）：

| 屬性 | 影響（提案） |
|------|------|
| **HP** 生命 | 生命上限（歸零＝陣亡） |
| **ATK** 攻擊 | 箭矢傷害 |
| **AGI** 敏捷 | 出手順序 / 額外箭矢機率 / 閃避 |
| **DEF** 防禦 | 承受怪物攻擊的減傷 |
| **VIT** 體力 | **補給消耗減緩** + 背包負重上限（綁生存層） |
| **LUK** 幸運 | 掉寶率 / 爆擊率 / **雜貨評估價值** |

六維來源 = 射手等級基底 + 貓貓 + **公會裝備**（`calcGuildExpeditionStats` 彙總）。

### 8.2 公會專屬裝備（ARPG 式深度 itemization）
- **槽位（5，可再擴）**：弓 / 箭 / 護具 / 箭袋 / 藥水袋。
- **每槽有多種「基礎裝備 archetype」**：例如弓槽有 木弓 / 鐵弓 / 獵弓 / 長弓…，各有偏向。
- **每一種基礎裝各自有 6 個品級**（common→mythic）：即「木弓」本身就有 T1~T6 六階，越高階參數越強。
- **豐富六維參數**：每件（基礎 × 品級）帶 HP/ATK/AGI/DEF/VIT/LUK 的參數組合，可各有偏向（例：獵弓偏 AGI+LUK、長弓偏 ATK、藥水袋偏 VIT）→ 形成刷裝/配裝深度。保留**強化/詞綴/隨機屬性**的擴充空間。
- **資料表** `src/guild/data/guildEquipCatalog.js`：定義基礎裝 × 品級 × 六維參數。
- **來源**：打怪掉落 + 開寶箱機率 + 公會商店（CAT幣）。
- **隔離**：`guildEquip` 只被 `calcGuildExpeditionStats` 讀，永不進主線（grep 佐證）。

### 8.3 怪物（沿用打怪數值 + 2.5D 外觀 + 獨立 loot）
- **戰鬥數值**：直接取原打怪模式怪物設定（不重新平衡）。
- **外觀**：另做 **2.5D 俯視 sprite**（ComfyUI）。
- **獨立 loot 設定**：建一份**公會模式專屬掉落表**（`src/guild/data/`），與主線 lootTable **分開**——這樣公會掉的東西（公會裝/雜貨/CAT幣素材）**不會跟主線混在一起**。

### 8.4 經濟與貨幣（貓村×打怪的黏合劑）
- **通用材料**：遠征過程掉落**打怪 / 貓貓村都能用的材料** → 讓公會產出回饋主線經濟，公會不再是孤島。
- **雜貨/收藏品**（類地下城圖鑑）：撿到後**帶回公會評估價值** → 換得 **金幣 + CAT幣**（`LUK` 影響評估價值）。
- **CAT幣（新貨幣）**：公會限定，可在公會**兌換各式材料 / 購買公會裝備 / 特殊用途**。
- 新增 member 欄位：`catCoins`（CAT幣）、公會材料/雜貨庫存等，進 rules 白名單。

### 8.5 分期補充
- **先單人**，多人模式之後再做（架構預留，但 P1-P3 只做單人）。
