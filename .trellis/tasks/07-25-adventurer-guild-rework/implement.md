...
## P1：委託遠征核心玩法（獨立戰鬥 + 補給）
- [ ] `src/guild/domain/`：`rollExpedition`（隨機怪陣容 by client/danger）、`expeditionFlow`（逐 wave 狀態機）、`guildStats`（射手等級+貓+公會裝→HP/ATK/DEF）、`settleExpedition`。
- [ ] **公會自己的 2.5D 鳥瞰戰鬥**（`src/guild/ui/`）：等距戰場，玩家/貓/怪 sprite；戰鬥數學重用 `damage.js`/`score.js`，**不嵌 MonsterBattle**。
- [ ] 背包/補給：重用 `backpackEngine`，`consumeNodeSupplies` 每 wave 消耗食/水，shortage→減益；出發前備包畫面（裝備+補給 vs 容量重量）。
- [ ] 失敗處理（陣亡/補給耗盡 → 委託失敗，當天鎖同一張）。
...
## P3：公會專屬裝備（隔離）
- [ ] `src/lib/guildExpedition/guildStats.js`：`calcGuildExpeditionStats(member, guildEquip)`（**僅遠征用**）。
- [ ] 公會裝儲存 `members/{id}.guildEquip`、槽位（主武/防具/飾品/斗篷）、強化。
- [ ] 取得管道：委託獎勵掉落 + 公會商店（花公會幣/聲望）。
- [ ] 裝備有 `weight`，與補給共用背包容量（策略張力）。
- **驗證（關鍵）**：`grep` 佐證 `guildEquip`/`calcGuildExpeditionStats` **未被** MonsterBattle/dungeonDb/partyDb/worldBoss/constants.calcArcherStats 引用；主線 build 與數值不變。
