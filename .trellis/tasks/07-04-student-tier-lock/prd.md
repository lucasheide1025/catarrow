...
### 權限矩陣改為可調整設定（2026-07-04 追加）

- 原本「哪個分級能看哪些頁面」不寫死在程式碼常數，改為**教練後台可勾選調整的設定**（Firestore 儲存）
- 教練後台新增「權限設定」頁面：以表格呈現「頁面/功能 × 分級（受限／正式學生鎖定中／退休中）」的打勾矩陣，教練可自由調整每個分級能看到哪些頁面
- 表格預設值＝上方「權限矩陣」已確認的內容，教練可從這個基礎上再自行微調
- `official`（未鎖定）恆為全開，不需要、也不提供勾選（本來就是最高權限基準）
- `accountFrozen` 與系統維護鎖是獨立的更高優先權機制，不在這個可調矩陣的管轄範圍內（見上方對應章節）
...
- [ ] 教練後台有「權限設定」頁，以打勾矩陣（頁面 × 分級）調整受限/鎖定中/退休中三種狀態各自能看到哪些頁面；調整後即時生效，不需改程式碼重新部署
