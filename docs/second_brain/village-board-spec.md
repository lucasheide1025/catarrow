...
| 🎰 | 扭蛋幣格 | 扭蛋幣 ×1~3 |
| 🧪 | 藥水格 | 隨機藥水 ×1（品質隨 T） |
| 🎁 | 寶箱格 | **不射箭**（2026-07-30 改版）：踩到就隨機 **1~5 箱**，階級＝進場選的 T；族系箱/通用材料箱 |
...
## 3.5 獎勵演出（2026-07-30 新增）

踩到格子後的獎勵一律走共用元件 `BoardRewardPopup`（單機與組隊共用）：
**前置動畫（約 0.95 秒，圖示與台詞依格子類型）→ 獎勵逐項淡入 → 「收下！」**。
演出期間點背景不關閉，避免手滑跳過又看不到拿了什麼。遵守 `prefers-reduced-motion`。

## 4. 射箭格（怪物 / 挖礦 — 一律射 6 箭；寶箱格已於 2026-07-30 改為不射箭）

「射箭格」踩到時彈出視窗、**就地射 6 箭**，沿用現有完成度計分（`TargetFaceOverlay`/分數），完成度決定獎勵。**不看能力值**（核心是採集取代打怪）。
...
| 寶箱格 | **不射箭**；隨機 1~5 箱，階級＝進場選的 T（仍受建築 stage 上限夾住） |
| 箭露格 | +(15~50)×T |
| 金幣格 | +(80~400)×T |
| 扭蛋幣格 | ×1~3 |
| 藥水格 | 隨機藥水 ×1 |
...
- ✅ 射箭格：怪物/挖礦 **射 6 箭**，完成度決定加乘。寶箱格已改為不射箭、隨機 1~5 箱。
