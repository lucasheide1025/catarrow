...
| ⛏️ | 挖礦格 | 該材料 ×6~15（2x~5x）＋15% 額外皮草/家族素材 |
| 👾 | 怪物格 | 就地射 3 箭 → 完成度 S/A/B/C → 300/200/150/100% × 挖礦基準；S 送寶箱 |
| 💧 | 箭露格 | 箭露 +(15~50)×T |
| 🪙 | 金幣格 | 金幣 +(80~400)×T |
| 🎰 | 扭蛋幣格 | 扭蛋幣 ×1~3 |
| 🧪 | 藥水格 | 隨機藥水 ×1（品質隨 T） |
| 🎁 | 寶箱格 | 族系箱/通用材料箱（接現有箱系統） |
...
- 踩到 → 彈出「遭遇」視窗，**就地射 3 箭**，沿用現有完成度計分（`TargetFaceOverlay`/分數）。
- 判定：S=300% / A=200% / B=150% / C=100% × 挖礦基準獎勵；S 級額外送寶箱。
- **不看能力值**（核心是採集取代打怪）。
- 組隊：全員各射或房主代表射，獎勵發全員（實作時定，建議房主代表射，全員同享）。
...
- 棋盤所有「材料類」掉落的階級上限 = `getPlayerMaterialTierCap(profile)`（依玩家已解鎖的最高素材/地下城 tier）。
- 高階材料機率隨進度提升；不動 `getBuildingStage`（維持 5 階建築）。
...
|---|---|
| 素材格 | 該模式材料 ×3~6 |
| 挖礦格 | 該材料 ×6~15 ＋15% 皮草/家族素材 |
| 怪物格 | S/A/B/C = 300/200/150/100% × 挖礦基準；S 送寶箱 |
| 箭露格 | +(15~50)×T |
| 金幣格 | +(80~400)×T |
| 扭蛋幣格 | ×1~3 |
| 藥水格 | 隨機藥水 ×1 |
...
## 11. 唯一待定
- **每日骰子發放量/上限**：提案 +15/日、上限 45。作者確認即可定稿。
