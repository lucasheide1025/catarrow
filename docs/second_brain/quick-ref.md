...
### 王房抽王（2026-07-19 定案：全系統只有一套）

```js
// 唯一的抽王引擎：src/lib/dungeonBossEncounter.js
createLockedDungeonBossEncounter({ runId, roomId, family, difficultyTier, consecutiveNonBoss, lockedEncounter })
//   → { encounter:"miniBoss"|"boss", monsterSnapshot, nextConsecutiveNonBoss, ... }（Object.freeze）
//   Tn 只出 Tn 的王（小王每族每階 2 隻 / 大王 1 隻），連續小王後保底大王
//   seeded by runId+roomId ⇒ 同一場遠征重整不會換王
//   lockedEncounter 命中 isLockedDungeonBossEncounter() 就原封不動回傳（不重算）

// 生成端：src/lib/dungeonExcavation.js::rollExcavationBoss(difficulty, family, excavation, { runId })
//   → { boss, bossEncounter, miniStreak }；地下城物件存 boss / bossEncounter / bossRunId 三個欄位
//   保底計數欄位：members/{id}.dungeonExcavation.miniBossStreak

// 戰鬥端：DungeonExpedition.jsx:341
//   fixedBoss = expansionBossEncounter?.monsterSnapshot || (isGuest ? guestFixedBoss : excavation?.boss)
//   L336 把 excavation.bossEncounter 當 lockedEncounter 傳進去 ⇒ 沿用生成時那筆，不重抽
```

**⚠️ 絕對不要再寫第二套抽王。** 2026-07-19 曾在 `dungeonExpansionMonsters.js` 加了
`drawDungeonBossEncounter`，忘了 7/18 就已有 `createLockedDungeonBossEncounter`，
結果變成「選擇畫面預覽一隻王、實際打到另一隻」。已刪除，統一走上面這條。

**⚠️ 升降級沿用同一個 `bossRunId`**（`upgrade`/`downgradeExcavationDifficulty`）：
王因此是決定性的，升上去再降回來得到原本那隻。降級免費，不鎖 runId 就是無限刷王。

**⚠️ `rollExcavationBoss` 的 try/catch 是靜默 fallback**：某族某階湊不齊「2 小王+1 大王」
時會安靜退回舊 `drawExpeditionBoss`（抽到雜怪），畫面看不出異狀。
`dungeonBossEncounter.test.js` 有「六族 × 六階都湊得齊王池」整表釘死，別刪。

**⚠️ 訪客路徑**（`GuestDungeonEntry.jsx` / `DungeonExpedition.jsx` 的 `guestFixedBoss`）
仍走舊 `drawExpeditionBoss`，尚未接。
