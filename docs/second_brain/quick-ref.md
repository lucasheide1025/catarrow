...
{
  progress: 0,              // 0~100
  dailyArrowsUsed: 0,       // 今日練箭量（用於稀有度骰）
...
// 進度累積
// 登入 +10（MemberApp init）/ 報到 +10（submitCheckin）/ 每箭 +0.3（addRoundArrows）
// 金幣強化：揭曉時隨機 500~2000 金幣強化一級

// 稀有度骰子（依 dailyArrowsUsed 決定權重）
