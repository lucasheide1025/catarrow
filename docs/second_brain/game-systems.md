...
### 時間（`villageGoalSchedule.js`，後台可調）
```
完成期限 30 天（所有階級）／結束後冷卻 3 天
公式：基礎時數 + 階級 × 每階增量（預設 720 + 0）
```

### 階級
Lv1-5 發展期(0)｜Lv6-10 繁榮期(1)｜Lv11-15 興盛期(2)｜Lv16+ 輝煌期(3)
⚠️ **階級要存進目標文件**，村莊會在這一個月內升級。

### 目標值（8 種，2026-08-03 下修階級 2/3）
| 目標 | 0 | 1 | 2 | 3 |
|---|---|---|---|---|
| 全員箭數 | 5,000 | 15,000 | 25,000 | 38,000 |
| 全員傷害 | 50,000 | 150,000 | 250,000 | 380,000 |
| 全員擊殺 | 20 | 50 | 80 | 120 |
| 採集進度 | 1,500 | 4,000 | 6,500 | 10,000 |
| 採集人次 | 20 | 45 | 70 | 100 |
| 指定素材 | 80 | 180 | 280 | 400 |
| 指定資源 | 60 | 140 | 220 | 320 |
| 繞圈 | 30 | 70 | 105 | 150 |

### 獎勵三層（`villageGoalRewards.js`，與世界王同一套數學）
| 層 | 內容 |
|---|---|
| 出席保底 | 箭露 300/500/900/1600、金幣 200/300/500/900、抽獎幣 3/5/8/12 |
| 努力分潤 | 每人份 箭露 200/300/550/1000…，依 **√貢獻** 分，鍋子＝每人份×人數 |
| 達成慶功 | 咪咪箱 1/1/2/3 ＋ **七族材料箱** 2/3/4/5 個一族（14/21/28/35 箱） |
| 安慰獎 | 箭露 100/150/250/450（舊版固定 30，推一個月只拿 30 沒人想推） |

⚠️ **咪咪箱 ≠ 貓貓箱**；⚠️ 材料箱**只能用 `family_mat`**；⚠️ 族別用**七族**那組。
（三個都不會報錯，只會靜靜地做錯事——細節見 changelog 2026-08-03）

---

## 🏅 實體榮譽加成（2026-08-04 重做）

> 道館裡**教練親手發的**實體徽章與檢定證，不是遊戲內產出。

| 榮譽 | 難度 | 軸 | 每顆 |
|---|---|---|---|
| 🐱 肥貓章 | **最難** | ATK | 銅 +1／銀 +4／金 **+12** |
| 🏆 積分章 | 中間 | DEF | 銅 +1／銀 +3／金 +9 |
| 🏅 成就章 | **最好拿** | HP | 銀 +3／金 +8／黑 +15 |
| 🎯 射手證 | — | 全部 | 藍證 ATK+10 DEF+10 HP+100；**金證再 ×1.05** |

### ⚠️ 三條不能破的規則

1. **不設上限**（作者：「我就是煞車，而且這遊戲會繼續更新往上攀升」）。
   發放速度本身就是節流閥。**不要因為看起來會通膨就加回上限。**
2. **加在所有 `Math.min` 之後**（`calcArcherStats` 末段）。放進夾制裡的話
   老手早就頂到天花板，再拿章完全沒感覺——那才是 2026-08-04 之前的真正問題。
3. **難度排序是作者定的，不要從點數權重推**。成就章要 160 點才封頂看起來最難，
   實際上它最好拿。配 HP 也是刻意的：HP 會被等級加成稀釋（117 級 +580），
   影響最小的軸配最容易拿的榮譽。

⚠️ 金證的 5% 乘的是 `calcArcherStats` 回傳值（含章），**不含等級加成**。

### 玩家怎麼看到
「我的」→ 等級卡下方「📊 我的三圍是怎麼來的？」（`describeStatSources`）。
**加成再大，看不到就等於沒有。**
