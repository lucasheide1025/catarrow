...
- **掉落率調高**：`equipChance` 10%~52% → **45%~100%**（T1~T6）。理由：一天最多 18 張委託、每張只能接一次（勝敗都結案），舊值讓新手（只能接 3 張 T1）期望值僅 **0.3 件／天**＝等於刷不到。新值新手約 1.4 件／天、高階全清約 12 件／天。**給得大方不會破壞刷裝意義**——低階裝主要是分解成碎片的燃料，真正稀有的是「高品級 × 好詞綴」。
...
- 129 測試全過（新增 4：命中率計算、分帶門檻遞減、神射 vs 生疏、首領額外判定）。
