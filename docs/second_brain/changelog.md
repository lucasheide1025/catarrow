...
## 2026-07-31（世界王重製：宣告制弱點・全新討伐版式，`?raid` 沙盒可看，未接線上）

**作者回報三件事**：沒有打世界王的感覺、戰鬥過程無趣、**低等玩家打不動，都是高等玩家在貢獻**。

**根因只有一個**：世界王是一隻血很多的普通怪。`src/lib/damage.js:221` 的註解白紙黑字寫著
`Single arrow damage for world boss (no part system)`——一般怪有 11 個部位（`monsterData.BODY_PARTS`），
世界王**刻意關掉**，傷害只剩 `分數 × 攻擊力`。實測貢獻差距 **34 倍**（新手每箭 ~18、114 級 ~183，
再加分數倍率複利），而且整場 30 箭玩家**沒有任何一刻需要做決定**。

### 做了什麼

**① 宣告制（call your shot）**——射之前先選部位，每個部位一條分數門檻：
眼 ≥9／心 ≥7／腿 ≥5（打斷蓄力）／尾 ≥3（削弱大招）。**分數達標就是命中，不再擲骰**，
玩家賭的是自己的穩定度——這正是射箭真正的技術。沒達標＝擦過，眼/心 的一般傷害還要 ×0.5（賭輸要有代價）。

**② 弱點傷害不乘 ATK，是王最大血量的固定比例**（眼 0.15%／心 0.09%／腿 0.075%／尾 0.045%）。
這是整份改版最重要的一條：只要傷害是 ATK 的函數，怎麼包裝都還是碾壓。
再把世界王的一般傷害壓到五成（`RAID_NORMAL_DAMAGE_SCALE = 0.5`）——**只動配重不動總量**，
王的死亡速度幾乎不變，不必回頭改 24 隻王的血量。結果：**34× → 3.16×**。

**③ 破防槽算命中次數、不算傷害**。新手打腿 ×4 拿 8 點，老手打眼 ×4 拿 12 點——**差距只有 2 倍**。
槽滿 → 全場增傷 ×1.5 兩回合。排行榜要同時出傷害榜與**破防貢獻榜**（新手真的排得上去的那個）。

**④ 全新流程：預告 → 抉擇 → 分歧**。R2/R4 蓄力（**沿用既有 24 王的 r2Strike/r4Finisher 設定，
一行資料都沒重寫**），蓄力時弱點外露 +50%，此時要決定貪輸出還是分箭打腿。
斷成 → 王硬直、下回合全部位開放且傷害 ×2；沒斷成 → 大招＋破防槽被打掉 20%。

**⑤ 三階段**（100/66/33%）各護住不同部位，強迫改變宣告；第三階段破防累積 ×1.5。

### 架構

新開 `src/worldboss/`（頂層，比照 `src/guild/` 的 domain/ui/data 三層）。
**舊的 `src/components/worldboss/` 一行沒動**，新舊並存。入口 `?raid` 是假資料沙盒。

⚠️ **鐵律：domain 產生 log，UI 照 log 原順序重播**（`raidTimeline.buildRaidTimeline`）。
公會就是因為按事件類型分桶，才會「怪物全滅前直接跳過戰鬥動畫」。所有聲光效果掛在 log 事件上。

### 踩坑

- **`Number(null) === 0`**：方位加碼的 `clockOf(nx, ny)` 沒擋 null，按分數鍵的玩家（沒有座標）
  被當成「正中心＝12 點」而**白拿加碼**。測試抓到的，已在 `clockOf` 開頭擋掉。
- **討伐永遠不會結束**：`state.finished` 算對了，但沙盒的 `onFinish` 只存戰報沒有離開戰鬥畫面，
  回合一路加到「第 8/5 回合」。修法是 RaidScreen 自己在 `finished` 時鎖住輸入並蓋結算層——
  **不能只靠呼叫端守**。已加 4 個回歸測試。
- **測試不要寫死平衡數字**：調 dmgPct 時 4 個斷言同時碎掉，改成從 `WEAK_POINT_MAP` 推導。

### 美術

`scripts/gen-raid-art.py`（新）生 8 張討伐場地 2:1 背景 → `public/assets/raid/raid_bg_<族>.webp`。
⚠️ **王的立繪一律不生**：教練/師母/YUMI 是真人、九隻貓王是真貓，六族王也已有既有立繪。

### 現況

domain 全套 + 92 個測試（含平衡模擬 `raidBalance.test.js`），沙盒 `?raid` 已能完整跑五回合。
**還沒接線上世界王**——Firestore 欄位（breakGauge/breakPoints）、破防貢獻榜、`WorldBossAttack` 接線未做。
