...
## 2026-07-19（裝備品級曲線改凸曲線 + 材料需求依品級分級 + 修精煉舊需求覆蓋）

**新檔 `src/lib/equipGradeCurve.js`**（純葉節點模組，不 import 任何東西，避開之前踩過的跨檔 TDZ 坑）。

- **為什麼改**：舊公式 `(品級index × 5 + 1) + plusLevel` 是**完全直線**——30 個階梯每步都剛好 +1，所以「普通+4 → 稀有+0」的品級突破跟一般 +1 一模一樣。但突破要付王之印記＋暴增金幣（傳說 12000、神話 30000）。**付出跳很多、回饋完全沒跳**，這是玩家覺得升不升沒差的根因。
- **新曲線**：每品級的單級步長遞增（普通 1 → 史詩 2 → 神話 3 → 永恆 7），**突破 = 新品級的 2 個步長**，越高階跳越明顯。各品級 +0→+4：普通 1→5、稀有 7→11、精英 13→17、史詩 21→29、傳說 33→41、神話 47→59。
- **保證沒人被削**：逐格比對 30 個階梯，新值全部 ≥ 舊值（有測試守住）。代價是神話+4 從 30 → 59（約 2 倍），ATK 四格合計 120 → 236，而非裝備成長上限約 160 → **裝備會成為主導項，怪物數值可能需要跟著調**（尚未做，待觀察實際體感）。
- **T7~T9 上古／天啟／永恆**：只有數值定義在 `GRADE_CURVE` + `PREVIEW_GRADES`，**刻意不放進 `EQUIP_GRADES`**——`upgradeEquipSlot` 的 `isMaxGrade` 是用 `EQUIP_GRADES.length - 1` 判斷，放進去會讓玩家真的升上去。測試明確守住「神話仍是最高可升品級」。
...
- **踩坑**：精英以上要 4+3+1 = **8 種**，但只有六族。舊 `generateRandomMats` 從單一洗好的六族陣列切片會**切爆**。改成每個 tier 各自洗牌——不同 tier 的 id 本來就不同，同族重複出現正常且不撞 id。
...
- 驗證：53 suites / **357 tests 綠** + build `Compiled successfully` 無警告 + 四支改動檔 `no-undef` 專項掃描乾淨。
